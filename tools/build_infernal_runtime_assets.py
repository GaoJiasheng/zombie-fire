...
def beveled(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], cut: int, fill, outline=None, width: int = 1) -> None:
    x0, y0, x1, y1 = box
...
def button_asset(width: int, height: int, primary: bool) -> Image.Image:
    # Every control is authored directly at its requested dimensions.  Width
    # families change their corner/coupler construction; there is no stretching.
    ratio = width / height
    ...
    glow = (255, 102, 28, 125 if primary else 72)
    copper = (205, 92, 37, 245 if primary else 205)
    beveled(draw, (0, 0, width - 1, height - 1), cut, (17, 18, 21, 238), glow, max(2, height // 20))
    beveled(draw, (margin, margin, width - 1 - margin, height - 1 - margin), max(5, cut - margin), (8, 13, 17, 244), copper, max(1, height // 36))
    inner = max(4, height // 9)
    beveled(draw, (inner, inner, width - 1 - inner, height - 1 - inner), max(4, cut - inner // 2), (7, 14, 18, 250), (63, 42, 32, 220), 1)
    # Independent structural nodes: compact double furnace, standard central
    # coupler, long multiple heat sinks.
    nodes = 2 if family == "short" else 3 if family == "standard" else 5
    for i in range(nodes):
        t = (i + 1) / (nodes + 1)
        x = round(width * t)
        r = max(2, height // 18)
        draw.ellipse((x - r, margin, x + r, margin + r * 2), fill=(255, 116, 30, 175 if primary else 110))
        draw.ellipse((x - r, height - margin - r * 2, x + r, height - margin), fill=(134, 53, 26, 150))
    rail_y = height // 2
    rail_pad = cut + max(8, height // 7)
    draw.line((rail_pad, rail_y, width - rail_pad, rail_y), fill=(255, 90, 18, 34 if primary else 18), width=max(1, height // 24))
    # Dark translucent copy well keeps English and Chinese equally legible.
    draw.rounded_rectangle((rail_pad, height * 0.27, width - rail_pad, height * 0.73), radius=max(2, height // 10), fill=(2, 8, 12, 82))
    return image
