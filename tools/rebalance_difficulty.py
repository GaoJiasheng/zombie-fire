...
def total_spawn_seconds(waves: list[dict], economy: dict, level_no: int) -> float:
    duration = 0.0
    for wave in waves:
        count_mult = late_wave_count_mult(economy, wave_number(wave), level_no)
...
def difficulty_coef(n: int, boss_level: bool, waves: list[dict], zombies: dict, bosses: dict, characters: dict, weapons: dict, economy: dict) -> float:
    target_hp = estimate_player_dps(characters, weapons, economy, n) * total_spawn_seconds(waves, economy, n) * target_pressure_ratio(n, boss_level)
    raw = target_hp / (float(base_hp_ref(n)) * enemy_hp_weight(waves, zombies, bosses, economy, n))
...
def level_pressure(level: dict, zombies: dict, bosses: dict, economy: dict) -> float:
    ...
    for wave in level.get("waves", []):
        ...
        if "boss" in wave:
            raw += float(bosses[wave["boss"]].get("hp_coef", 1.0)) * late_wave_hp_bonus(economy, wave_no, True, level_no, card_picks) * boss_level_bonus * boss_survival_hp_ramp(economy, level_no) * 8.0
