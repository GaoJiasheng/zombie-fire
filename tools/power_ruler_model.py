#!/usr/bin/env python3
"""单一战力口径 5.0 的 Python 镜像与通关线求解器。

玩家仍只看到一个“战力”，但内部不再把输出与生存做几何平均。每关先离线生成
清群、Boss、防线三份容量合同；运行时按当前装备、永久技能等级与本关确定能拿到
的卡牌预算计算三条比值，最终只取最短板。5.0 继续使用同一战斗模拟器估出的
防线门槛，同时把武器的清群/Boss 两轴接到 Godot 真实碰撞基准，避免理论弹道、
穿透与连锁收益在静态公式中重复计价：
...
def weapon_axis_calibration(economy: dict, weapon_id: str, axis: str) -> float:
    """Return the measured collider-throughput correction for one weapon axis.

    The table is generated by generate_weapon_power_profiles.py.  A 1.0
    fallback keeps legacy fixtures readable, while release CI requires every
    shipping weapon to be present in the generated single-source table.
    """
    ruler = economy.get("power_ruler", {}) or {}
    profiles = ruler.get("weapon_runtime_axis_calibration", {}) or {}
    row = profiles.get(weapon_id, {}) or {}
    return max(float(row.get(axis, 1.0)), 0.01)
...
def build_power_contract(level: dict, requirement: dict, characters: dict,
    ...
    crowd_required = max(
        reference_offense * axes["crowd"] * mob_element
        * weapon_axis_calibration(economy, "weapon_autocannon", "crowd"),
        0.01,
    )
    ...
    reference_boss = (
        reference_offense * axes["boss"] * boss_element
        * weapon_axis_calibration(economy, "weapon_autocannon", "boss")
        if boss_weights else 0.0
    )
    ...
    # The full runtime roster is already charged by boss_capacity.  Scaling the
    # headline number by copy count as well made Boss milestones spike twice
...
def maxed_free_build_for_level(level: dict, contract: dict, characters: dict,
    ...
    if not all((free_characters, free_weapons, free_armors, free_chips, free_pets)):
