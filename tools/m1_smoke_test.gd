...
func _verify_recommended_power_calibration(save_manager: Node, data_loader: Node) -> void:
	...
	_expect(int(observed_breakdown.get("power", 0)) == 4439, "observed level_080 build must include all three identical Necromancers, got %d" % int(observed_breakdown.get("power", 0)))
	_expect(int(observed_breakdown.get("recommended", 0)) == 2528, "level_080 recommendation must include all three authored Necromancers")
	...
	_expect(float(observed_breakdown.get("power", 0)) / 2528.0 > warning_gate, "observed level_080 build must no longer trigger the false severe warning")
