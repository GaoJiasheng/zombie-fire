...
def button_asset(width: int, height: int, primary: bool) -> Image.Image:
    ...
    rim = (142, 226, 255, 205 if primary else 142)
    violet = (132, 104, 224, 125 if primary else 82)
    beveled(draw, (0, 0, width - 1, height - 1), cut, (20, 31, 45, 238), rim, max(2, height // 20))
    beveled(draw, (margin, margin, width - 1 - margin, height - 1 - margin), max(5, cut - margin), (8, 20, 34, 246), violet, max(1, height // 36))
    inner = max(4, height // 9)
    beveled(draw, (inner, inner, width - inner - 1, height - inner - 1), max(4, cut - inner // 2), (4, 13, 24, 248), (190, 242, 255, 110), 1)
    nodes = 2 if family == "short" else 3 if family == "standard" else 5
    for i in range(nodes):
        x = round(width * (i + 1) / (nodes + 1))
        r = max(2, height // 18)
        draw.polygon([(x, margin), (x + r, margin + r), (x, margin + r * 2), (x - r, margin + r)], fill=(174, 240, 255, 155 if primary else 92))
        draw.polygon([(x, height - margin), (x + r, height - margin - r), (x, height - margin - r * 2), (x - r, height - margin - r)], fill=(132, 98, 224, 100))
