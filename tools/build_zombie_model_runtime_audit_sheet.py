...
OUT_DIR = ROOT / "tmp/zombie_model_runtime_audit_2026_07_26"
...
def compose(cases: list[tuple[str, Path]]) -> None:
    ...
    sheet.save(SHEET)
    print(f"zombie model runtime audit sheet: {SHEET}")


def main() -> int:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    base = {"level_id": "level_001", "warmup_frames": 6}
    capture_specs = [
        ("8 种重制模型", "redesigned", {}),
        ("全部 20 种轮廓", "roster", {}),
        ("24 怪密度", "dense", {}),
        ("高屏安全区", "redesigned", {"viewport_size": [1080, 2348]}),
    ]
    rendered: list[tuple[str, Path]] = []
    for label, mode, extra in capture_specs:
        payload = dict(base)
        payload["debug_zombie_model_showcase"] = mode
        payload.update(extra)
        suffix = "_tall" if extra else ""
        rendered.append((label, capture(mode + suffix, payload)))
    compose(rendered)
    return 0
