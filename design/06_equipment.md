...
| ID | 名称 | 攻击 | 自带技能 | 元素 |
|---|---|---|---|---|
| `pet_turret_drone` | 机枪无人机 | 自动点射首要威胁 | 过热爆发：短时同步提高自身射速与伤害 | 物理 |
| `pet_fire_imp` | 火灵 | 抛火球 | 熔火爆发：对首要威胁周围造成范围火伤与强化灼烧 | 火 |
| `pet_frost_wisp` | 霜灵 | 冰锥点射 | 寒霜领域：大范围冰伤与强化减速 | 冰 |
| `pet_volt_orb` | 电球 | 雷击首要威胁 | 电弧过载：连续打击多个高威胁目标，目标数随等级成长 | 雷 |
| `pet_medic_drone` | 医疗无人机 | 无直接攻击 | 三层修复：波次整备、持续维修、低血应急救援 | — |
| `pet_collector` | 拾荒机器人 | 弱点射 | 战场回收：每波结算一批随等级成长的等效击杀收益 | 物理 |
...
- 医疗无人机的修复按基地最大生命成长，不会像固定数值一样在后期失效；应急救援只在低血量且冷却完成时触发，保留防线管理压力。
- 六只宝宝都必须拥有 `pet_skill`；战斗表现、收藏详情、升级预览与战力估算读取同一份技能数据，不允许只写说明而没有实际效果。
