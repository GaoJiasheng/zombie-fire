...
- **Owner-approved direction**: the app remains free, without ads, subscriptions, stamina, random paid rewards or paid currencies. Four permanent visual themes are planned at a US$1.99 reference price, together with four permanent attribute-specific Apocalypse Arsenal sets.
...
- **Phase 1 lock**: the owner changed the first production set to Neon Tempest and the complete Thunder Apocalypse weapon/armor/chip/pet set. The other three catalog sets remain planned but may not begin before the first end-to-end set passes acceptance.
- **Source of truth**: `design/21_premium_themes_and_apocalypse_arsenal_plan.md` contains the full catalog, entitlement matrix, four-series briefs, upgrade milestones, balance definition, asset/package budgets, architecture, QA gates and cross-session handoff checklist.
