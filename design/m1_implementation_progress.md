...
## 2026-09-01 · 一级跳弹首目标重撞修复

- `skill_ricochet` 的 `chain=1` 数据与解析一直正确；故障位于弹射子弹从首个命中中心外 18px 出生，却没有继承首目标排除项，因此会被半径 70px 的同一敌人碰撞体立即截获。
- `_spawn_chain_projectiles` 现在把已确认首目标写入子弹 `hit_target_ids`，普通敌人与 Boss 均不会再次消费这次弹射，既有 430px 选敌半径、伤害衰减与链深度保持不变。
- M1 smoke 使用一级自动机枪与两个邻近假目标验证完整生成链，锁定首目标零二次命中和次目标一次命中。

## 2026-08-30 · C 阶段战力 6.0 与付费套 v2

- 有效战力已改为中性纯构筑函数，推荐战力从 B2 99×10 验收结果反解；防线投影计分和稳定技能投影修复已进入同一 Python/GDScript 镜像。
- 付费武器现在允许弹药卡覆盖本命元素，子弹保持单一最终元素；燃爆、脆化、过载等本命机制只在最终元素仍匹配时生效。
- Tier B 审计终值：雷霆 1.249x、炼狱 1.251x、绝对零度 1.250x、黄金法则 Lv50 1.251x / Lv65 1.556x。黄金法则移除独有 endgame 层，前 50 级按共同口径归一化，Lv51–65 使用标准 `atk_growth_per_level=0.08` 且不延伸基础攻速；该段金币总成本仍为 192,960。
- 商店结构值已重标为 30/50/70/90 与黄金法则 US$8.99 / US$6.99；可见中英文“元素主宰”文案未改，候选稿仅进入审计报告等待 Owner 审批。
- 版本 3 老存档迁移后会在菜单显示一次中英文标尺变化说明；版本 4 新存档默认不弹，确认状态进入存档并由 smoke 锁定。
- 完整 m1 smoke 已通过，包括分段成长端点、免费武器旧公式、弹药覆盖、解锁/价格、存档迁移和商业化诚实隐藏断言。
- 离屏视觉矩阵覆盖 1080×1920 / 1320×2868、中英文关卡/配装/结算共 12 张；全部尺寸逐像素匹配且自动审计零问题，高屏四张关键页人工抽看通过。
- 普通 099 实测硬门禁已过：雷霆、炼狱、绝对零度各沿用未漂移的固定十种子 10/10；黄金法则 1.556x 二次返工后重新实测 10/10、0 timeout，最慢 257.02 秒。
- 86/2207 与 90/1103 的确定性 A/B 完整 run 对象逐位一致；免费 99×10 以同一 v6 合同的付费改动前提交 `4be071fb` 为零泄漏基线，低并发复判主机 timeout/outlier 后 990/990 完整 run 对象逐位一致（共同 SHA-256 `00b7e346…`），不把更早 B2/v5 合同差异误记为付费回归。
- 黄金法则二次返工使用当前 4a fixture（SHA `128fe3b8…`）分别运行修前镜像与修后 worktree，免费 99×10 原始结果 990/990 直接逐位一致、双方 canonical run SHA-256 均为 `9532dc4e…`、0 timeout；未复用带旧 level-062 输入的上一轮 raw sweep。
- 非视觉 RC 在首项资产包检查停下；隔离 worktree 缺少 111 个已索引生成源/联系表文件。当前与 4a583104 修前镜像均退出 1，112 行输出逐字节一致，作为隔离目录既有资产源缺口保留；其余静态、Godot、数值与探针门禁已独立执行。
- 商业化 0 触发的产品根因已按 Owner 方案 B 修复：付费装备仍授予 1 级，但权益首次到账时冻结除本套武器外的最高既有武器等级，追赶段读取 economy 的 `0.0005` 成本乘数，恰好到达冻结等级后恢复全价；付费武器自身后续成长不会无限续折扣。
- 四套在 30/50/70/90 揭示点的整套追平价分别为 83 / 206 / 214 / 249 金币，占结算奖励到账后余额 1.74% / 3.54% / 4.88% / 5.04%。免费装备 1178 项成本向量与 `7db4a725` SHA-256 完全一致，金币与 XP 覆盖门禁变化为 0。
- 商业化投影现在统一折算到玩家最高武器等级并披露追平金币；重标后配装触发 34/69（49.3%，旧 47.2%）、结算触发 17/69（24.6%，旧 25.0%），偏差 +4.3% / −1.4%。逐套分布仍如实保留：雷霆与绝对零度在 B2 当前角色构筑下没有单独成为最优候选，不通过改 coef 或负提升推荐掩盖。
...
- Existing paid-series behavior remains intact. The 43-route Neon screenshot matrix passes after repairing stale audit-only API calls and three legacy weapon fixture IDs; the accepted Thunder max-build audit remains exactly `1.550x` the free Volt reference build.
- Produced six Owner-review prototype boards for Infernal Dominion and Inferno Apocalypse: four full hero outfits, eight free-weapon coatings, three mechanically distinct plasma-projector candidates with armor/chip/phoenix, UI/world/logo/material language, an exact 1080×2340 battle composite, and a ten-panel combat-effect storyboard.
- All generated source references and reproducible prompts live under `assets/production/source_refs/generated/premium_infernal_dominion_inferno_phase2_2026_07_31/`. They are prototypes only; no Infernal runtime sprites, equipment data, combat values, or purchasable products have been added.
- Step 2 stays gated on Owner approval of the overall direction and selection of projector candidate A, B, or C.

### Four-Hero Shooting Animation Redesign (2026-07-26)

- Rebuilt the complete four-character/eight-weapon/three-aim-direction matrix as
  eight-frame transparent runtime sequences: 96 sequences and 768 production
  frames in total.
- Every sequence now follows one semantic contract: F1 brace, F2 ignition, F3
  recoil peak, F4 mechanism response, F5 counter-motion, F6 recovery, F7 settle
  and F8 ready. Vanguard absorbs weight, Blaze drives sustained fire, Frost
  controls precision weapons and Volt rebounds elastically.
- Weapon-specific duration, pre-fire lead and recoil curves distinguish the
  autocannon, flamethrower, cryocannon, tesla coil, toxic launcher, railgun,
  shotgun and plasma cannon without changing fire rate, damage or targeting.
- Runtime firing switches to the exact F2 ignition texture before resolving the
  muzzle and spawning VFX/projectiles. Projectiles still launch immediately, so
  the visual anticipation does not add input or combat latency.
- Corrected a legacy coordinate-space mismatch: the 96 muzzle anchors were
  authored for sprite scale `0.64`, while the accepted runtime character scale
  is `0.512`. All left/center/right offsets now share the real `0.8` conversion
  and sit on the visible barrel instead of above it.
- Manual touch aim, manual lock and automatic aim selection are unchanged.
- Motion-only image-generation references, exact accepted-character baselines,
  prompts, manifest, storyboard, full 32-loadout sheet and four runtime captures
  live under
  `assets/production/source_refs/generated/character_shooting_animation_redesign_2026_07_26/`.
  Generated references guide body mechanics only and never replace hero identity.
- `tools/check_attack_animation_motion.py` permanently gates all 96 sequences:
  exact frame count, adjacent motion, ignition-to-recoil delta, aim-opposed
  recoil direction, silhouette area stability, transparent safe margins, muzzle
  attachment and left/center/right firing corridors.
- `tools/m1_smoke_test.gd` additionally proves the complete 32-loadout matrix
  loads eight frames and `_play_character_attack()` selects the exact ignition
  frame. Runtime capture also caught and corrected a stale Godot import cache
  before acceptance.
- Final verification passes all 39 Release Candidate checks: 9,222 production
  files, 353 `res://` references, all 99 level-pressure rows, data/card
  simulation, the 96-sequence animation gate, Godot/battle/save startup, the
  full M1 smoke test and 79 routed runtime screenshots.

### Ordinary Zombie Attack Animation Redesign (2026-07-26)

- Rebuilt all 20 ordinary-zombie attacks from identity-specific authored
  anticipation/contact/recovery renders, producing 160 transparent 512×512
  runtime frames. Frame 4 is the explicit contact pose for every identity.
- Added `attack_animation` data contracts to `data/zombies.json` for semantic
  mode, complete duration, contact ratio, contact frame and baseward lunge.
- Normal base attacks now settle `breached` exactly at visual contact instead of
  at animation start. Recovery cannot duplicate damage, and enemies idle between
  attack cycles rather than looping attack frames.
- Profiled playback maps frames 1–3 into anticipation, frame 4 onto the authored
  contact ratio, and frames 5–8 into impact/recovery, so heavy and rapid enemies
  keep distinct timing without changing damage or cooldown balance.
- Hit recoil no longer hides an already committed base-contact pose. Summoner,
  ranged-spit and regeneration mechanics now call their special pose hooks.
- Source triptychs, chroma originals, prompts, manifest and reproducible builder
  live under
  `assets/production/source_refs/generated/zombie_attack_animation_redesign_2026_07_26/`.
- `tools/check_zombie_attack_animations.py` gates frame count/order, 512 canvas,
  transparent margins, detached fragments, key-pose delta, runtime loader and
  special hooks. `tools/m1_smoke_test.gd` proves all 20 contact events occur on
  runtime frame 4 exactly once.
- Runtime phone-scale review is generated by
  `tools/build_zombie_attack_runtime_audit_sheet.py`; the accepted sheet is
  `assets/production/contact_sheets/contact_zombie_attack_runtime_2026_07_26.png`.
- Four deterministic attack-stage routes are part of the permanent full-screen
  visual matrix, covering all 20 identities at anticipation, authored contact
  and recovery without spawn-aura clutter.
- Final release validation passes all 39 checks: 8,805 production files, 353
  `res://` references, 99 pressure rows, Godot/battle/save boot, full M1 smoke
  and 79 routed runtime screenshots. The validation loop also updated the
  pre-existing model-silhouette gate from its superseded four-frame attack
  assumption and corrected production-relative manifest resolution in the new
  attack gate.
...
## Bottom Combat Resource Row Optical Drop (2026-07-19)

- **Owner evidence**: the physical-iPhone battle screenshot showed the gold counter, XP track and base-HP track floating too close to the hero and defense-line composition.
- **Scoped layout correction**: all three resource widgets now share a `20px` internal row drop inside `Hud/BottomBar`. The dock, hero, pet, skill shelf, active-skill button and gameplay geometry are unchanged.
- **Regression coverage**: M1 smoke asserts the new gold, XP and HP baselines across six standard/tall viewport heights and requires every widget to remain inside the bottom dock.
- **Visual evidence**: `tmp/bottom_resource_row_20260719/battle_1080x1920.png` and `battle_1080x2340_safe.png` were manually reviewed; the row is lower and aligned without clipping or colliding with the safe-area edge.
- **Verification**: `python3 tools/check_release_candidate.py` passed the complete gate. The HUD overlap audit retains a `10.1px` minimum bottom-resource gap, and all 46 routed screenshots passed.

## Character Collection Button-Label Optical Alignment (2026-07-21)

- **Owner evidence**: a physical-iPhone capture showed the mathematically centered Glow Sans labels reading low inside the collection screen's asymmetric armored button bezels.
- **Scoped correction**: collection card actions, detail actions and the bottom back button retain their existing controls and layout, while their label boxes share one `-4px` optical Y correction. Resource chips, card copy and page geometry are unchanged.
- **Regression coverage**: M1 smoke requires both vertical-centering mode and the shared optical offset on character-card and back-button labels.
- **Visual evidence**: `tmp/collection_text_alignment_20260721/after_1080x1920.png` and `after_1080x2340_safe.png` were manually reviewed; action and back labels now sit on the bezel's visual center without clipping at either aspect ratio.

## Premium Autocannon UI Icon Replacement (2026-07-21)

- **Owner evidence**: the weapon collection showed the starter autocannon as a tiny flat rifle inside a sparse, unrelated frame while the other seven weapons used dense, premium 3D arsenal icons.
- **ImageGen replacement**: built-in `image_gen` used the accepted scattergun, railgun and flamethrower icons as style references to create one heavy automatic machine gun with a reinforced receiver, rapid-fire barrel group, cooling shroud, ammunition box, battle-worn steel and restrained orange emissive detail. The final icon preserves the shared circular ring and armored corner frame.
- **Global UI integration**: `weapon_autocannon_icon.png` was replaced at the production, legacy compatibility and M1 sample paths with identical `512×512 RGBA` output (`SHA-256 e9f197e94f411c09673c65a5f4f16b848561ccd0e6daec8e5f66f48283f38183`). Existing JSON/TSCN/map references therefore update without ID or path churn.
- **Gameplay boundary**: the battle character-weapon animation set, handheld rifle, turret fallback, projectile, VFX, SFX, balance and combat logic are unchanged, matching the Owner's request to limit the replacement to global UI presentation.
- **Provenance**: the `1254×1254` source render and exact structured prompt live at `assets/production/source_refs/generated/weapon_autocannon_icon_v2_*_2026_07_21.*`; `OUTSOURCER_ASSET_INDEX.json` records the owner-directed override.
- **Visual evidence**: `tmp/autocannon_icon_alignment_20260721/weapons_1080x1920.png`, `weapons_1080x2340_safe.png`, `loadout_autocannon_1080x1920.png`, and `map_autocannon_1080x1920.png` confirm readable small and large presentation. The shared collection button-label correction also centers weapon-page actions and the back control.
- **Verification**: `python3 tools/check_release_candidate.py` passed the complete asset/data/reference, balance, Godot boot, M1 smoke and 46-route visual gate after the new icon was force-reimported.

## Locked-Target Multishot Priority Fix (2026-07-21)

- **Owner evidence**: a physical-device capture showed the cyan lock ring on the frontline zombie while a two-lane volley continued toward the rear-center group.
- **Root cause**: single-shot auto aim and `TargetingManager` correctly returned the locked enemy, but `_primary_shot_directions()` recomputed every multi-lane volley around the battlefield centroid. That second-stage fan calculation silently discarded the player lock; even lane counts could also straddle the intended direction with no projectile on the center line.
- **Behavioral fix**: active manual aim or a valid enemy lock now becomes the multi-shot center priority, skips centroid recentering, and snaps the nearest fan lane exactly onto the explicit direction. Remaining lanes retain the authored fixed-angle spread, so point targeting is honored without turning every projectile into an independent homing shot.
- **Unchanged systems**: no-lock auto targeting still prefers frontline pressure and then uses the enemy centroid for multi-shot coverage. Damage falloff, lane count, fire rate, homing, pierce and enemy threat scoring are unchanged.
- **Regression coverage**: M1 smoke now requires a two-lane volley to keep one exact lane on a locked enemy and one exact lane on an active dragged manual-aim point; the original equal-angle fan assertions remain for the no-lock path.
- **Verification**: `python3 tools/check_release_candidate.py` passed the complete asset/data/reference, balance, battle boot, M1 smoke and 46-route visual gate after the targeting change.

## Global Mobile Typography +2 And Layout Closure (2026-07-22)

- **Unified scale contract**: the project default font moves from `32` to `34`; every explicit TSCN font size moves by exactly `+2`; UiKit exposes `FONT_SIZE_STEP=2` for scaled controls and `bumped_font_size()` for direct runtime labels. Combat float text, enemy labels, combo text, collection details, loadout labels and result actions now use the same contract instead of independent literals.
- **Readability target**: the approved Glow Sans / 未来荧黑 binary and `FONT_SCALE=1.4` remain unchanged. The pass increases phone legibility without changing copy hierarchy, button hit targets, gameplay geometry, data IDs or combat behavior.
- **First visual iteration**: runtime audits found chapter boss-state chips and two skill-effect summaries exceeding their bounds after enlargement. Their authored text regions were widened while the surrounding cards remained fixed.
- **Second visual iteration**: manual contact-sheet review found issues that autowrap-aware clipping checks intentionally skip. Collection descriptions now reserve full two-line height; owned entries expose their upgrade/special line, while locked entries keep price only in the action button. Modal backdrops use a generated `GradientTexture2D`, and the underlying list/back action is hidden while a detail is active, eliminating text bleed behind translucent card skins while preserving page-title context and the texture-backed UI rule. Character headers reserve clearance for the ornamental rule.
- **Coverage expansion**: `tools/check_visual_screens.py` grows from 46 to 54 routed states, adding armor/chip/pet lists, weapon/armor/chip/pet detail modals and the in-battle card-detail overlay. The matrix covers `1080×1920`, `1080×2340`, debug safe insets, ten campaign environments, four character details, menu/map/loadout/settings/result, pause and card offers.
- **Visual evidence**: all 54 final captures are retained under `tmp/font_readability_2026_07_22/screens/`; six review sheets are retained as `tmp/font_readability_2026_07_22/font_layout_contact_01.png` through `font_layout_contact_06.png`. Automated UI audits report zero issues and the final sheets were manually reviewed for clipping, overlap, alignment, modal bleed-through and safe-area completeness.
- **Regression guardrails**: M1 smoke locks the default `34px` project font, the `+2px` UiKit step and representative scaled/direct calculations. Visual routes permanently retain the expanded list/detail coverage.
- **Verification**: `python3 tools/check_release_candidate.py` passed the complete font-license, asset/data/reference, pressure/balance/economy/card, runtime-UI, Godot boot, battle boot, M1 smoke and 54-route visual gate.

## TestFlight Build 34 (2026-07-22)

- **Release gate**: `tools/ship_testflight.sh` passed the complete source Release Candidate, including font licensing, asset/data/reference checks, campaign balance and economy simulations, runtime UI rules, Godot boot, M1 smoke, and all 54 routed screenshots.
- **Exported-runtime proof**: the Build 34 PCK contains 5,770 files and 2,772 imported resources at 568.5 MiB. The generated Xcode target is iPhone-only, and the exported PCK independently passed battle boot plus the full M1 smoke suite.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded for `1.0.0 (34)`. The audited IPA is `625,832,900` bytes (596.8 MiB), SHA-256 `bbaca11450e32cdf81d4a33b77743d7b6382f2dbed5d5d7857bd492c2ffde6a6`.
- **Apple delivery**: upload returned `UPLOAD SUCCEEDED with no errors`; Delivery UUID `9c59677f-f5df-4e86-97e9-96b095ba17df`. Final polling reached `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, and `IS-ON-APP-STORE-CONNECT: true`; `usesNonExemptEncryption=false`.
- **Local artifact**: the validated IPA remains at `build/ios/ZombieFire.ipa`. macOS denied replacing the older Desktop copy, which did not affect the App Store Connect upload or processing result.

## App Store Preflight Deep Polish And Visual Closure (2026-07-23)

- **Adaptive combat information density**: `battle.gd` now computes a stable priority set instead of leaving every enemy label active during peak waves. Eight or fewer enemies retain full labels; medium/high density keeps the explicit lock, every boss, the highest-pressure elite, the current auto target, then fills a capped list by targeting score/frontline position. `enemy.gd` applies the policy independently to threat and status labels without altering target selection, hitboxes, AI, damage, or manual aim.
- **Regression proof for crowded combat**: M1 smoke builds an 18-target synthetic battle and asserts the high-density cap plus locked/boss/elite retention. `tools/_shot.gd` adds a deterministic 16-enemy visual route; its final audit reports `priorities=7` and `visible_threat_markers=7`.
- **Large-type edge fixes**: the map navigation status badge gains sufficient inner width for `Lv40/Lv20`, and collection item titles use the free upper-card width so long names such as `火焰喷射器  等级41 III` remain complete. The first 55-route pass caught both issues; targeted recaptures and the full second pass report zero runtime UI audit findings.
- **Store screenshot finish**: marketing badges are compact filled capsules instead of long placeholder-like outlines. Current battle/map/card/loadout/Boss routes were recaptured for both 6.5- and 6.7-inch deliverables. The Boss capture uses an isolated showcase path so incidental mobs cannot overlap the health bar; campaign spawning and boss behavior are unchanged.
- **App Preview geometry and pacing**: the live Godot preview is now a 22-second curated arc covering manual/frontline lock, card selection, Boss pressure and an active skill. Capture runs in a temporary `1080×2340` project viewport and scales proportionally to `886×1920` with square pixels (`SAR 1:1`), replacing the previous non-proportional `1080×1920 → 886×1920` pipeline. Store checks now reject non-square pixels or stale capture provenance.
- **Package decision**: a content-hash audit found about `37.4 MiB` of theoretical exact-duplicate savings across 110 groups, dominated by repeated animation/VFX semantics. Because runtime aliasing or lossy texture recompression would broaden launch risk for a modest gain, the accepted action is to preserve the already validated lossless export exclusions. A clean iOS PCK export succeeds at about `569 MiB`.
- **Visual evidence**: manually reviewed outputs are retained under `tmp/deep_polish_2026_07_23/`, including the dense battle, Preview contact/poster, collection-title fixes, map badge fix and both App Store screenshot contact sheets. The permanent automated matrix now passes `55` routed screenshots covering standard/tall viewports, debug safe insets, all ten environments, four characters, equipment lists/details, battle card detail, dense combat, pause, settings and result.
- **Deterministic smoke correction**: the full gate exposed an old Blaze assertion that compared signature growth against a character-level/rank-boosted radius, making it fail only when the local Blaze save was already near Lv40. The test now checks the authored `+5% of base radius per signature level` contribution directly; gameplay data and runtime math were unchanged.
- **Final verification**: `python3 tools/check_release_candidate.py` passes the complete 31-check source gate, including the seven AGENTS-required asset/data/reference/pressure/card/Godot/smoke commands and the 55-route visual suite. The clean exported PCK independently passes `tools/check_release_package.py` with `5,770` files, `2,772` imported resources and `568.5 MiB`; SHA-256 is `bed848c9d815b926e31a6a24d7fbc6d71e5df33a580d52a1b2789f5ed4bc8bb5`.

## Launch Conversion, Combat Hot Path And Candidate Closure (2026-07-23)

- **Combat hot path**: `_physics_process()` now snapshots `EnemyLayer` once per frame and reuses that exact ordered array for battle-report control, auto targeting, combat-information density, enemy mechanics and slow-field work. Aura range checks use squared distance. Target selection order, manual aim, damage, mechanics and authored data are unchanged.
- **Regression guard**: `tools/check_gameplay_polish.py` requires the shared enemy snapshot, squared aura distance and compact loadout summary contracts so a later refactor cannot silently restore the repeated scans or high-level wrapping.
- **Store opening**: the first 6.5/6.7-inch screenshot now uses a deterministic live-runtime eight-enemy composition with real bullets, status mechanics and a centered manual lock. The first Preview lock is triggered at `0.7s`, making the auto-fire plus player-priority loop readable in the opening second.
- **Loadout clarity**: the full showcase loadout no longer wraps the pet level onto a third line. The two-line summary keeps the approved font size, uses compact `Lv` notation and renders projected power as `成型 738 (+141)` so the card-budget contribution is traceable without adding an unvalidated analytics panel before review.
- **Preview quality failure and repair**: the first rebuild exposed a real launch-gate failure: target `10 Mbps` ABR encoded quiet footage at only about `2.2 Mbps`. The generator now uses x264 HRD CBR with filler and fails immediately below `8 Mbps`; the final H.264/AAC, square-pixel `886×1920`, 24 fps, 22-second asset is about `10.1 Mbps / 27 MiB`.
- **Hermetic smoke tests**: exact Lv1/Lv2 skill assertions now clear permanent skill levels inside a temporary save and restore the original save afterward. This preserves the intended runtime rule—new cards inherit permanent progression—while making release validation independent of the developer's local high-level save.
- **Visual verification**: regenerated raw loadout, both ordered iPhone screenshot sets and three opening Preview frames were manually reviewed. The complete matrix then passed all `55` routed screenshots across standard/tall viewports, safe insets, dense combat, all four characters, lists/details, battle overlays, settings and result states.
- **Final verification**: `python3 tools/check_release_candidate.py` passed end to end after the repairs. `/tmp/zombie_fire_build35_candidate.pck` passed package audit with `5,770` files, `2,772` imported resources and `568.5 MiB`; exported-pack battle boot and full M1 smoke both passed. SHA-256 is `efbf2431ea89a549b05745ee0ba5d6fdd464353413f466f96098a3c29f5577e4`.

## Save Integrity And iOS Lifecycle Release Gate (2026-07-23)

- **Persistence matrix**: `tools/save_integrity_test.gd` now verifies atomic writes and backup rotation, corrupt/schema-invalid main recovery, missing-main fallback, legacy migration, recursive default merging, manual backup/restore, and simulated temporary-file failure without damaging the last good main or backup.
- **Lifecycle behavior**: the regression instantiates the real `main.tscn` lifecycle controller, starts an active aim gesture, mutates live save data, and drives application pause/resume notifications. It requires pause to persist the latest data, cancel the touch/aim state, and suspend managed audio; resume must restore the audio state.
- **Honest failure logging**: corrupt and failed-write fixtures are expected negative tests. `SaveManager` exposes an internal test-only suppression flag that prints those expected fixture failures without producing a false Godot error; normal runtime failures still use `push_error`.
- **Release integration**: source Release Candidate validation now has 32 checks and requires the explicit `SAVE INTEGRITY TEST PASSED` marker. `tools/ship_testflight.sh` reruns the same suite from the exported PCK between battle boot and M1 smoke, so a source-only pass cannot mask packaging/autoload regressions.
- **Verification**: the complete 32-check source candidate and all 55 routed screenshots passed with clean Godot logs. No physical iPhone was connected; device-only thermal, background eviction, touch feel, speaker/headphone and blind-test evidence remains explicitly assigned to the Owner checklist.

## TestFlight Build 35 (2026-07-23)

- **Source gate**: the shipping script reran the complete 32-check Release Candidate after the final lifecycle additions. All asset/data/reference, campaign pressure, endgame balance, economy, runtime UI, audio/VFX, Godot boot, battle boot, save integrity, M1 smoke and 55-route visual checks passed.
- **Exported-runtime proof**: the iPhone-only Build 35 PCK contains 5,770 files and 2,772 imported resources at 596,164,880 bytes (568.5 MiB). Exported-PCK battle boot, save/lifecycle integrity and M1 smoke all passed with clean Godot logs. SHA-256 is `ec36df1ed439875f5d9d8e5611fd5908b7576cdad1185fba6b8114315cee45f6`.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded for `1.0.0 (35)`. The audited IPA is 625,836,434 bytes (596.8 MiB), SHA-256 `3155029c75a308f44b79ad6501dd98194b02f54632b5c9e059173eb297189f48`.
- **Apple delivery**: upload completed without errors with Delivery UUID `a21524ab-a777-453c-af1d-a12d1a86bd68`. Final status is `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, and `IS-ON-APP-STORE-CONNECT: true`; `usesNonExemptEncryption=false`.
- **Local artifact**: the validated IPA remains at `build/ios/ZombieFire.ipa`. An existing Desktop copy was not overwritten; this did not affect upload or App Store Connect processing.

## TestFlight Build 36 (2026-07-23)

- **Source identity**: Build 36 is a build-number-only rerun from the committed Build 35 code baseline; no gameplay, data, visual or asset change was introduced.
- **Repeated release proof**: all 32 source Release Candidate checks and 55 routed screenshots passed again. The iPhone-only exported PCK contains 5,770 files and 2,772 imported resources at 596,164,880 bytes (568.5 MiB), and independently passed battle boot, save/lifecycle integrity and M1 smoke. PCK SHA-256 is `ec36df1ed439875f5d9d8e5611fd5908b7576cdad1185fba6b8114315cee45f6`.
- **Signed artifact**: Xcode Archive, App Store Distribution export and IPA audit succeeded for `1.0.0 (36)`. The IPA is 625,836,432 bytes (596.8 MiB), SHA-256 `7093bc951b72bdad26cbb80e4b950b42c6aa7c3b7435a974bb6cf73a6e246343`.
- **Apple delivery**: upload completed without errors with Delivery UUID `a90cad41-f8b8-4287-8d27-48b679e8d352`. Final status is `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, and `IS-ON-APP-STORE-CONNECT: true`; `usesNonExemptEncryption=false`.
- **Network note**: Apple ContentDelivery transferred 625,836,432 bytes in 1.11 hours at approximately 1.252 Mbps. The validated IPA remains at `build/ios/ZombieFire.ipa`; the existing Desktop copy was not overwritten.

## TestFlight Release Pipeline Hardening (2026-07-25)

- **Final Apple-state gate**: `tools/ship_testflight.sh` now extracts the labeled Delivery UUID from the upload log and calls `altool --build-status --wait` with that UUID alone, matching the current Apple CLI contract and avoiding its redundant-identity warning. A release only reports success after `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, App Store eligibility and App Store Connect presence are proven; the record also cross-checks Apple's returned Delivery UUID, Build number and non-exempt-encryption state.
- **Consumed-build safety**: the script keeps the incremented build number and audited IPA as soon as Apple accepts the upload. A later import/status failure can no longer restore a build number Apple has already consumed or delete the only matching local artifact.
- **Auditable record**: `tools/release_delivery_record.py` validates Apple output and atomically writes `build/ios/release/build_<N>/release_manifest.json`, including full source commit, tracked-change flag, app identity, Delivery UUID, final status, byte counts and SHA-256 for both IPA and PCK. Upload and final-status logs are retained beside it; credentials are excluded.
- **Truthful Desktop handoff**: the optional Desktop copy uses a unique temporary file, forced atomic replacement and `cmp` verification. If macOS permissions or destination flags block replacement, the script warns instead of claiming an old IPA was updated.
- **Regression coverage**: the release candidate now runs `bash -n tools/ship_testflight.sh` and the delivery-record parser/manifest self-test, increasing the source gate from 32 to 34 checks. No gameplay, data, visual, audio or exported runtime content changed in this pass, and no new TestFlight build was uploaded.
- **Final proof**: the modified 34-check Release Candidate passed end to end, including all seven AGENTS-required commands, save integrity and all 55 routed screenshots. A read-only query against the existing Build 36 Delivery UUID confirmed the corrected Apple CLI call emits no argument warning and returns `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `BUILD-AUDIENCE-TYPE: APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, `VERSION: 36` and `USES-NON-EXEMPT-ENCRYPTION: false`.

## Boss Silhouette Redesign And Encounter Presentation Closure (2026-07-25)

- **Eight-boss audit**: runtime art confirmed that five bosses still shared the same upright armored-humanoid silhouette. Storm Caller, Plague Mother and Void Phantom already read distinctly and were preserved; Armored Colossus, Inferno Maw, Frost Warden, Necrotitan and Apex Overlord were redesigned.
- **Distinct visual grammar**: the replacements are respectively a gorilla-like siege ram, low quadrupedal furnace beast, glacial prison walker, asymmetric walking ossuary and tripod six-armed command organism. The pass preserves every existing boss ID, data/resource path, mechanic, stat, weakness, collision radius, spawn rule and level assignment.
- **Complete asset families**: five prototype, portrait and icon sets, 145 runtime animation frames and five intro videos were regenerated. `tools/regenerate_boss_models_2026_07_25.py` records the deterministic derivation; prompt/source/manifest evidence and before-after sheets live under `assets/production/source_refs/generated/boss_model_redo_2026_07_25/`, with the production override registered in `OUTSOURCER_ASSET_INDEX.json`.
- **Phone readability**: the shared runtime boss visual scale increases from `0.44` to `0.50` while collision, targeting and battle geometry remain unchanged. The isolated screenshot route applies its own additional framing scale only for marketing captures.
- **HUD hierarchy repair**: long onboarding/wave banners move below the global boss rail whenever a boss is active, the rail label uses a higher-contrast warm tone, and ambiguous one-character status labels now use full Chinese state names.
- **Name ownership**: the local marker now shows only the monster's translated proper name, while the global rail remains the sole `name · weakness · HP%` summary. Armor break updates the separate status label instead of replacing the monster name.
- **Store synchronization**: both iPhone screenshot sets and the 22-second live Godot App Preview were rebuilt from the current runtime. The Boss card now leads with `异形 Boss 压境，锁定弱点反击`.
- **Verification**: the asset pack validates at 7,847 files; data, `res://` references, level pressure, card simulation, Godot boot and M1 smoke all pass. The permanent visual matrix first expands from 55 to 60 routes for the five redesigned bosses, then to 63 routes for all eight bosses; every run passes. The complete Release Candidate gate passes end to end.

## Boss Base-Attack Identity And Animation Redesign (2026-07-25)

- **Eight authored attack identities**: every boss now owns a data-driven `base_attack_profile`. Tank Titan performs a single siege slam; Inferno Maw fires a three-orb molten barrage; Frost Warden drops a two-stage ice prison; Storm Caller channels five lightning pulses; Plague Mother lobs four brood projectiles; Void Phantom performs a three-cut phase combo; Necrotitan delivers one necrotic crush; Apex Overlord cycles four elemental impacts.
- **Complete presentation arc**: boss attacks now expose explicit wind-up and per-hit visual events. Battle presentation composes telegraphs, cast glows, projectile arcs, trails, channel beams, dash afterimages, element-specific impacts, camera response and a new metal base-rupture texture according to the profile instead of emitting one generic breach effect.
- **Balance-preserving multi-hit contract**: visual hit counts do not multiply authored damage. One completed attack sequence still emits exactly one `breached` event carrying the original `base_attack_damage`, so the base-damage cap, barrier consumption, attack interval and campaign tuning remain unchanged.
- **Data and asset provenance**: `data/bosses.json` owns mode, label, element sequence, timing, projectile style, attack-line offset, scale, color and VFX references. `tools/validate_data.py` enforces those contracts. The generated rupture source, production alpha asset and prompt/provenance record live under `assets/production/source_refs/generated/boss_base_attack_redesign_2026_07_25/` and are registered in `OUTSOURCER_ASSET_INDEX.json`.
- **Deterministic regression coverage**: M1 smoke instantiates all eight bosses and requires one telegraph, every authored visual hit, one damage event and exact damage preservation. `tools/_shot.gd` disables unrelated combat activity and stages each attack on its real gameplay line.
- **Visual closure**: the permanent screenshot matrix expands from 63 to 71 routes with one tall-phone base-attack peak route per boss. Manual peak-frame review verifies distinct silhouette/element language, boss-to-hero clearance, effect layering and readable base contact across all eight attacks.
- **Final verification**: the asset pack validates at 7,852 files; data, `res://` references, all 99 level-pressure rows, card simulation, Godot boot, battle boot, save integrity and M1 smoke all pass. The complete 71-route visual matrix and full Release Candidate gate pass end to end.

## Storm Caller Siege-Lightning Rendering Redesign (2026-07-25)

- **Rejected provisional rendering removed**: Storm Caller's profiled base attack no longer uses the solid yellow four-point `Line2D` or the old storm-chain sequence whose full-frame energy bed exposed a rectangular texture boundary.
- **New layered identity**: a generated transparent white-core/cyan-glow/violet-corona column is fitted to the live caster-to-base vector for every pulse. Horizontal mirroring, small angular jitter, width breathing and five authored color temperatures keep the channel continuous without repeating one rigid silhouette.
- **Contact hierarchy**: a separate compact electrical crown aligns its painted contact core to `BREACH_Y`, expands beneath the defense actor, and hands off to the existing telegraph, hit audio, damage flash and final camera response. It does not cover the hero or bottom HUD.
- **Data contract and isolation**: the `channel` profile now owns `beam_texture` and `impact_texture` resource paths, enforced by data validation. The change is exclusive to Storm Caller's boss siege profile; player lightning skills, damage, interval, barrier consumption and the five-visual-hit/one-damage contract remain unchanged.
- **Generation provenance**: chroma sources, final prompt record and transparent production outputs live under `assets/production/source_refs/generated/boss_storm_lightning_redesign_2026_07_25/` and are registered in `OUTSOURCER_ASSET_INDEX.json`.
- **Final verification**: early, middle and finishing contact frames were manually reviewed at `1080×2340`. The asset pack validates at 7,861 files; data, references, level pressure, card simulation, Godot/battle boot, save integrity and M1 smoke all pass. The complete 71-route visual matrix and full Release Candidate gate pass end to end.

## Premium Combat VFX Re-Render And Cropping Closure (2026-07-26)

- **Unified quality target**: the approved Storm Caller lightning column became the rendering baseline for the complete combat feedback family. New authored sources cover all eight boss base attacks, eight boss mechanics, four character actives, eight weapon projectiles, five muzzle families, nine hit states and five elemental death finishes. Each family keeps a distinct silhouette, color identity and visual weight while preserving gameplay IDs, damage, cadence, targeting and balance.
- **Production integration**: `tools/integrate_premium_combat_vfx_2026_07_26.py` derives transparent runtime sequences from six padded master renders without replacing content IDs or data paths. Boss attack profiles now reference their authored premium contact sequences, and battle presentation selects premium boss-mechanic, hit and death media through the existing data-driven flow.
- **Cropping audit and repair**: the review expanded beyond the new renders to all 79 referenced combat VFX sequences. `tools/normalize_combat_vfx_safe_margins_2026_07_26.py` refitted 32 legacy sequences that could touch the canvas edge, including card casts, ordinary-zombie mechanics and one character active. A whole sequence shares one centered scale, so the repair cannot introduce frame-to-frame zoom jitter; frame counts, timing and manifests remain unchanged.
- **Permanent safety gate**: `tools/check_combat_vfx_safe_margins.py` validates only frames used by runtime manifests, accepts intentional blank bookends and requires at least 7.5% transparent clearance on every side. It passes all 701 non-empty frames and is now part of `tools/check_release_candidate.py`.
- **Deterministic visual routes**: `tools/_shot.gd` can isolate character actives, boss mechanics, hit states and death finishes, and can stage every card and normal-zombie skill. The manually reviewed specialty matrix contains 83 runtime screenshots: 4 actives, 8 boss skills, 8 boss attacks, 8 weapon paths, 9 hit states, 5 death states, 16 card casts, 19 zombie mechanics and 6 representative tall-phone captures. No effect is cropped, no chroma rectangle leaks through, and HUD/safe-area hierarchy remains readable.
- **Provenance and package hygiene**: prompts, six generated masters, integration manifest and the legacy safe-margin report are retained under `assets/production/source_refs/generated/premium_combat_vfx_2026_07_26/`; the override is registered in `OUTSOURCER_ASSET_INDEX.json`. Synchronized iOS exclusions contain 237 rules and keep 65.9 MiB of authoring sources plus 216 unused tail-cache frames out of the release package.
- **Final verification**: `python3 tools/check_release_candidate.py` passes end to end after the export-exclusion synchronization. The asset pack contains 8,404 files; data, 349 `res://` references, all 99 level-pressure rows, balance/economy/card simulation, font licensing, attack motion, audio, active media, tall layouts, HUD overlap, App Store assets, Godot/battle boot, save integrity and M1 smoke all pass. The permanent 71-route full-interface screenshot suite also passes with no runtime layout findings.

## TestFlight Build 37 (2026-07-26)

- **Published source identity**: the complete boss and premium-combat-VFX candidate is committed as `74231425132d887a9fcb18640aa6b0ce5ccca135` and pushed to `origin/main`. The release record confirms no tracked working-tree changes were present when the build started.
- **Repeated source gate**: the shipping script reran the full Release Candidate after the push. Asset/data/reference, all 99 level-pressure rows, balance/economy/card simulation, font and VFX licensing/safety, audio, tall layouts, HUD overlap, Godot/battle boot, save integrity, M1 smoke and all 71 routed screenshots passed.
- **Exported-runtime proof**: the iPhone-only Build 37 PCK contains 6,472 files and 3,116 imported resources at 581,256,680 bytes (554.3 MiB). Exported-PCK battle boot, save/lifecycle integrity and M1 smoke passed with validated Godot logs. SHA-256 is `4e390d2400b3b6b99e879ade9825955209fcb5ce9fe5bd45bc172cc3a50bdb6c`.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded for `1.0.0 (37)`. The audited IPA is 610,745,333 bytes (582.5 MiB), SHA-256 `8359580d26c888996d0f689d8c87621b51d4b0771c3ffb18fc9c068684cd20dd`.
- **Apple delivery**: the upload completed after automatic recovery from two transient network-part retries. Delivery UUID is `d1839f5a-882c-494e-8485-7419249d27ec`; Apple reports `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, version `37` and `USES-NON-EXEMPT-ENCRYPTION: false`.
- **Local record**: the validated IPA and atomic delivery manifest remain under `build/ios/`. macOS denied replacing the optional Desktop copy, which did not affect the uploaded artifact, Apple validation or TestFlight availability.

## Combat VFX Semantic Acceptance And Runtime Re-Audit (2026-07-26)

- **Why the prior gate was insufficient**: transparent margins and complete frame manifests proved that a bitmap could render safely, but did not prove that its painted direction agreed with gameplay. A right-facing dash wedge could therefore pass while the zombie actually moved down toward the base. The new contract treats VFX as gameplay communication rather than decoration.
- **Permanent semantic contract**: `design/assets/combat_vfx_semantic_contracts.json` records the source-forward angle for every directional enemy sequence and projectile, required runtime vector, group coverage, intentional hard-edge exceptions, semantic silhouette groups and standard/tall viewport rules. `tools/check_combat_vfx_semantics.py` validates all 83 sequence directories, 710 non-empty referenced frames, 11 projectile textures, eight boss base-attack profiles, four active skills and all 16 card casts.
- **Runtime direction repair**: Battle now rotates dash, charge, leap, phase-shift and ranged-spit artwork from its authored source-forward vector onto the actual movement/target vector. Directional sequences no longer receive random spin. M1 smoke asserts the exact screen-down rotations and confirms non-directional effects preserve their fallback rotation.
- **Rejected source crops repaired**: six legacy sequences (`corrosion`, `ranged_spit`, `toxic_cloud`, `regen`, `skill_cast_venom`, `death_dissolve`) exposed a straight atlas-cell boundary even after outer padding. They were replaced with contained derivatives of accepted premium production sequences while preserving target IDs, paths, frame counts, FPS and gameplay references. Acid spit and split-mini projectiles were upgraded from flat geometric placeholders to accepted premium projectile silhouettes.
- **Mechanic identity closure**: runtime review found that an initial safe replacement made corrosion, ranged spit and toxic cloud visually interchangeable. A new distinctness gate now requires unique mid-frame silhouettes for poison impact, directional launch, persistent area and regeneration. The final runtime uses a radial corrosion burst, downward venom wedge, grounded poison pool and vertical healing column respectively.
- **Import-cache failure caught by screenshots**: the first post-repair Godot captures still showed the old rectangular textures because the editor import cache had not refreshed. The assets were force-reimported and every affected route was recaptured; this confirms why source-PNG checks cannot replace the live-render layer.
- **Visual evidence**: the focused runtime sheet at `tmp/combat_vfx_semantic_audit_2026_07_26/runtime_contact_sheet.png` contains 11 standard-phone captures and one `1080×2348` tall-phone capture. The accepted before/after asset sheet is `assets/production/contact_sheets/contact_combat_vfx_semantic_repair_2026_07_26.png`. Together with the unchanged 83-route specialty matrix, every combat family retains current runtime evidence.
- **Release integration and verification**: the semantic checker is part of `tools/check_release_candidate.py`; the gate now contains 36 checks. Asset pack (8,407 files), data, 349 `res://` references, all 99 pressure rows, card simulation, 96 attack-animation sequences, VFX safe margins, visual assets, Godot/battle boot, save integrity, M1 smoke and all 71 routed screenshots pass end to end. Full findings and acceptance rules are recorded in `design/assets/combat_vfx_semantic_audit_2026_07_26.md`.

## Persistent Enemy Status VFX Top-Tier Polish And Runtime Closure (2026-07-26)

- **Independent state language**: fire, ice, glacier, poison and lightning now own separate data-defined presentation profiles and runtime layers. Fire clings to the upper body and shoulder, ice anchors at the feet with a restrained body chill, poison occupies the waist/lower silhouette, and electricity climbs the upper/right outline. Bosses use authored offsets and scale instead of inheriting a blown-up normal-enemy aura.
- **Complete lifecycle**: each layer is created lazily, fades in, loops without restarting when its timer refreshes, receives a short pulse on application or periodic damage, and fades out before being released. Fire/poison/ice/lightning can remain visible simultaneously; the previous single mutually exclusive `StatusAura` path has been removed.
- **Controlled stacking**: multi-status enemies phase-shift their loops and reduce per-layer opacity at two and three-plus active states. This preserves the monster face, name and readable silhouette instead of turning stacked effects into a white/green bloom.
- **Combat-density LOD**: `data/status_vfx.json` defines full, condensed and minimal tiers at 24 and 48 active enemies. Bosses, manually locked targets and the current priority target retain the full two-layer treatment; ordinary enemies progressively lose the secondary body loop while keeping compact ground/state evidence. Reduced-effects mode uses the same restrained route.
- **Permanent regression gate**: `tools/check_status_vfx.py` validates five distinct sequence families, all 62 referenced frames, colors, scales, alpha limits, integration hooks, absence of the legacy aura and dense-battle LOD. The checker is part of the Release Candidate. M1 smoke additionally exercises simultaneous states, rendered alpha, minimal/full transitions, expiration fade and cache cleanup.
- **Runtime evidence**: `tmp/status_vfx_runtime_audit_2026_07_26/contact_sheet.png` contains four clean live-Godot panels: individual fire/ice/poison/lightning, four-way stacking on a normal enemy plus Boss, a 36-enemy density load, and the maximum desktop-captured high screen (`1080×2046`). The static tall-layout gate continues to cover `1080×2337 / 2340 / 2348 / 2622`. Manual review found no cropping, texture rectangles, overexposure, HUD collision or stale layer.
- **Gameplay safety**: no authored status duration, damage, slow, stun, targeting, boss mechanic, enemy stat or level-pressure value changed. The pass is purely presentation, lifecycle hygiene and visibility scaling.
- **Final verification**: the Release Candidate now contains 37 checks and passes end to end. Asset pack (8,407 files), data, 353 `res://` references, all 99 pressure rows, card simulation, Godot/battle boot, save integrity, M1 smoke and all 71 routed screenshots pass with the status checker enabled.

## Ordinary Zombie Model Redesign And Roster Silhouette Closure (2026-07-26)

- **Roster-wide audit**: all 20 ordinary zombies were compared as alpha silhouettes at runtime scale and in live mixed formations. Twelve already-distinct families were preserved. Bomber, Spitter, Juggernaut, Necromancer, Charger, Regenerator, Splitter and Warden were selected for full owner-approved redesign because their previous bodies overlapped another family or failed to communicate their mechanic.
- **Mechanic-first rendering**: the eight replacements now read through primary massing before color—barrel bomb core, low acid predator, industrial siege chassis, tall corpse-cage conductor, wedge-headed ram, asymmetric healing anatomy, three-lobed fusion body and crossbar shield projector. They retain the locked semi-realistic rendered-sprite lighting and material language.
- **Path- and balance-safe integration**: every existing ID, JSON row, sprite path, mechanic, stat, weakness, collision and campaign reference remains intact. The pass replaces the three collection assets and all runtime actions in place. The original model-redesign set contained 232 imported `512×512` RGBA frames; the later roster-wide attack pass expands the eight redesigned families to 264 frames by upgrading each attack from four to eight frames.
- **Provenance and reproducibility**: built-in image generation produced padded magenta-key masters; local chroma extraction created transparent sources. Prompts, source renders, masters, pre-change prototypes, integration manifest and three contact sheets are retained under `assets/production/source_refs/generated/zombie_model_redo_2026_07_26/` and registered in `OUTSOURCER_ASSET_INDEX.json`.
- **Permanent acceptance contract**: `tools/check_zombie_model_silhouettes.py` guards required outputs, alpha corners, exact frame families, at least six transparent pixels on every animation edge, non-static frame variation, authored proportion bands and a maximum pairwise roster silhouette IoU. M1 smoke additionally instantiates all eight models and checks every imported action texture and unchanged runtime scale.
- **Runtime closure**: deterministic screenshots cover the redesigned eight, all 20 families, 24-enemy combat density and the tallest desktop-capturable phone viewport (`1080×2046`). The first audit exposed only staging-edge placement; the lineup was moved into safe bounds and recaptured cleanly. Four routes are now part of the permanent full-interface matrix.
- **Final verification**: the complete 38-check Release Candidate passes. The asset pack contains 8,559 files; data, 353 `res://` references, all 99 pressure rows, card simulation, Godot/battle boot, save integrity and M1 smoke pass. The full-interface runtime matrix expands from 71 to 75 routed screenshots and passes with all four zombie-model scenes enabled.

## Endgame Character DPS Convergence And Uncapped Volt Chains (2026-07-26)

- **Late-game active-skill scaling**: character-basis signatures can now opt into a bounded fraction of permanent weapon-level damage growth without inheriting the weapon coefficient, cadence or turret modifier. Blaze uses `0.65`; Frost and Volt use `0.90`. Frost and Volt also receive stronger level/rank/signature growth so their authored actives remain central at max progression.
- **Volt mechanism growth**: max Storm now resolves 16 candidate targets and 26 strikes. Projectile chains no longer truncate at five; the resolver includes run skills, weapon specials, character affinity and the previously orphaned pet chain stat. A `0.92` subsequent-target falloff bounds dense-wave multiplication without discarding authored chain count.
- **Single-target chain value**: each Volt chain beyond the authored five-chain reference adds 2% primary-target damage. A max Tesla/skill/affinity setup therefore resolves 13 chains and a 1.16x primary multiplier even when a Boss has no nearby adds; a max chain pet raises the logical count to 16.
- **Burn correctness**: repeated fire applications now blend toward the current source strength instead of retaining the historical maximum forever. Weaker refreshes decay an old crit peak and expiration resets both duration and cached DPS. Blaze's permanent attack/affinity curve is reduced alongside the correctness fix so its large-area burst does not dominate neutral single-target output.
- **Reproducible audit**: `tools/audit_character_endgame_dps.py` mirrors live max-level formulas, enumerates every chip/pet pairing and evaluates the same neutral 130 px Boss geometry with three of five multishot lanes connected. The accepted totals are Vanguard 119,776, Blaze 134,479, Frost 128,002 and Volt 137,219 DPS, a 1.146x / 14.6% max-min spread. Five-lane overlap ceilings are 182,154 / 215,845 / 197,906 / 200,506 respectively.
- **Regression contract**: data validation bounds weapon inheritance, overflow conversion and chain falloff. M1 smoke requires full 13-chain resolution (16 with the chain pet), 16-target/26-strike Storm growth, meaningful max-weapon active scaling, burn-peak decay and complete burn-cache cleanup.
- **Final verification**: asset validation passes at 9,222 files; data, all 353 `res://` references, 99-level pressure, card simulation, Godot boot, M1 smoke and the standalone endgame DPS audit all pass. No UI, asset or layout surface changed in this balance-only pass.

## Late-game confrontation and Boss survival-window rebalance (2026-07-26)

- **Measured failure mode**: the old level-99 Apex had about `3.34M` HP against nearly `200K/s` of standard three-lane maxed physical counter damage, leaving only `16–20s` before death. Reaching its `y=560` core-mechanic zone already takes roughly `19s`, so the Boss could genuinely die before “Apex Pressure” mattered.
- **No late attack inflation**: `late_wave_damage_ramp` remains as a compatibility field but is locked to `1.0x`. Levels 50–99 no longer add a multiplier to ordinary breach, enemy skill, Boss skill or Boss base-attack damage; data validation permanently rejects reintroducing that multiplier.
- **Exact all-skill runtime audit**: `tools/audit_physical_endgame_runtime.gd` stages real Battle / Projectile / Enemy code for autocannon, railgun and scattergun across chip/pet candidates, one Apex collider and a deterministic 45-enemy formation. It includes max split shot, pierce, five-lane multishot, homing, crit, ricochet, salvo, charge shot, Vanguard active direct damage and barrage uptime. `tools/physical_endgame_runtime_benchmark.json` records the accepted collision-derived throughput and the audit rejects drift beyond 6%.
- **Boss survival window**: the corrected `boss_survival_hp_ramp` grows smoothly from `1.0x` at level 50 to `56.0x` at level 98, with a `60.48x` finale. Runtime spawning, pressure tools, simulations and smoke coverage apply the same HP-only multiplier. Against the fastest legal full-skill counter (scattergun), Apex TTK at levels 90/95/99 is `57.1 / 90.2 / 116.6s`, providing about `8 / 15 / 21` core-skill windows.
- **Crowd density**: `late_wave_count_level_ramp` starts after level 55 and only affects wave-three-plus ordinary/support enemies. It reaches `1.25x` for levels 90–98 and `1.35x` at level 99, raising the finale from roughly `424` to `550` ordinary enemies without duplicating a Boss or changing waves one/two. The `level_025` endless template stays outside this main-campaign ramp.
- **Clearability matrix**: level 99 contains about `23.52M` mob HP and `214.50M` Apex HP. Using the same-loadout runtime benchmark, maxed physical autocannon, railgun and scattergun lower-bound damage times are `231.1 / 199.7 / 117.2s`; all remain below `260s`. Actual wave duration is additionally bounded by the authored spawn cadence for roughly 550 ordinary enemies.
- **Player-facing pressure**: recommended power includes the count curve linearly and the Boss survival multiplier logarithmically, producing about `757` at level 99. Runtime, XP budgeting, reconstruction tools, schema documentation and all balance gates share the same formulas.
- **Verification**: asset pack (`9,222` files), data, `355` `res://` references, all 99 pressure rows, card simulation, balance profile, campaign simulation, exact endgame matrix, neutral four-character DPS audit, Godot boot and M1 smoke all pass. The real projectile benchmark was rerun after integration and stayed within its 6% collision-variance gate.

## Pet-role audit and three-layer medical-drone repair (2026-07-26)

- **Roster audit**: all six pets were reviewed at maximum level. Turret Drone retains the strongest direct physical assist, Fire Imp retains fire status pressure, Frost Wisp retains slow plus base durability, Volt Orb retains chain growth, and Collector retains its economy tradeoff. Medical Drone was the clear outlier: its old wave heal grew only from 8 to 17.28 absolute HP and became negligible against late-game base pools measured in the thousands.
- **Three-layer repair**: the repair role is now fully data-driven. Wave Preparation combines a growing flat value with maximum-base-HP percentage healing; Sustained Repair runs on an authored interval; Emergency Rescue triggers below an authored HP threshold and has an independent cooldown. Every path clamps to maximum base HP and stops after battle completion.
- **Accepted maximum values**: level 30 provides `17 + 7.9%` per wave, `1.725%` every 18 seconds, and a `12.35%` rescue at or below 35% HP with a 45-second cooldown. Existing passive bonuses remain `+15.7%` maximum base HP and `10.8%` breach-damage reduction. The pet gives up direct damage, so the stronger sustain remains a deliberate defensive-slot tradeoff.
- **Combat feedback**: repairs animate the pet, draw a green repair link from the drone to the defense line, play the accepted contained regeneration sequence, add a bounded contact ring and heal number, and pulse the base-HP bar. Routine and major repairs use separate visual weights without adding new risky source art or changing existing VFX assets.
- **Collection and power clarity**: the pet detail sheet now shows current and maximum Wave Preparation, Sustained Repair and Emergency Rescue values, plus threshold, interval and cooldown. Upgrade previews show percentage growth instead of the old flat-only hint. Loadout power accounts for authored repair uptime and emergency value as well as passive stats.
- **Permanent contracts**: data validation requires all repair-role fields and bounds percentages, intervals, threshold, cooldown and level growth. M1 smoke stages a level-99 base pool and asserts exact periodic, emergency and per-wave healing, cooldown entry, maximum-HP clamping and repair-aware power growth. The screenshot helper can deterministically trigger medical repair, and the full visual matrix now uses the denser level-30 Medical Drone detail route.
- **Verification**: asset validation passes at 9,222 files; data, all 355 `res://` references, 99-level pressure, card simulation, Godot boot and M1 smoke pass. Two focused live-runtime captures were manually reviewed, and the complete 79-route visual screenshot suite passes with no layout or safe-area findings.

## Full pet-skill identity and flattened collection star economy (2026-07-27)

- **Six complete pet roles**: every pet now owns a data-driven `pet_skill`. Turret Drone uses a timed fire-rate/damage overclock, Fire Imp and Frost Wisp resolve local area skills around the locked/highest-priority enemy, Volt Orb grows from three to five overload targets by level 30, Medical Drone retains its three-layer base repair, and Collector converts every wave into level-scaled equivalent-kill gold.
- **Shared growth contract**: live combat, collection summaries, max-level detail values, upgrade previews and loadout power all read the same skill row. At maximum pet level, the accepted signatures are 4.2 seconds / 1.90x fire rate / 1.22x damage for Turret, 175 radius for Fire, 328 radius for Frost, five targets for Volt, and about 5.2 kill equivalents per wave for Collector.
- **Readable combat feedback**: existing accepted production sequences provide cast identity, bounded rings, pet animation and named feedback. Fire and Frost clearly mark affected groups; Volt combines forked arcs with a thicker cyan energy trunk so the multi-target path remains readable at phone scale.
- **Flat unlock curve**: characters, weapons, armor, chips and pets now cost 8–16 stars per paid item, with no category exceeding a 2x max/min ratio. The full paid collection totals 318 stars: 297 from the normal campaign buys almost everything, and 21 challenge stars complete the collection. No item requires a 60–230 star hoard.
- **Permanent economy gates**: data validation checks every pet skill shape, synchronized legacy weapon price, the 8–16 band, per-category ratio and a 300–330 total envelope. Balance and economy checks independently enforce the same player-facing curve and print total / maximum / remaining challenge-star values.
- **Regression and visual closure**: M1 smoke stages each pet skill against deterministic enemies and verifies target counts, area coverage, status strength, cooldowns and exact gold settlement. The full visual suite grows from 79 to 89 routed screenshots with five live pet-skill scenes and five additional tall pet-detail scenes; all routes pass layout and safe-area audit.

## TestFlight Build 38 (2026-07-27)

- **Published source identity**: the complete character/zombie animation, combat-VFX, late-game confrontation, pet-skill and collection-economy candidate is committed as `ae47d33353f8bee2847ca0b41ab73ad8c53622e2` and pushed to `origin/main`. The delivery manifest confirms no tracked working-tree changes were present when the build started.
- **Repeated source gate**: the shipping script reran the complete Release Candidate after the push. Asset/data/reference validation, all 99 pressure rows, balance/economy/card simulation, endgame runtime benchmarks, font and VFX safety/semantics, audio, tall layouts, Godot/battle boot, save integrity, M1 smoke and all 89 routed screenshots passed.
- **Exported-runtime proof**: the iPhone-only Build 38 PCK contains 6,923 files and 3,340 imported resources at 603,709,648 bytes (575.7 MiB). Exported-PCK battle boot, save/lifecycle integrity and M1 smoke passed with validated Godot logs. SHA-256 is `37dfac7f33c9e7fd02185cb7fba66167a153ba20d60fc6a31a67487e59f9bed0`.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded for `1.0.0 (38)`. The audited IPA is 633,091,255 bytes (603.8 MiB), SHA-256 `4043afa9e93a96a8976c5ce3b76d8dbc1fb5b72626d8632aa05ba33326f1f96a`.
- **Apple delivery**: the 603.8 MiB upload completed in about 32 minutes. Delivery UUID is `14646ccd-e6e3-456f-9bd2-7f27b233e7f6`; Apple reports `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, version `38` and `USES-NON-EXEMPT-ENCRYPTION: false`.
- **Local record**: the validated IPA and atomic delivery manifest remain under `build/ios/`. macOS denied replacing the optional Desktop copy, which did not affect the uploaded artifact, Apple validation or TestFlight availability.

## Pre-release Asset, Interaction And Experience Closure (2026-07-27)

- **Modal priority correctness**: entering a skill-card offer now removes both the visible wave/onboarding banner and its queued successor. The whole battle remains paused and the player keeps the battlefield context, but no lower-priority copy leaks behind the modal.
- **Deliberate high-risk entry**: loadout compares projected and recommended power. Below `65%`, it presents a severe-power state and requires a second press within 2.6 seconds. This remains a warning rather than a hard gate, preserving deliberate challenge runs.
- **Completed progression copy**: maximum-level collection details show `已满级 / 成长已完成`; the disabled upgrade action also reads `已满级` instead of surfacing a stale coin cost. Smoke coverage opens a real max-level weapon detail and checks both surfaces.
- **Campaign hierarchy**: map chapters distinguish current progression, reviewable cleared content and merely expanded content. The current level receives a compact marker, and chapter actions use `继续推进` or `回顾战区` according to state. Loadout keeps combat-power state ahead of counter guidance.
- **Result identity**: results use the level's real environment background and show the selected hero, outcome, kills, Boss kills or survival moment in a compact panel. Entry is staged across content, showcase, rewards, stars and actions, with a reduced-effects bypass.
- **Visual bug found and closed**: the first victory-particle capture exposed TextureRect native-size expansion, which covered most of the result card. Sparks now explicitly ignore source size, stay at 20–32 px, use eight particles and travel only 110–210 px around the star row. Both animation-middle and settled screenshots are clean.
- **Runtime audio hierarchy**: Boss intro, signature skills, breach warnings and result stingers request short BGM duck profiles with fast attack and slower release. Source masters remain untouched because no monitored listening pass was available; the audio guard now requires the runtime hierarchy.
- **Safe package audit**: release excludes still keep source references, videos, contact sheets, flow assets, environment sources and registered VFX tails outside the PCK. The remaining package weight is dominated by 2,022 runtime animation PNGs (~446 MiB) and 1,022 VFX PNGs (~192 MiB); risky last-minute lossy conversion was intentionally rejected and documented in `design/assets/release_package_asset_audit_2026_07_27.md`.
- **Regression proof**: M1 smoke covers modal toast clearing, max-level completion copy, severe-power confirmation, power/counter hierarchy and result outcome/background bounds. Focused standard/tall screenshots were reviewed, then the full 89-route visual matrix and complete Release Candidate passed. Asset validation reports 9,223 files; data, 355 references, 99-level pressure, balance/economy, color/contrast, every animation/VFX/audio guard, Godot/battle boot and save integrity all pass.

## First-Run Controls And Long-Campaign Navigation Continuity (2026-07-27)

- **Accurate touch coaching**: level 001 and the Settings help copy now describe the actual input contract: automatic fire, hold-and-drag manual aim, double-tap zombie lock and double-tap-empty clear. The old single-tap wording could no longer disagree with `InputManager`'s 0.30-second hold and double-tap detector.
- **Campaign context restoration**: the outer map focuses the chapter containing the first unlocked uncleared level. Current chapter detail focuses that level, while returning from any reviewed chapter restores the chapter card the player just left. Stable `level_NNN` node names make the behavior deterministic and testable.
- **Collection context restoration**: rebuilding a collection list preserves its scroll offset after equip, purchase or upgrade. Old rows leave the parent namespace before deferred release, so replacement rows retain their data IDs instead of generated `@TextureButton` names.
- **Release-truth cleanup**: both Owner action lists now identify Build 38 as the latest uploaded valid build and distinguish it from newer local source. Asset status no longer calls the accepted runtime animation/VFX or completed 22-second gameplay preview placeholders; monitored audio mastering remains explicit external sign-off.
- **Regression proof**: M1 smoke covers late-campaign chapter 9 / level 089 focusing, chapter-back restoration, collection offset retention, stable purchase IDs and exact first-run copy. Severe-power double confirmation is exercised with a deterministic fresh-save/final-challenge fixture, while the normal challenge route no longer mistakes the deferred scene-change frame for an armed warning. The routed visual matrix grew from 89 to 91 states and passed; focused captures in `tmp/final_experience_round_2026_07_27/` were manually reviewed without clipping, overlap or safe-area defects. The complete Release Candidate passes with 9,223 assets, 356 `res://` references, all static/runtime gates, save integrity and the full screenshot matrix.

## Premium Themes And Apocalypse Arsenal Plan Freeze (2026-07-27)

- **Owner-approved direction**: the app remains free, without ads, subscriptions, stamina, random paid rewards or paid currencies. The final catalog contains three permanent visual themes at a US$1.99 reference price and three permanent attribute-specific Apocalypse Arsenal sets.
- **Product relationship**: each complete arsenal is planned at US$6.99 and includes its matching theme. A theme owner sees a separate approximately US$4.99 arsenal-upgrade product, preventing the theme value from being charged twice. The plan uses three non-consumable SKUs and two logical entitlements per series.
- **Progression and power**: every premium item begins at level 1 and uses ordinary gold. The maximum-level complete set targets 1.52x–1.58x the real total output of the matching strongest free maximum-level build, with a 1.55x center. All weapon, skill, projectile, status, pet and set-bonus damage is included; armor survival is measured separately.
- **Visual contract**: themes cover all four hero outfits, weapon finishes, UI, base, HUD, result presentation and branded art. Apocalypse equipment requires new silhouettes, animation, projectile, hit, persistent status, kill, showcase and maximum-level awakening presentation rather than palette swaps.
- **Phase 1 lock**: the first production set is Neon Tempest and the complete Thunder Apocalypse weapon/armor/chip/pet set. Later series could not begin before the first end-to-end set passed acceptance.
- **Source of truth**: `design/21_premium_themes_and_apocalypse_arsenal_plan.md` contains the final three-series catalog, entitlement matrix, series briefs, upgrade milestones, balance definition, asset/package budgets, architecture, QA gates and cross-session handoff checklist.
- **Implementation truth**: no StoreKit code, App Store IAP record, premium data row or premium runtime asset has been added in this planning pass. Build 38 remains the current no-IAP uploaded baseline.
- **Phase 1 inventory**: `design/22_neon_tempest_thunder_phase1_inventory.md` records the current lightning build baseline, asset counts, file touchpoints, three-SKU purchase model, bilingual gap, Phase 1A~1F work breakdown and acceptance gates.
- **Architecture finding**: duplicating the existing fully composited hero/weapon animation matrix would add roughly 184 MiB per theme. Phase 1 therefore gates on proving the existing weaponless-hero + independent-weapon composition path across four representative combinations before bulk rendering.
- **Theme integration finding**: current screens contain roughly 95 direct premium-relevant UI path occurrences across 17 scene/script files, with about 40 unique UI textures. A centralized `ThemeManager` / semantic asset resolver is required before introducing theme assets.
- **Battlefield finding**: the 14 battle backgrounds bake the default base into their lower area. Neon Tempest will use a safe-area-aware bottom base overlay and validate it on lava, glacier, storm and apex environments instead of duplicating all 14 full-size backgrounds.
- **Commerce/localization finding**: there is no purchase manager, StoreKit configuration, iOS plugin or entitlement migration yet, and runtime localization currently loads only `data/localization_zh.json`. These are explicit Phase 1B/1D prerequisites, not completed features.
- **Balance baseline**: the current maxed Volt/Tesla/element/free-pet audit is 137,219 single-target DPS and 200,506 five-lane output. The Thunder premium target band is therefore 208,573–216,806 single-target and 304,769–316,799 five-lane, subject to deterministic combat-audit confirmation once data exists.
- **Next executable package**: Phase 1A-1 visual direction is complete. Phase 1A-2 is limited to selecting and runtime-sizing one Heavenfall Arc Cannon silhouette, completing the other three equipment concepts, proving four representative modular hero/weapon combinations, coating three free weapons and validating the base overlay on four environments. It does not replace accepted runtime assets or change combat values.
- **Planning-pass regression**: asset-pack validation passed at 9,223 files; data validation passed at 99 levels / 20 zombies / 8 bosses / 16 skills / 14 environments / 10 challenge rules; all 356 `res://` references resolved; level pressure, card simulation, Godot headless boot and the full M1 smoke test passed. The macOS headless runner emitted only the known system CA lookup warning.

## Neon Tempest Phase 1A-1 Visual Direction Batch (2026-07-27)

- **Repository-owned candidate pack**: five selected visual sheets and the complete prompt / correction log live in `assets/production/source_refs/generated/premium_neon_tempest_phase1a_2026_07_27/` and are registered in `assets/production/OUTSOURCER_ASSET_INDEX.json`. They remain source references and do not enter the runtime export.
- **Hero direction**: one 2x2 sheet establishes distinct Neon Tempest outfits for the four locked heroes. Graphite / cyan / violet materials are shared, while silhouette language stays character-specific instead of applying one costume or a flat palette swap.
- **Weapon choice**: the Heavenfall Arc Cannon has three meaningfully different silhouettes: long twin-ring crown rail, heavy triple-coil rotary storm cannon, and suspended-capacitor cathedral bullpup. No selection has been frozen before owner review.
- **Cross-screen theme proof**: one style board covers the menu emblem and environment, collection card, battle HUD/base overlay and result panel without generated labels or placeholder glyphs.
- **Combat language**: the six-panel storyboard defines charge, upward primary shot, ordered chain, attached overload mark, pet-origin discharge and max-level pillar. Every effect has a visible source and target and preserves the game's bottom-to-top attack direction.
- **Self-review correction**: the first integrated battle composite was rejected because it contained seven zombies and made secondary arcs too dominant. The selected revision contains one hero and six zombies; the marked primary target is brightest, secondary arcs are thinner/dimmer, and hero, weapon, pet, base and HUD remain visible.
- **Implementation truth**: no transparent runtime cutouts, animation sequences, UI slices, theme resolver, premium data, balance change or StoreKit code was added. Phase 1A-2 is gated on owner direction and must prove runtime-scale modular weapon mounting plus four-environment base overlays before bulk generation.
- **Validation**: asset-pack validation passed with 9,229 files after registering the five PNG candidates and prompt log; data, all 356 `res://` references, 99-level pressure, card simulation, Godot headless boot and the complete M1 smoke test passed. The only boot diagnostic was the known macOS system CA lookup warning.
- **Owner review**: the owner confirmed that the overall Neon Tempest rendering direction is satisfactory. Hero outfits, cross-screen UI/base language and the six-beat Thunder VFX direction may proceed to runtime-scale proof. The exact cannon silhouette remains the only visual selection still open; the current production recommendation is the center triple-coil rotary design, shortened for modular character mounting.

## Neon Tempest Phase 1A-2 Runtime-Scale Proof (2026-07-27)

- **Locked set direction**: the owner's continue instruction accepted the recommended center Heavenfall Arc Cannon. The production proof shortens its body by about 20% while preserving exactly three induction rings, two-hand mounting and the premium silhouette. Tempest Conductor Armor, Superconductive Storm Core and Storm Orb now have distinct equipment forms in the same material family.
- **Free-weapon hierarchy**: automatic cannon, handheld Tesla coil and flamethrower have Neon Tempest coatings while keeping ballistic, electrical and fire identities. The first generated sheet was rejected because it reproduced the Tesla icon as a freestanding tower; the selected `v2` uses independent handheld runtime frames and remains visually below the legendary cannon.
- **Modular mounting proof**: four representative combinations preserve hero identity and body build, keep full heads and feet inside frame, show two visible hand contacts, keep stocks at shoulders and leave every upper-right muzzle unobstructed. No accepted combo has a floating hand or weapon passing through the torso.
- **Base-overlay proof**: one Neon Tempest barricade language works on lava, glacier, storm and apex fields without recoloring their environmental identity or blocking the center lane. The first phone-scale sheet omitted the glacier base and was rejected; `v2` restores the same complete base behind the HUD without covering Frost.
- **Phone-scale proof**: four 9:16 comparison viewports keep each hero at roughly 14%~17% of viewport height. Weapon identity, hero silhouette, base and HUD remain readable without combat effects.
- **Repository record**: five selected sheets plus full prompts, rejected-first-pass causes and correction constraints live in `assets/production/source_refs/generated/premium_neon_tempest_phase1a2_2026_07_27/`, are Git-trackable through a narrow `.gitignore` exception and are registered in `assets/production/OUTSOURCER_ASSET_INDEX.json`.
- **Implementation truth**: Phase 1A-2 is visual/compositing proof, not transparent runtime art. No free runtime asset, balance value, entitlement or StoreKit path changed. The next package is Phase 1B-1: ThemeManager, minimal theme schema, save migration and semantic routing on five representative pages using a development fixture.
- **Validation**: asset-pack validation passed with 9,235 files; data, all 356 `res://` references, 99-level pressure, card simulation, Godot headless boot and full M1 smoke passed. The only boot diagnostic was the known macOS system CA lookup warning.

## Neon Tempest Phase 1B-1 Theme Runtime Foundation (2026-07-27)

- **Semantic runtime**: added `ThemeManager`, `data/themes.json`, Save v2 theme selection and verified permanent-entitlement fields. A missing entitlement, invalid selection or missing asset resolves safely to the default theme. Debug preview uses `ZOMBIE_FIRE_THEME_PREVIEW=neon_tempest`; it does not grant or persist a production purchase.
- **No-stretch button production**: generated independent short, compact, standard, long, ultra-long and 17:1 ribbon models. The rejected first runtime derivation visibly repeated center diamonds and was discarded. The selected builder uniformly contains complete models inside 72 exact-size primary / secondary PNGs, and `UiKit` now enforces `STRETCH_KEEP_ASPECT_CENTERED`.
- **Character runtime finish**: Neon Tempest applies an animated cyan / violet / rainbow circuit-rim material to the battle character and result portrait. A protected upper band keeps the head, hair and identity untouched; warm-skin masking further suppresses spill. Reduced-effects mode freezes movement and lowers intensity.
- **Representative visual QA**: real 1080×1920 Godot captures covered menu, character collection, loadout, settings, storm battle and stable victory result. Runtime UI audits reported no clipping or safe-area errors. Shader QA rejected the first build because CanvasItem fragment functions cannot early-return; the corrected material compiled and was recaptured.
- **Complete regression**: the premium debug route passed all 91 real-screen captures. Asset validation passed with 9,417 files, data validation with 99 levels / 20 zombies / 8 bosses / 16 skills, all 360 `res://` references resolved, level pressure and card-director simulations passed, and Godot headless boot, Save v2 integrity, theme routing and the complete M1 smoke test all passed.
- **Commerce truth**: this phase adds only entitlement-aware theme selection plumbing. StoreKit products, purchase UI, restore purchases and Thunder Apocalypse combat values remain Phase 1B / 1C work and are not represented as complete.

## TestFlight Build 39 · Neon Tempest Preview (2026-07-27)

- **Acceptance-build contract**: Build 39 is a TestFlight-only visual acceptance build. Its exported iOS custom feature activates `neon_tempest` without granting or persisting a purchase entitlement, so the owner can immediately inspect the no-stretch buttons and character iridescence. The local iOS release preset was restored to ordinary `release` immediately after upload; Build 39 must not be selected for App Review.
- **Source identity**: the delivery manifest records source commit `49af255883abf1efcb4f566b0e8fe0f8f890cda8` with tracked working-tree changes present. This is intentional for owner preview and is not represented as the committed production baseline.
- **Release gate**: the complete Release Candidate passed with 9,417 assets, 360 resolved `res://` references, all balance / economy / animation / VFX / audio / long-screen checks, Godot boot, battle boot, Save v2 integrity, M1 smoke and all 91 routed screenshots.
- **Exported runtime**: the iPhone-only Build 39 PCK contains 7,071 files and 3,412 imported resources. It is 607,877,860 bytes with SHA-256 `170cd123b0631cb08aad3140e92f70a8dad1fdadcf7525d28e5014df8d323b02`; exported-PCK battle boot, save integrity and M1 smoke passed.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded. The audited IPA is 637,222,351 bytes (607.7 MiB), SHA-256 `2c338669b6c3a092451365201d1511fbe2346246850a5ede4d73ca41c9b2ed4c`.
- **Apple delivery**: Delivery UUID `ec1400d1-68ec-43cc-9f64-42049f5156ac`; final status is `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, and `usesNonExemptEncryption=false`.
- **Artifacts**: the verified IPA and release manifest remain under `build/ios/` and `build/ios/release/build_39/`. macOS blocked replacement of the older Desktop IPA copy, which did not affect the upload or Apple validation.

## Simplified Chinese / English Runtime Localization (2026-07-27)

- **Two-language runtime**: `LocalizationManager` registers a dynamic English Translation and switches between `zh_CN` and `en`. Settings persist the selection; fresh installs follow the device language while legacy settings without a language key remain Chinese.
- **Complete English copy**: the 88 stable content IDs now have exact English parity. Separate UI, gameplay and story catalogs cover navigation, loadout, collection, combat HUD, card offers, pause, results, all campaign objectives, ten zone stories, challenge rules and Boss mechanic copy.
- **Formatted-string compatibility**: the translation resource recognizes authored printf templates after business code has formatted them, recursively translates inserted content and falls back to longest reusable terms. Exact terms take priority over ambiguous short patterns; complete templates take priority over partial term concatenation.
- **Mobile editorial pass**: English uses concise button and badge copy rather than literal translation. Chapter prose was rewritten as phone-sized summaries and wrapped on word boundaries; loadout empty slots, collection tags, card descriptions / recommendation badges, Settings help and result titles received locale-aware layout treatment.
- **Regression gates**: `check_localization.py` enforces ID parity, CJK-free English targets, placeholder parity and coverage of every runtime Chinese source. `localization_smoke_test.gd` verifies exact, dynamic, term-conflict and locale-switch behavior. The visual audit now measures translated text, button labels, wrapped height and explicit multiline height.
- **Visual proof**: 14 English routes cover menu, overview/detail map, loadout, three collection modes plus tall details, Settings, pause, card offer/detail and challenge result. Focused real Godot captures of map, loadout, Skill Codex, card offer, Settings and result were manually reviewed after iterative overlap fixes.
- **External metadata boundary**: runtime localization does not publish App Store Connect or StoreKit localization. English App Store metadata, screenshot captions and future IAP display names remain an explicit owner/release task documented in `design/23_bilingual_localization_framework.md`.

## English Menu Branding And Skill Codex Alignment Closeout (2026-07-27)

- **Locale-specific title art**: added a production-ready stacked `ZOMBIE FIRE` title that preserves the Chinese logo's cracked steel, orange underglow, cyan rim light and portrait-first visual weight. Runtime selection follows the current language without changing the Chinese title.
- **Production provenance**: registered the generated title, prompt and transparent source references in the production asset pipeline so the English branding remains reproducible.
- **Authored English card grid**: Skill Codex cards now use fixed name and level columns, compact semantic tags and a dedicated `Projectile` localization term. Long skill names no longer push `Lv.0` out of alignment.
- **Focused visual proof**: real 1080×1920 English menu and Skill Codex captures report zero runtime UI-audit findings; a targeted Chinese Skill Codex regression confirms the original locale remains intact.

## TestFlight Build 40 · Neon Tempest And Bilingual Acceptance (2026-07-28)

- **Acceptance-build contract**: Build 40 combines the forced Neon Tempest TestFlight preview with the complete Simplified Chinese / English runtime, the rendered English title and the corrected Skill Codex grid. It grants no entitlement and must not be selected for App Review.
- **Gate-found regression**: the first release pass stopped before consuming a build number because the localized loadout refactor had broken the compact pet-summary source contract. The summary and matching English template were corrected, then gameplay-polish, localization, release-string and Godot boot checks passed before the full pipeline restarted.
- **Release gate**: the complete candidate passed with 9,424 assets, 366 resolved `res://` references, all balance / economy / animation / VFX / audio / tall-screen checks, Godot boot, battle boot, Save v2 integrity, localization smoke, M1 smoke and 105 routed screenshots.
- **Exported runtime**: the iPhone-only Build 40 PCK contains 7,081 files and 3,413 imported resources. It is 609,183,408 bytes with SHA-256 `60193a5a36aaf0d412c3108804a020e6cea6c8db7e3b8131bfdc3c44ad1365cb`; exported-PCK battle boot, save integrity and M1 smoke passed. A direct post-upload PCK probe proved `neon_tempest_preview=true` and resolved the active theme to `neon_tempest`.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded. The audited IPA is 638,491,266 bytes (608.9 MiB), SHA-256 `fe8a1b3c715743bb64530ce4372717e6e4c15bdd8995a1cdf7f295e14f560981`.
- **Apple delivery**: Delivery UUID `f8f36abd-ed24-46db-a278-77b03a862167`; final status is `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, and `usesNonExemptEncryption=false`.
- **Local release state**: the audited manifest is stored at `build/ios/release/build_40/release_manifest.json`, and the iOS preset is back to ordinary `release` with build number 40. macOS blocked replacement of the optional Desktop IPA copy; the uploaded artifact and Apple validation are unaffected.

## Starter Autocannon Projectile Hierarchy Repair (2026-07-28)

- **Root cause and tradeoff**: `weapon_autocannon` itself had no split, chain or splash value, but Vanguard granted one physical pierce at level 1. The shared pierce runtime can immediately sweep or modestly retarget toward a second enemy, so the starter round read like ricochet. At the same time, projectile contact and battle impact each contributed radial light / shock-ring layers, making a zero-radius hit look like splash damage.
- **Progression correction**: Vanguard now grants zero base pierce and unlocks two pierces at growth rank II (character level 15). The first-run weapon is therefore single-target, while the maximum physical-affinity ceiling remains the same two pierces used by the existing endgame balance.
- **Visual correction**: the starter weapon now resolves to an explicit `autocannon` visual profile. Its hit owns one compact directional contact glow plus five metal sparks and returns before the generic impact-ring stack. Split, chain, splash and learned pierce paths keep their authored specialist feedback.
- **Permanent regression**: the M1 smoke suite verifies zero level-one split / chain / splash / pierce, zero runtime splash / chain, the rank-II two-pierce unlock and the compact autocannon profile. `check_gameplay_polish.py` independently gates the same data and runtime contracts.
- **Focused visual proof**: `tools/_shot.gd` can stage `debug_hit_showcase.kind=starter_autocannon`. A real 1080×1920 capture with level-one Vanguard, level-one autocannon and empty armor / chip / pet slots was reviewed at `/tmp/zombie_fire_starter_autocannon_hit.png`; the contact remains local to the target with no expanding ring or secondary trajectory.
- **Release regression**: asset validation passed with 9,424 files, data validation with 99 levels, all 367 `res://` references resolved, and level pressure, card-director simulation, endgame balance, gameplay polish, Godot boot and the full M1 smoke all passed. The graphical audit independently passed all 105 routed Simplified Chinese / English screenshots after running in the required macOS window-server context.

## Neon Tempest Four-Hero Runtime And Bilingual Layout Closeout (2026-07-28)

- **Rendered hero runtime**: the theme now owns four frameless portraits and 44 transparent modular battle frames. Each locked hero has four idle, four authored attack / recoil and three hurt poses; heads, feet and silhouette-safe margins were checked after a clipped contact-sheet extraction was rejected and rebuilt from complete connected subjects.
- **Modular battle path**: an active Neon Tempest theme resolves the themed weaponless character sequence before the legacy 992-frame composited matrix, while preserving the independent weapon layer and existing weapon identity. This keeps the runtime addition bounded and gives future premium weapons one stable socket path.
- **Rendered firing aura**: the rejected procedural shoulder-to-muzzle grid was removed. Every shot now plays a four-frame cyan / violet plasma-arc wing behind the hero, rotates toward the firing direction and keeps the face, torso center and gun readable. Reduced-effects mode uses a smaller, calmer presentation.
- **Theme-specific combat language**: character suit edges, free-weapon surfaces, projectiles and muzzle treatment share the Neon Tempest palette. Active casts add the same premium energy baseline without replacing Vanguard heavy salvos, Blaze molten bursts, Frost crystal eruption or Volt storm-chain semantics.
- **Reversible acceptance access**: Debug / TestFlight preview access exposes default and Neon Tempest in Settings without writing a verified purchase entitlement. The environment fixture remains available only for deterministic automation.
- **English layout closeout**: the English matrix grew to 26 routes, with deterministic long-press coverage for every hero. Adaptive labels, concise gameplay copy and corrected authored bounds resolve active-skill copy, Boss phase help, recommendation badges, card-detail close target, loadout summaries, purchase actions and prior CJK leakage.
- **Safe-area follow-up**: the complete visual run found fixed-width detail actions pushing collection modals 2–14 px past a narrow tall-device safe edge. The outer gutter now yields while native button rasters remain untouched. Compact Settings reallocates internal whitespace so Chinese and English instructions both stay inside top and bottom insets without shrinking touch targets.
- **Provenance**: prompts, original generated sheets and correction history live under `assets/production/source_refs/generated/premium_neon_tempest_runtime_2026_07_28/`; production portraits, frames and aura sequences are registered in `assets/production/OUTSOURCER_ASSET_INDEX.json`.
- **Final regression**: asset validation passes with 9,541 files, data validation with 99 levels / 20 zombies / 8 bosses / 16 skills, all 368 `res://` references resolve, and level pressure, card simulation, localization, gameplay / visual assets, Godot boot, theme routing and M1 smoke all pass. Runtime UI proof passes 117 complete routed screenshots plus a 43-screen Neon Tempest matrix containing the full four-hero / eight-free-weapon firing grid.
- **Commerce boundary**: this is a visual preview implementation, not StoreKit. No paid entitlement, purchase flow, Thunder Apocalypse equipment data or +50% endgame balance has been added.

## TestFlight Build 41 · Neon Tempest Full Runtime Acceptance (2026-07-28)

- **Acceptance-build contract**: Build 41 uses the temporary exported `neon_tempest_preview` feature so the owner can inspect all four themed hero runtimes, rendered firing aura, theme combat VFX and bilingual UI. It grants and persists no purchase entitlement and must not be selected for App Review.
- **Release-pipeline hardening**: `tools/ship_testflight.sh` can now inject a TestFlight-only custom feature and restores the original preset on every exit path. A post-export PCK probe independently proved `neon_tempest_preview=true`; the local iOS preset returned to plain `release` after delivery.
- **Gate-found layout regressions**: the release gate caught a six-pixel English pause-chip clip and a randomized tall Chinese skill-card description before build upload. Locale-aware pause sizing, adaptive card height and deterministic long-copy screenshot fixtures fixed both without stretching native button assets.
- **Release gate**: the complete candidate passed with 9,541 assets, 368 resolved `res://` references, all balance / economy / animation / VFX / audio / tall-screen checks, Godot boot, battle boot, Save v2 integrity, localization smoke, ThemeManager, M1 smoke and 117 routed screenshots.
- **Exported runtime**: the iPhone-only Build 41 PCK contains 7,185 files and 3,465 imported resources. It is 616,014,340 bytes with SHA-256 `1f4a3e358758e1e14c169ad71a5f6f667b17bbda8e8def912379b1739a762fbf`; exported-PCK battle boot, save integrity, M1 smoke and feature probe all passed.
- **Signed artifact**: Xcode Archive and App Store Distribution export succeeded. The audited IPA is 645,281,641 bytes (615.4 MiB), SHA-256 `c3c6d0c27cc4a425deea4763fe85aeb55124bc794855ffd161cbfb518b004a4c`.
- **Apple delivery**: Delivery UUID `afde2490-39e9-485d-90e5-aa0f6d5ef956`; final status is `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, and `usesNonExemptEncryption=false`.
- **Local release state**: `build/ios/release/build_41/release_manifest.json` records source commit `140754ae0d29d4eee2dc4d93f80f98d622385bdb` with working-tree changes present. The iOS preset is plain `release` at build 41. macOS blocked replacement of the optional Desktop IPA copy; the uploaded artifact and Apple validation are unaffected.

## Difficulty And Growth-Curve UX Tuning · Phase 0 Simulator Repair (2026-07-29)

- **Root cause**: `tools/simulate_balance.py` switches the whole level model at `n >= 50` boss levels to the measured runtime benchmark. That benchmark's `crowd_dps` (18.6M) is peak throughput against a saturated 45-enemy formation, so even after scaling by permanent progression and card budget the mob phase cost only 0.1–1.3s for millions of HP. Combined with `boss_survival_hp_ramp` still being 1.0x at its own start level, levels 50/55/60/65 reported 2.0 / 10.1 / 16.0 / 19.2s clear times, and 50–70 boss durability was tuned against those numbers.
- **Lower-bound protection**: the mob phase is now bounded by `min(crowd_dps, dps_ws)` — the same campaign-wide crowd model every non-boss level already uses, since `estimate_skill_mult` is documented as effective crowd throughput rather than single-target DPS. The boss phase deliberately keeps the measured `boss_dps`, because `dps_ws` folds in crowd multipliers and is not a valid single-target rate; bounding it the same way would have reported a 2,197s finale against a measured 230s.
- **No gameplay knob moved**: `boss_survival_hp_ramp.max_mult` and `curve_power` stay at 56.0 / 1.15. The plan's allowance to nudge `curve_power` up to 1.3 was not needed.
- **Baseline recalibration (design/24 Phase 0)**: `clear_time_cap` for levels 90–98 moved from 330s to 350s. The 330s guard was set while the boss branch understated the band (level_095 read 188.6s); the corrected model reads 334.4s, which is consistent with the pre-existing 460s finale allowance and still catches a real HP wall.
- **Result**: boss clear times now read 67.2 / 83.4 / 96.9 / 104.2 / 134.5 / 165.7 / 202.0 / 271.2 / 281.1 / 334.4 / 446.6s for levels 50–99, monotonically increasing. `Levels < 30s (with skill)` is down to level_001 alone (28.1s), and no boss level is under 45s.
- **Verification**: `validate_data.py`, `check_level_pressure.py`, `check_balance_profile.py`, `simulate_balance.py`, `check_endgame_balance.py` and the full `check_release_candidate.py` all pass.

## Difficulty And Growth-Curve UX Tuning · Phase 1 Unified Star Rating (2026-07-29)

- **The split that caused it**: `battle.gd` hardcoded "3 stars needs a full base line, 2 stars needs half", while `tools/simulate_balance.py` rated levels on a leak budget (3 stars at leak ≤40%). All 99 levels were tuned against the simulator's rule, which the game never applied. `data/levels.json` even carried a `star_rule: "base_hp_percent"` field that no runtime code ever read.
- **Single source of truth**: `data/economy.json` now owns `star_thresholds` (`three_star_hp_ratio` 0.70, `two_star_hp_ratio` 0.35). `core/data/star_rules.gd` is the only reader; it carries an identical fallback table so a missing knob cannot silently change ratings. The simulator's `star_leak_caps()` converts the same file into its leak view (3 stars ≤30% leak, 2 stars ≤65%), and the old 40/70 constants are gone.
- **Visible rule**: the result screen prints the rule under the star row and the loadout tactical summary repeats it, both generated from the economy knob rather than typed out. The English catalog gained the matching printf template so the line localizes with the rest of the UI.
- **Layout follow-up**: the tactical summary panel grew from 316 to 388 px. English wraps the armor / chip / pet line to three rows, which had already left the panel nearly full; the added rule line clipped until the panel cleared that worst case. Verified with real Chinese and English captures.
- **Save compatibility**: no migration. Stars are credited as `max(new - old, 0)`, so a loosened rule can only add stars to existing profiles.
- **Headless proof**: a `level_001` victory forced to 80% base HP now returns 3 stars where the old rule returned 2; 50% returns 2 and 20% returns 1, matching the table exactly.
- **Distribution after the change**: 12 levels rate 3 stars, 66 rate 2 and 21 rate 1. Every one of the 20 boss levels is still 1 star — the exact problem Phase 2 exists to fix. The plan's rough estimate of 25 / 48 assumed the old 40% leak bucket; the shipped 0.70 threshold is the stricter number the plan specified.
- **Verification**: full `check_release_candidate.py` including 117 routed screenshots passes.

## Difficulty And Growth-Curve UX Tuning · Phase 2 Boss-Level Fairness (2026-07-29)

- **Goal**: boss levels went from "always 1 star" to "2 stars normal, 3 stars a stretch" without shaving the boss HP sponge that gives them their drama.
- **Data knob**: `data/economy.json` gained `boss_level_base_hp_mult` (1.25). Any level with a boss in a wave starts the base line at `base_hp_ref x 1.25`, before character / armor / chip / pet multipliers. Enemy pressure, the boss survival ramp and the recommended-power formula are untouched, and `simulate_balance.py` divides by the same cushion so the two sides agree.
- **The knob was only half the story**: 1.25 alone moved boss leak from 66–100% to 53–97%, leaving 16 of 20 boss levels at 1 star. The real driver was a second simulator defect — `leak_damage()` charged the *entire* level the 12% boss leak rate, including waves 1–4, which are ordinary mob waves with no boss on the field. Measured across the campaign, those waves carry 60–79% of a boss level's breach damage. Leak rate is now taken per wave (12% for the wave holding the boss, 5% elsewhere, the same rate ordinary levels already use).
- **Result**: boss leak now reads 33–57% and all 20 boss levels reach the 2-star threshold. Campaign distribution is 12 / 86 / 1 across three / two / one star; normal levels did not move a single point, since they contain no boss wave. No level exceeded 90%, so no wave counts needed trimming.
- **Not done, deliberately**: no guaranteed repair event, and `boss_survival_hp_ramp.max_mult` untouched.
- **Open item for the owner**: the plan also asked for levels 5 / 10 / 15 to reach the 3-star threshold. That is unreachable inside the knobs the plan allows — it needs roughly a 2.4x base-line cushion, or wave-count cuts far past the 10% ceiling. Those levels now sit at 50 / 57 / 48%, and early boss levels are in fact the hardest band (late ones sit at 33–45%). That inversion is a real early-game difficulty problem and is flagged rather than silently patched.
- **Verification**: headless confirms level_005 / level_010 carry exactly 1.25x the base-HP ratio of level_004 / level_009. Data validation, level pressure, balance profile, balance simulation, endgame balance, economy loop and the full release candidate with 117 routed screenshots all pass.

## Difficulty And Growth-Curve UX Tuning · Phase 3 Star Economy Review (2026-07-29)

- **Outcome: no unlock price changes.** Full collection costs 318 stars. After Phase 1 and 2, a baseline build (free Vanguard + autocannon at recommended level, no elemental counter) earns 209 campaign stars and, modelled against the real `challenges.json` coefficients, 167 challenge stars — 376 total, or 1.18x demand. The plan's trigger for discounting the top tier is supply below 1.10x, so the prices stand.
- **Why the estimate method changed**: the plan suggested rating challenge levels one band below their normal rating. That over-penalizes any level sitting near the top of its band — a 34% leak level becomes `34 x 1.34 x 1.00 = 46%` under chapter-one challenge rules, still two stars, where the one-band rule would call it one star. Modelling leak against the authored `hp_mult` (1.30–1.42) and `breach_damage_mult` (1.00–1.18) gives 167 rather than 110. Both figures are recorded in the plan appendix so the judgement can be re-examined.
- **This is a Phase 1 + 2 dividend**: before the star rule was unified the game demanded a full base line for three stars, so realistic yield was far below the tuned model and full collection was close to impossible. Fixing the rating rule fixed the economy without touching a single price.
- **XP spot check**: demand for all 16 permanent skills plus 4 signature skills is 137,400 XP. Single clears of levels 10 / 30 / 50 / 70 / 90 pay 665 / 731 / 1,644 / 2,202 / 2,968. A single full campaign pass pays 138,055 — 100% of demand, and 201% including challenges, well past the 60–80% the plan asked for. XP is not a bottleneck; gold and stars are.
- **Logged for the owner, out of this phase's scope**: XP is oversupplied to the point that one campaign pass maxes every permanent skill, flattening the long-tail goal; `econ_xp_growth` is read from economy into `battle.gd` and never used; and `design/data/schema.md` documents a `star_total_cap` field that `data/economy.json` does not contain.
- **Verification**: `check_economy_loop.py` passes (star_unlock_total 318, max_item 16, normal_campaign 297, challenge_needed 21). No data or code changed in this phase.

## Difficulty And Growth-Curve UX Tuning · Phase 4 Smoothing (2026-07-29)

- **Chapter-opening spike: reverted, the premise was wrong.** `difficulty_coef` is not a difficulty index — it is a multiplier layered on top of each level's authored wave composition. Levels 61 and 89 carry high coefficients precisely because a chapter opener resets to fewer and softer enemies and needs the multiplier to carry the previous level's pressure forward. Actual pressure runs 37,302 at level 59 to 38,380 at level 61, a 2.9% step, and level 61 follows a boss level at 116,752 — a large drop, not an ambush.
- **The gate proved it**: applying the planned 4.13 → 3.10 and 5.13 → 4.40 made `check_level_pressure.py` fail with `level_061 pressure 28803.8 < level_059 37302.7` and `level_089 191905.2 < level_088 198573.8`. Under the plan's own red line — decide which side is right, never write a bug into the baseline — the check is right and the edit was dropped. Both coefficients stay at 4.1307 and 5.1302; levels 97 and 98 keep 6.62 and 8.24 as already agreed.
- **Late-game gold smoothing: shipped.** Levels 85–99 now floor `reward_gold_mult` at 0.26, replacing the 0.25-down-to-0.20 decay. `upgrade_cost_linear_k` is untouched.
- **Measured effect**: single-clear gold moves +4% at level 85, +8% at 90, +18% at 95 and +30% at 99, for +14.0% across the 85–99 band. The plan estimated +20–30%; the smaller total is simply because 85–87 already sat near the 0.26 floor. No economy baseline needed updating.
- **Verification**: data validation, level pressure, balance profile, balance simulation, endgame balance, economy loop and the full release candidate with 117 routed screenshots all pass.

## Difficulty And Growth-Curve UX Tuning · Phase 5 Element And Weapon Economy (2026-07-29)

- **Poison coverage tripled**: levels 032 and 034 (from physical) and 037 and 039 (from fire) now carry `primary_weakness: "poison"`, with each level's `card_bias` element key moved to `poison: 1.35`. Campaign weakness distribution is now physical 37 / fire 32 / ice 12 / lightning 12 / poison 6. None of the four levels changed star rating, exactly as expected — the simulator baseline carries no elemental counter, so weakness is pure player upside.
- **Two forced deviations from the plan, both documented**: the plan located `env_toxic_biolab` in chapters 8 and 9, but that environment exists only in chapter 4 (levels 31–40); chapters 8 and 9 are the void cathedral and orbital ruins. Chapter 4 also only holds two fire-weakness levels, so two physical levels — the most abundant element at 39 — were converted to reach the plan's count of four.
- **The venom launcher could only reach 8 stars, not 6**: `validate_data.py`, `check_balance_profile.py` and `m1_smoke_test.gd` all enforce that paid unlocks sit in an 8–16 star band and that no category spans more than a 2x price curve. Six stars breaks both (outside the band, and 2.67x against the 16-star plasma cannon). The weapon now sits at the band floor of 8, level with the flamethrower and cryo cannon and exactly 2.0x under the plasma cannon.
- **Baseline update (design/24 Phase 5, not silent)**: total unlock cost moves 318 → 316, so the smoke test's `total == 318` and `challenge_needed == 21` assertions become 316 and 19, with a comment pointing at the plan.
- **Counter suggestion in the loadout**: when the player already owns a weapon matching the level's weakness and has not equipped it, the tactical summary adds a green, tappable line — `建议武器：{name}（克制本关，伤害×1.5）` / `Suggested: {name} — counters this stage, damage x1.5`. It routes to the weapon collection and never swaps gear on its own. The 1.5 is read from `economy.json.weakness_mult`. The details panel switches between 388 and 452 px depending on whether the row is present; both languages were verified with real captures.
- **Left for the owner**: poison weakness is still confined to chapter 4, so the venom launcher remains a chapter-4 specialist even at 8 stars. Spreading poison into later chapters is a content and theming decision, not a tuning one.
- **Verification**: release string check, card director simulation and the full release candidate with 117 routed screenshots all pass.

## Difficulty And Growth-Curve UX Tuning · Phase 6 Skill Ecology (2026-07-29)

- **Scoring weights applied**: in `_combat_skill_effect_multiplier`, the survival weight moves 0.18 → 0.28, the barrier coefficient 0.22 → 0.35 and slow 0.30 → 0.40, while pierce's secondary-coverage credit drops 0.075 → 0.065. Gold rush is untouched; its low offer rate is the intended result of the one-economy-card-per-offer rule, not a strength problem. `RECOMMENDED_POWER_COEF` did not move, and measured recommended power is unchanged at 245 for level 50 and 757 for level 99.
- **Measured effect**: a four-level barrier stack gains 4.5% of skill multiplier (1.0317 → 1.0784), barrier plus slow gains 3.7%, a slow-field stack 2.4%, while a pure pierce stack loses 2.7% (1.4805 → 1.4405) and a mixed offensive draft loses 1.5%. Defensive drafts are now worth what Phase 1 made them worth: surviving base HP *is* the star rating.
- **Correction to the plan**: steps 4 and 5 of section 8 cannot be satisfied by steps 1 and 2, because they act on two unrelated systems. Skill offer rates come from the dealing stage — `gameplay/skill/card_director.gd` and its Python mirror weight each card as `4 + sum over tags of round(bias[tag] * 2)`, reading only `card_tags` and `card_bias` and never touching SaveManager. Likewise `simulate_balance.py` draws skill throughput from `tools/combat_power_model.py`, also independent of SaveManager. Empirically, the card director's full output is byte-identical before and after the weight change, and the star distribution stays at 12 / 86 / 1 — zero drift rather than the allowed plus or minus three levels.
- **Root cause of the offer imbalance, located**: offer weight is roughly proportional to a skill's `card_tags` count. `skill_barrier` carries a single tag, `defense`, and lands at 12.7% on level 50; `skill_pierce` carries four — `projectile`, `anti_armor`, `pierce`, `physical` — and the starter weapon being physical plus the `tank` threat tag stack on top of that, putting it at 31.7%.
- **Left for the owner**: moving offer rates requires editing `card_tags`, `card_bias` or the dealing formula, and section 8 explicitly restricts this phase to numbers only. Giving barrier extra tags is a content-semantics decision, so it is documented rather than taken unilaterally.
- **Verification**: card director simulation, balance simulation and the full release candidate with 117 routed screenshots all pass.

## Difficulty And Growth-Curve UX Tuning · Phase 7 Power Readout (2026-07-29)

- **The bug**: `get_loadout_power()` folds in a four-card projection, so the pre-battle number the result screen showed as 战前 already assumed a full draft. A run that picked fewer cards ended below it, and the line read like the player got weaker mid-run — a real capture showed 12 → 10.
- **Naming unified across the repo**: 战前 becomes 基准 / Baseline (the figure that includes the standard draft projection), 成型 becomes 预计成型 / Projected, and the stray 本局成型 becomes 终局战力 / Final Power since it always referred to the same quantity as 终局. 终局 / Final is unchanged. A repo-wide grep confirms no mixed usage survives, including the explanatory comment in `save_manager.gd`.
- **Partial draft marked**: when the final number lands below the baseline, the result hint now says so explicitly — `基准 12 → 终局 8（选卡未满）` and `Baseline 12 → Final 8 (partial draft)`. Each of the three result templates gained a marked variant, six catalog keys in total, plus four new shared terms.
- **The yardstick did not move**: `RECOMMENDED_POWER_COEF` is calibrated against this exact scale, so rescaling it would mean re-deriving every recommendation for a display problem. The plan reached the same conclusion. Section 9's long-press explainer was skipped by the plan's own rule — the loadout power cells carry no tooltip or detail popup today, so no new interaction was invented.
- **Headless proof**: finishing a level_003 run with zero cards picked returns `cards_picked=0`, standing 12, combat 8, and the rendered result label contains the partial-draft marker.
- **Verification**: release string check and the full release candidate with 117 routed screenshots pass, in both languages.

## Difficulty Tuning Follow-up · Early Boss Cushion Curve (2026-07-29)

- **Correction to the Phase 2 write-up**: boss-level pressure is U-shaped rather than simply front-loaded. Levels 5–20 and 65–99 both sit at 46–57% leak while the 25–60 middle sits at 33–46%. A flat 1.25 cushion therefore left the first three boss levels a player ever meets among the hardest in the campaign.
- **The knob is now a curve**: `boss_level_base_hp_mult` accepts an object — `base` 1.25, `early_mult` 1.75, `early_full_level` 10, `early_end_level` 25 — applying the early value at or below level 10, the base value at or above level 25, and a linear blend between. A plain float still works for backward compatibility. `battle.gd._boss_level_base_hp_mult()` and `tools/simulate_balance.py:boss_base_hp_cushion()` are two implementations of one formula and must be edited together.
- **Result**: the early arm drops from 50 / 57 / 48 / 46% to 35 / 41 / 38 / 41% at levels 5 / 10 / 15 / 20, level with the 33–46% middle, while the late arm stays at 43–57% and remains the hardest stretch by design. All 20 boss levels still reach two stars and the campaign distribution holds at 12 / 86 / 1.
- **Headless proof**: comparing each boss level against its adjacent non-boss level cancels the per-level survivability multipliers and yields measured cushions of 1.756, 1.753, 1.583, 1.417, 1.249 and 1.250 at levels 5, 10, 15, 20, 25 and 99 — matching the formula. Level 50 reads 1.159 because the pre-existing "below level 50 and underpowered" 1.08 accessibility cushion drops out there; that is existing behavior, not a regression.
- **Verification**: full release candidate with 117 routed screenshots passes.

## Skill Ecology Follow-up · Barrier Card Tags (2026-07-29)

- **Change**: `skill_barrier.card_tags` goes from `['defense']` to `['defense', 'anti_swarm']`. A barrier is what holds a horde off the line, so the tag reads honestly, and it lifts the card's dealing weight from roughly `4 + 2 * bias` to `4 + 4 * bias`.
- **Measured**: barrier's campaign-wide offer rate rises from 9.8% to 12.8%, and across the 27 high-pressure levels — the 20 boss levels plus everything from level 90 — from 11.7% to 13.9%. Every other skill moves by 0.3 points or less, and gold rush is untouched at 8.2%.
- **Still short, with new evidence for the owner**: section 8's two quantitative targets remain unmet — barrier is 1.1 points below the 15% high-pressure goal, and pierce is still top-two in 98 of 99 levels against a target of no skill above 80. Measurement shows one lever clears both at once: dropping `physical` from `skill_pierce.card_tags` takes pierce from 98 top-two levels to 15, makes charge shot the new maximum at 60, lifts barrier to 14.3% in high-pressure levels, and leaves pierce at a healthy 23.5% average. The cost is that `skill_pierce` is the only skill in the table carrying `physical`, so removing it leaves physical-weapon players with no loadout-matched card at all while fire, ice, lightning and poison players each keep their elemental ammo skill — an asymmetry in the other direction. That change was outside the approved scope and was not made.

## Element Economy Follow-up · Poison Spread To Late Chapters (2026-07-29)

- **Change**: poison weakness expands from 6 levels to 10. Levels 053 and 059 in the flooded subway and levels 062 and 069 in the sandstorm refinery switch from fire or physical to `primary_weakness: "poison"`, with each level's `card_bias` element key moved to `poison: 1.35`. Both environments justify it — standing corrosive water and refinery fumes. Boss levels were deliberately left alone so no armor-break or immunity path changes.
- **Result**: distribution is now physical 36 / fire 29 / ice 12 / lightning 12 / poison 10, putting poison in the same band as ice and lightning. The venom launcher is no longer a chapter-4 specialist: it now counters levels across chapters 4, 6 and 7. All four converted levels keep their star rating, as expected — the simulator baseline carries no elemental counter.
- **Verification**: data validation, level pressure, balance profile, balance simulation, endgame balance, economy loop, card director and the full release candidate with 117 routed screenshots all pass.

## XP Economy · Repeat-Clear Decay (2026-07-29)

- **Design (owner's)**: a level pays full XP the first time it is cleared, half the second time, and a quarter from the third onward. The table lives in `data/economy.json` as `repeat_clear_xp_mult: [1.0, 0.5, 0.25]`, indexed by the number of prior clears and clamped to the last entry, so the curve can be retuned without touching code.
- **Save shape**: two additive dictionaries, `level_clear_counts` and `challenge_clear_counts`, track normal and challenge clears independently. A defeat never consumes a count. Because `_merge_defaults_recursive` fills missing keys, existing saves need no version bump or migration; both fields were added to the numeric shape validation.
- **Applied at the source of truth for display**: `battle.gd._finish()` multiplies the run's XP into `result.xp` before the payload leaves the battle, and also passes `xp_full` and `repeat_xp_mult`. The number the result screen shows is therefore exactly the number credited — SaveManager does not discount a second time. The multiplier is read before `apply_level_result` / `apply_challenge_result` increments the counter, so the current clear is priced by prior clears.
- **Told to the player**: when the multiplier is below one, the result screen's XP card title reads `经 验  ×50%` / `XP  x50%`, with the percentage derived from data. Without it a player would just see the same level quietly pay less the second time.
...
## Wave Formations Made Real, And A Swallowed First Wave (2026-07-29)

- **`wave_pattern` was a dead field**: all 99 levels carried one of `standard`, `rush`, `pincer`, `escort` or `siege` (24 / 20 / 19 / 18 / 18), but nothing in the runtime ever read it. Only `check_level_pressure.py` counted the variety and `rebalance_difficulty.py` wrote it. The five archetypes were labels a player could never feel — exactly the "reskinned HP and counts" concern in the todo.
- **Now wired to formation geometry**: `battle.gd._formation_lane()` sends `standard` down the authored lane, funnels `rush` entirely through the centre, alternates `pincer` between the flanks with the centre left open, puts `escort` support targets in the centre while the rest flank, and rotates `siege` through left / right / spread to fill the whole line. Bosses always keep their authored lane, and endless reads the pattern after the template swap so it cannot inherit the entry level's formation.
- **Deliberately geometry only**: counts, spawn intervals, HP and total spawn duration are untouched, so data validation, level pressure, balance profile, balance simulation, endgame balance, economy loop and card director all produce identical numbers, and the star distribution holds at 12 / 86 / 1. Composition signatures were already varied — 96 distinct ones across 99 levels — so the real gap was presentation, not data.
- **A pre-existing P0 found on the way**: the spawn loop was indented inside the wave-toast `else` branch, so `elite` and `treasure` levels showed their variant toast and queued *no enemies at all* for wave 1. Across 11 elite and 10 treasure levels that is 442 enemies that never appeared, while every balance model counted them. Variant should pick the toast, never gate spawning.
- **Permanent regressions**: `_verify_wave_formation_lanes` boots a representative level per archetype and asserts each formation's lane contract; `_verify_variant_wave_one_spawns` asserts every variant level authors enemies in wave 1.
- **Verification**: measured queues are `spread x7`, `center x10`, `left 6 + right 4`, `left 6 + right 6` and `left 4 + right 4 + spread 4`; the full release candidate with 117 routed screenshots passes.

## Per-Environment Audio Mixing (2026-07-29)

- **Two of the three clauses were already satisfied**: `get_sfx_concurrency_limit()` already caps concurrent `shot_`, `muzzle_` and `hit_` voices, and `get_sfx_priority()` already ranks boss entrances, breach alarms and threat warnings at 100, signature and character casts at 70, UI at 65, hits at 40 and gunfire at 30, with `_request_bgm_duck_for_sfx()` ducking music underneath. The missing piece was the per-environment half.
- **Data**: all 14 environments in `data/environments.json` gained an `audio_mix` block — `sfx_db`, `bgm_db`, `reverb_wet`, `reverb_room`, `reverb_damping` — voiced by each space. The void cathedral carries the longest tail at 0.24, the flooded subway 0.22, the glacier pass a bright 0.20, the toxic biolab a tight damped 0.09, and the sandstorm refinery is nearly dry at 0.05.
- **Bus**: the SFX bus gained an `AudioEffectReverb` defaulting to `wet = 0`, so nothing changes until an environment asks. UI sounds ride the UI bus and stay dry by construction.
- **Runtime**: `AudioManager.apply_environment_mix()` and `clear_environment_mix()` apply the decibel trims as *player volume offsets* rather than bus writes, so they cannot fight the settings volume sliders. Battle applies the mix in `setup()` and clears it in `_exit_tree()`, which pairs the two and keeps the menu, map and result screens from inheriting battlefield reverb.
- **Permanent regression**: `_verify_environment_audio_mix` asserts every environment declares a mix, that `reverb_wet` stays within 0–0.35, that environments genuinely differ from one another, that entering a battle applies the matching environment, and that leaving one returns the bus to dry.
- **Verification**: five sampled levels measured 0.14 / 0.20 / 0.22 / 0.24 / 0.12 wet with mix ids matching each level's environment and a clean reset after each exit. `check_audio_overlap.py` and the full release candidate with 117 routed screenshots pass.
- **Left to the owner**: the speaker-versus-headphone listening pass, which is already tracked as an owner acceptance item.

## Neon Tempest Local Commerce And Thunder Apocalypse Set (2026-07-29)

- **Complete local product loop**: the main menu now opens a bilingual Neon Arsenal. Fresh users see the `US$1.99` theme and `US$6.99` complete package; theme owners see only the `US$4.99` upgrade. Confirmation, local purchase, restore, reset, owned state, full-set equip and ordinary-gold upgrades are testable without Apple or a charge.
- **Receipt boundary**: Save v3 stores local demo product IDs only under `commerce.mock_receipts`. `entitlements.verified` remains a separate future StoreKit input. Revocation falls back illegal premium equipment/theme selections while preserving item levels for repeat testing.
- **Complete set**: `weapon_apocalypse_thunder`, `armor_apocalypse_conductor`, `chip_apocalypse_superconductive` and `pet_apocalypse_tempest` are data-driven and unlock together. The set implements chain retention, hit-count overload, defense-line counter discharge, pet multi-strike, and a non-recursive four-piece terminal pillar.
- **Measured promise**: the endgame audit now excludes paid items from the free baseline and models chip secondary stats, overload cadence, real-time terminal cooldown and pet skill. The complete set lands at `212,630` single-Boss DPS versus `137,219` for the strongest free Volt build: `1.550x`, inside the locked `1.52–1.58x` band.
- **Asset and crop acceptance**: four locked equipment masters produced seven independent catalog/equipment exports. The earlier isolated cannon-over-weaponless-hero composition was rejected after runtime review because the weapon read as floating and oversized. It is now superseded by the true-grip pass below. The full-size accepted lightning column is clamped to a crop-safe visual band and grounds back to edge targets with a short arc.
- **UI evidence**: Chinese/English fresh, confirmation, theme-upgrade and owned states; menu entry; premium collection detail; 1080×2046 safe-area store; full-set battle; and dense overload battle all passed runtime UI audit after fixing the English menu key and long weapon-name clipping.
- **Still not Apple commerce**: no `.storekit`, iOS plugin, verified transaction, Sandbox purchase, TestFlight purchase, refund callback or App Store Connect IAP record exists yet. The UI explicitly says local demo and no charge.

## Thunder Apocalypse True-Grip Hero Integration (2026-07-29)

- **Why the modular proof was not enough**: the catalog cannon was correct, but mounting the large isolated bitmap over a weaponless hero left the hands, shoulder and barrel mass visually disconnected. A premium weapon cannot pass final acceptance while reading as a floating sticker.
- **Four hero-specific models**: Vanguard, Blaze, Frost and Volt now each have a full-body Neon Tempest battle model carrying the exact triple-coil cannon. The rear hand owns the trigger grip, the forward hand supports the shroud, the rear power block braces into the shoulder, and the feet form a low recoil-ready base.
- **Static versus dynamic clothing**: silhouette, armor panels, cyan/violet conduit seams and prismatic material are baked into the model so the outfit reads even while paused. Rainbow travel, electric pulses, muzzle bloom, plasma-wing signature and recoil remain runtime layers, preserving motion quality and reduced-effects support.
- **Compact directional contract**: each hero ships one approved 380×520 master for left, center and right aim, 12 files total. The rig supplies breathing and an amplified heavy-cannon recoil accent instead of duplicating hundreds of nearly identical fused frames. The muzzle tables were recalibrated to the visible barrel faces for all three directions.
- **Evidence**: source and runtime contact sheets live under `assets/production/source_refs/generated/premium_neon_tempest_thunder_true_grip_2026_07_29/`; routed battle captures for all four center poses plus Vanguard left/right are under `tmp/neon_true_grip_runtime_2026_07_29/`. They confirm complete heads/feet/barrels, no secondary weapon sprite, and muzzle effects attached to the rendered barrel.
- **Regression**: M1 smoke now requires the premium true-grip path, 4 idle / 8×3 directional firing / 3 hurt rig beats, no floating weapon layer and separated left/right muzzle origins for every hero.

## TestFlight Build 43 · Neon Commerce And True-Grip Acceptance (2026-07-30)

- **Purpose**: deliver the complete Neon Arsenal local-demo purchase / restore / reset / equip loop, Thunder Apocalypse four-piece set, and the four-hero left / centre / right true-grip runtime for real-device owner acceptance.
- **Release shape**: plain `release`, without the temporary `neon_tempest_preview` feature and without auto-granting verified entitlements. Local mock receipts remain isolated from the future Apple-verified transaction input.
- **Gate hardening**: free-star economy totals, free-item price curves and free weapon DPS ranking now exclude premium rows and assert the `999999` sentinel remains quarantined. The store shade and confirmation modal now use texture-backed UiKit surfaces. Visual, animation-motion and HUD-overlap validators explicitly cover all 12 true-grip masters plus the runtime eight-beat heavy-cannon recoil curve.
- **Full release gate**: passed 117 routed Chinese / English screenshots, 1,056 battle sprite files, 689 combat-VFX frames, 83 semantic-VFX sequences / 710 frames, 108 attack sequences, 20 zombie attack identities, 908 HUD-overlap frames, plus headless quit, battle boot, save integrity, localization, theme manager and M1 smoke.
- **Exported runtime**: PCK contains 7,233 files and 3,484 imported resources. It is 619,240,720 bytes with SHA-256 `880f5c9beb4e1d85408e97ca80274eac96ed5bf34f2fd91f3cc9912d9ba334ea`; exported-PCK battle boot, save integrity and M1 smoke all passed.
- **Artifact and delivery**: IPA is 648,487,272 bytes with SHA-256 `c153e6ebbcad940bcc5dbc649c2560fe978b93974a8da193c5a276099e3fae02`. Delivery UUID `23b1fff1-7ae9-41e6-953e-3baa60a889df` finished `BUILD-STATUS: VALID`, `IMPORT-STATUS: VALID`, `APP_STORE_ELIGIBLE`, `IS-ON-APP-STORE-CONNECT: true`, `usesNonExemptEncryption=false`. The immutable record is `build/ios/release/build_43/release_manifest.json`.
- **Known boundary**: this is a TestFlight acceptance build, not an App Review candidate. It intentionally exposes local demo commerce and still has no StoreKit bridge, verified Apple transaction, Sandbox purchase, refund revocation or App Store Connect IAP metadata.

## TestFlight-Only Always-Visible Speed Control (2026-07-30)

- **Internal-test behavior**: exported builds carrying `testflight_speed_unlocked` show the battle-speed control from stage 1 and expose the complete 1X / 2X / 5X cycle. The selected speed remains a player preference; the build does not force combat to run at 5X.
- **Production behavior preserved**: without that feature, the existing progression remains stage 30 for the visible 1X / 2X control and stage 50 for 5X. Stored out-of-tier values are still clamped by the same SettingsManager path.
- **Delivery isolation**: `ship_testflight.sh` now injects `release,testflight_speed_unlocked` for its exported PCK, probes the feature, and restores the local iOS preset afterward. This keeps the App Store release switch explicit and reversible.
- **Regression shape**: source M1 smoke continues to assert the stage 29 / 30 / 50 production tiers; exported-PCK M1 smoke detects the feature and instead asserts visibility plus a complete 5X → 1X → 2X → 5X cycle at stage 1.
- **Verification**: plain-source Godot boot and M1 smoke passed; a temporary iOS PCK reported `testflight_speed_unlocked=true` through the export probe and passed the internal-test M1 smoke. Asset-pack, data, `res://` reference, all-level pressure and card-director checks passed, and the local iOS preset was restored to exactly `release`.
- **App Review rollback**: remove the TestFlight feature default from `tools/ship_testflight.sh`, export plain `release`, and rerun the production progression gate before selecting the review build.

## Neon Tempest Button Contrast Restraint (2026-07-30)

- **Owner finding**: the cyan/magenta punk treatment was liked, but the primary energy surface was bright enough to compete with its action label and make the copy feel softer than it is.
- **Surface-only correction**: UiKit now applies a restrained `0.80 / 0.82 / 0.86` modulation to Neon Tempest primary rasters and a very light `0.92 / 0.93 / 0.96` correction to secondary rasters. Native short/long models, aspect ratio, geometry and source textures are unchanged.
- **Copy remains dominant**: TextureButtons use `self_modulate`, not inherited `modulate`, so child labels remain full-brightness. Button-backed StyleBoxTextures receive the same surface-only modulation, while armored copy gains a 3px near-black outline.
- **Regression contract**: ThemeManager test asserts themed raster routing, exact surface modulation, full-bright child copy and the minimum outline thickness. Default-theme button assets and colors remain unchanged.
- **Visual and release proof**: manually reviewed Chinese / English menu captures and the English Neon Arsenal show the same punk identity with the action copy now dominant. The complete 43-route Neon Tempest screenshot matrix passed without overflow, stretching or runtime-audit findings; asset, data, reference, pressure, card-director, clean Godot boot and M1 smoke gates also passed.

## Global Theme And Per-Hero Outfit Routing (2026-07-30)

- **Information architecture**: Settings now opens a dedicated bilingual Theme & Appearance surface instead of cycling themes inside one button. Global theme owns menus, base, buttons, weapon colorway and ambient battle treatment; each hero can independently follow that theme or pin Default / Neon Tempest.
- **Persistence and entitlement safety**: Save v3 carries `cosmetics.character_outfits` for all four locked roster ids. Invalid or revoked premium overrides fall back to `follow_theme`; applying the complete look atomically selects Neon Tempest and restores every hero to following it. Cosmetic selection never alters combat stats.
- **Player paths**: Collection character detail gains a fourth exact-size Appearance action. The loadout hero portrait gains an Outfit shortcut while its bottom name strip retains the normal hero-selector action, so the two touch targets do not overlap. Battle and pause deliberately have no live appearance switch.
- **Complete purchase ending**: the local purchase-complete panel offers Apply Full Look or Dress Heroes. The first equips any owned arsenal and applies the complete visual set; the second opens the per-hero workflow without losing the store return path.
- **Runtime resolution**: collection portraits, battle hero sprite, directional firing sequence, iridescence material, firing aura and signature/cast effects all resolve through the hero's effective theme. Shared UI, base and weapon treatment continue to resolve through the global theme.
- **Bilingual and visual evidence**: six new deterministic screens cover global appearance, per-hero appearance and post-purchase choice in Chinese and English. The localization gate now passes 986 runtime Chinese sources (93 content ids, 391 exact messages and 532 reusable terms). Manual review also covered the four-button character detail, loadout shortcut and a global-default battle with an explicitly Neon Vanguard.
- **Regression**: ThemeManager tests pin mixed global/per-hero states, frame/shader routing and revocation fallback; M1 smoke covers local purchase, apply-full, four follow states and revocation. Asset pack (9,647 files), 99-level data, 383 `res://` references, pressure, card director, localization, clean headless boot, save integrity, ThemeManager and M1 smoke all pass.
- **Remaining boundary**: this is still local-demo commerce. StoreKit 2, verified transactions, refund revocation, Sandbox/TestFlight purchase testing and App Store Connect IAP records remain intentionally unimplemented.

## Infernal Dominion / Inferno Apocalypse Phase 2 Inventory (2026-07-31)

- **Owner selection**: the second premium series is Infernal Dominion plus Inferno Apocalypse. Polar remained deferred until the second series passed its full regression.
- **Frozen product identity**: theme `infernal_dominion`, entitlements `ent_theme_infernal_dominion` / `ent_arsenal_inferno`, set `set_apocalypse_inferno`, and the three already-reserved theme / complete / upgrade product ids. The set is the Infernal Plasma Projector, Molten Regenerator Armor, Stellar Combustion Core and Ember Phoenix.
- **Complete content boundary**: four identity-preserving hero outfits, 4 portraits + 44 runtime hero frames, eight free-weapon colorways, current cross-theme premium-weapon combinations, 72 native-size button rasters, full structural UI/base/HUD/logo treatment, and the complete ignition-to-kill Inferno VFX/audio/haptic chain.
- **Critical architecture finding**: the first product proved one complete series but left PurchaseManager, Store, the Neon material/VFX hooks, the Thunder true-grip root, recoil/muzzle tables and several validators hard-coded to one series. Phase 2 therefore starts with a no-visual-change data-driven architecture gate; copying another set of `if infernal` branches is explicitly rejected.
- **Combat boundary**: existing Burn remains the shared damage-over-time status; Inferno adds a premium-only combustion stack for readable full-stack detonation. Death propagation carries a one-generation marker and decay, armor retaliation has an internal cooldown, and every weapon/armor/pet/set damage source is included inside the locked weighted `1.52x–1.58x` total-output promise.
- **Execution contract**: Step 0 architecture, Step 1 owner-reviewed visual prototype, Step 2 theme runtime, Steps 3–4 mechanics and premium presentation, Step 5 local multi-series commerce, Step 6 full regression, Step 7 TestFlight, and a separately gated Step 8 for real Apple commerce. The authoritative table and acceptance checklist is `design/25_infernal_dominion_inferno_phase2_inventory.md`.

## Infernal Dominion Runtime And Inferno Mechanics (2026-08-01)

- **Step 2 runtime**: the accepted Infernal Dominion direction now resolves through `themes.json` into four frameless portraits, 44 identity-preserving battle frames, 72 independently rendered native-size buttons, eight free-weapon icon / handheld / turret colorways, a forged title lockup, an independent heat-flow shader and a four-frame mechanical fire-wing signature. The deterministic manifest records 168 runtime files.
- **Theme separation**: global theme and per-hero outfit choices remain independent. Free weapons receive the selected theme colorway, while Apocalypse weapons preserve their own model identity; entitlement loss still falls back to valid default selections without deleting levels.
- **Rear-view correction**: the first true-grip generation read as a front-facing shop pose. It remains display-only. Battle now uses four newly authored rear-view sheets and exports twelve 380×520 left / center / right fused hero-weapon masters. All three muzzle anchors point upward from the visible barrel, the back-mounted copper fire wings remain upright, and no standalone weapon sticker is layered over the character.
- **Step 3 mechanics**: the Infernal Plasma Projector implements enhanced burn, per-target five-stack combustion, a 1.4 s detonation gate, bounded candidate counts and one-generation spread. Molten Regenerator Armor uses a three-hit charge and nine-second retaliation cooldown with separate restore and damage paths. Stellar Combustion Core scales fire, burn and combustion independently. Ember Phoenix follows manual lock when present and performs a bounded, non-persistent multi-target flyby.
- **Set and reporting**: the two-piece bonus accelerates combustion / burn; the four-piece death propagation is generation-limited and decayed. Damage reporting separates weapon, burn, combustion, armor counter, Phoenix and set propagation without breaking existing four-argument enemy damage calls.
- **Measured output**: full-build audit versus the strongest free fire / plasma baseline gives Boss `1.475x`, dense horde `1.659x`, mixed elite `1.589x`, and weighted 40 / 40 / 20 output `1.571x`, inside the locked `1.52x–1.58x` promise.
- **Current boundary**: this section records the Step 2–3 baseline. The subsequent Step 4–5 completion is recorded below; Apple StoreKit remains outside the local-demo boundary.

## Inferno Step 4 Presentation And Step 5 Local Commerce (2026-08-01)

- **Authored combat language**: one Owner-directed image-generation master produced six deterministic runtime sequences: 6-frame body-attached burn, 8-frame combustion, 7-frame one-generation death spread, 8-frame mechanical Phoenix, 8-frame upward base counter and 8-frame max-level awakening. Five matching SFX cover ignition, combustion, Phoenix, counter and awakening.
- **Semantic runtime binding**: combustion and spread stay on the true gameplay event. Edge targets reduce the authored sprite scale instead of moving the mechanic. The Phoenix source heading is explicitly `-45°` and rotates onto the real pet-to-target vector, preventing another visually attractive but backward effect. Awakening renders behind the rear-view hero; counter faces from the fixed base into the lane.
- **Feedback restraint**: reduced-effects mode preserves fire direction, impact, persistent status and danger radius while lowering density. Burst haptics are throttled, counter / Phoenix are single events, and awakening has a seven-second presentation cooldown; no continuous vibration or full-screen flash was introduced.
- **Multi-series store**: Infernal theme, complete and theme-owner-upgrade rows are visible in the local demo store. The store groups offers and owned states per series, supports a series-only reset, keeps global reset, restore, full-set equip and ordinary-gold upgrades, and exposes the same post-purchase Apply Full Look / Dress Heroes choice as Neon.
- **Isolation proof**: smoke coverage purchases Neon theme + Thunder upgrade, purchases Inferno complete, equips both sets, mixes Neon / Infernal hero outfits, restores three receipts, resets Inferno only while preserving Thunder / Neon, then globally resets to valid free fallbacks. Invalid revoked Infernal outfits reset without deleting still-owned Neon outfits or item levels.
- **Visual proof and fix loop**: routed 1080×2340 captures cover Chinese / English fresh offers, the `US$4.99` theme-owner upgrade, complete-owned equipment, purchase confirmation, body burn, central / edge / Boss combustion, spread, Phoenix, base counter, awakening and reduced effects. The pass found and fixed the shared long-screen confirmation container escaping its safe area and a translucent modal backing that let store copy interfere with purchase text.
- **Automated proof**: Inferno-only screen regression, combat VFX semantics, attack-motion validation, asset / data / `res://` / pressure / card-director gates, clean Godot boot and M1 smoke cover the finished work. Apple product records, StoreKit 2 verified transactions, Sandbox, refunds and TestFlight remain Step 8 / Step 7 work and were not simulated as complete.

## Infernal / Neon Step 6 Full Regression And Grip Correction (2026-08-01)

- **Owner-found defect**: the theme/free-weapon branch loaded a weaponless themed hero and mounted the complete handheld PNG at foreground z-order. It passed crop and blank-screen checks but failed the physical model: the receiver covered both hands, the stock did not brace into the shoulder and the weapon read as a floating sticker.
- **Runtime correction**: battle resolution order is now premium `true_grip` → free `character_weapon_combos` → weaponless theme fallback. Neon / Infernal character materials, rear firing signatures and projectile palettes decorate the accepted fused silhouette; no standalone `CharacterWeapon` node is created when a selectable combo exists. The back-mounted Neon discharge and Infernal mechanical fire wings remain below the fused actor.
- **Permanent acceptance rule**: every routed shooting capture aborts if `character_weapon_combo_active` is false or a standalone weapon sprite exists. `check_gameplay_polish.py` also locks the resolver order, so a visually nonblank screenshot can no longer hide a false grip.
- **Visual matrix**: default 137 routes, Neon 43 routes, Infernal 43 routes and 16 cross-theme premium combinations passed real Godot rendering. The two theme matrices include all four heroes × eight free weapons; M1 smoke and attack-motion checks cover all four heroes × ten weapons and left / center / right muzzle motion.
- **Numerical and package proof**: Inferno measures Boss `1.475x`, dense horde `1.659x`, mixed elite `1.589x`, weighted `1.571x`. The second-series runtime set is 224 raw files / `26.43 MiB`, under the 60 MiB budget; 65.9 MiB of source-only material and 216 unused VFX tails remain excluded from the iOS preset.
- **Full gate**: `tools/check_release_candidate.py` passes asset pack (10,221 files), OFL font license, 99-level data, 386 `res://` references, pressure / balance / economy / card simulation, 1068 battle sprites, 120 attack sequences, 89 VFX semantic sequences, 54 status frames, audio, bilingual localization, clean Godot boot, battle boot, save integrity, ThemeManager, M1 smoke and all 239 routed screenshots.
- **Honest boundary**: Step 6 desktop automation is complete. TestFlight upload, real-device sustained FPS / thermal / memory review and Apple commerce remain Step 7 / Step 8; this pass did not commit, push, export or upload.

## Polar Aurora / Absolute Zero Phase 3 And Final Three-Series Catalog (2026-08-01)

- **Phase-3 catalog at that milestone**: premium scope was then exactly three series: Neon Tempest / Thunder, Infernal Dominion / Inferno and Polar Aurora / Absolute Zero. The canceled Obsidian Forge candidate had no runtime product or entitlement; the later Owner-approved Gilded Eclipse series is a distinct fourth direction documented below.
- **Production masters**: four identity-preserving Polar outfits, the selected cryogenic accelerator, Permafrost armor, Entropy chip, Aurora Wisp, crystalline UI/world board, four rear-view hero battle sheets, six-event VFX master and rear-mounted aurora-wing signature live under `assets/production/source_refs/generated/premium_polar_aurora_absolute_zero_phase3_2026_08_01/` with their complete prompt/correction log.
- **Deterministic runtime**: `build_polar_runtime_assets.py` produces 168 theme/equipment files; `build_absolute_zero_vfx_assets.py` produces 52 VFX frames plus five SFX. Four portraits, 44 hero frames, 72 native buttons, eight three-form free-weapon coatings, seven premium item assets, twelve rear true-grip directions, four rear firing-wing frames and a title preserve the same contracts proven by the first two series.
- **Grip and layering**: all four premium heroes face up-lane from the rear, hold the accelerator with both hands and keep heads, boots and barrel tips inside 380×520. The aurora-ice signature is z-ordered behind the fused hero/weapon actor; no front-mounted wing or independent floating weapon is permitted.
- **Combat identity**: Absolute Zero builds target-local Brittle, triggers bounded Shatter on the real impact, sends one independently cooled crystal wave toward the densest group, gives the Aurora Wisp a finite field and gives Permafrost armor a three-hit / nine-second upward counter with small base repair. Bosses gain one extra Brittle hit instead of being hard-frozen. Crystal wave damage is explicitly generation-limited and cannot recurse.
- **Measured output**: after correcting an overpowered first draft, the all-skills-maxed audit measures Boss `1.342x`, dense `1.752x`, mixed `1.550x`, and weighted output `1.547x` versus the strongest free ice build, inside the locked `1.52x–1.58x` promise.
- **Commerce and localization**: three local-demo SKUs implement theme, complete and theme-owner upgrade offers in Chinese and English. Purchase, equip, Apply Full Look, per-hero dressing, three-theme mixing, restore, series-only revocation and global revocation remain separated from Apple verified entitlements.
- **Regression expansion**: ThemeManager and M1 smoke now cover three themes and three product series. Visual routing adds 43 Polar theme screens, 36 cross-theme premium combinations and dedicated Chinese/English store plus Brittle, central/edge/Boss Shatter, crystal wave, field, counter and awakening captures. The direction contract binds wave rotation to its real source-target vector and keeps the base counter upward.
- **False-positive closure**: the capture harness previously allowed Main to build the menu from the developer's persisted theme before applying its screenshot override. Every route now rebuilds after entitlement/theme refresh. This exposed and closed a real visual mismatch: Polar menu title/buttons now resolve from Polar assets and its data-driven semantic accent colors the subtitle/store entry icy cyan instead of inheriting Infernal orange.
- **Final gate**: the complete Release Candidate passes 10,733 assets, 391 `res://` references, all static/gameplay/audio/localization/theme/save gates and `324` real Godot screenshots: base/bilingual/tall `148`, Neon `43`, Infernal `43`, Polar `43`, cross-theme premium `36`, and Absolute Zero `11`.
- **Remaining boundary**: source/local-demo implementation and desktop validation are complete. Owner real-device review and Apple StoreKit / verified transaction work remain separate; this task does not commit, push, export or upload.

## All-theme Logo And 1.5× Hero Presentation Pass (2026-08-01)

- **Logo sizing contract**: the menu no longer scales the full source canvas. It derives a tight alpha-visible region for default / Neon assets and consumes optional data-authored presentation regions for Infernal / Polar design boards, then fits that aspect ratio inside a 1040×500 title envelope. This makes every `ZOMBIE FIRE` lockup materially larger without geometric stretching.
- **Source-board cleanup**: Infernal uses a small canvas-item dark-key shader plus an authored crop to remove its opaque concept-board ground and separator; Polar uses an authored crop that contains only the logo / crest composition. These presentation rules live in `themes.json` rather than theme-specific menu branches.
- **Character presentation**: the entire in-battle character rig is `1.50×`, positioned from the actual alpha-visible boot line so every animation remains foot-anchored. Fused hero / weapon frames, held-weapon fallback, muzzle sockets and body-attached presentation all share the same transformed coordinate space.
- **Effect continuity**: global muzzle flash, cone, heat haze, fork lines, bubbles and short sparks use a restrained `1.25×` presentation multiplier; theme rear firing signatures, shoulder anchors, aura buses, cast / awakening rings and premium true-grip effects follow the enlarged rig. Gameplay collision shapes, target selection, projectile damage, ranges and area-of-effect radii are intentionally unchanged.
- **HUD reflow**: the pet anchor moved to `x=800`; skill chips moved to a compact six-column block above the enlarged hero. Static audit across 932 character / weapon frames reports a 72.6 px minimum skill gap and 7.2 px minimum bottom-resource gap at the locked maximum growth scale.
- **Visual proof**: all four menu themes and four representative hero / weapon / theme combinations were rendered at 1080×1920 and manually reviewed for crop, alignment, weapon grip, rear-effect layering and bottom HUD integrity. Automated smoke assertions now lock the title atlas, 1.50× rig scale, visible-foot lift, transformed muzzle sockets, six-column skill area and pet anchor.

## Gilded Eclipse / Golden Law Phase 4 Step 0 Proposal (2026-08-01)

- **Owner direction**: a new fourth series is being considered around flowing black-gold presentation, highest-tier visual spectacle, modest 20%–30% entry advantage and a highly back-loaded 2.00× maximum attribute target. This is not the canceled Obsidian Forge candidate.
- **Product identity**: the working names are 鎏金永夜 / `Gilded Eclipse` and 终焉·黄金律 / `Golden Law Apocalypse`, with a physical / defense identity centered on the Sovereign Verdict Cannon, Eternal Night Aegis, Golden Law Core and Aureate Skyfalcon.
- **Progression proposal**: Infernal becomes visible after campaign 30, Neon after 50, Polar after 80, and Gilded Eclipse only after clearing level 99 plus reaching Lv.40 on any hero. Verified ownership always overrides the visibility gate.
- **Balance proposal**: Level 1 actual value remains `1.20x–1.30x`; maximum weapon / armor slot attributes target `2.00x`. The recommended real all-skills full-build attack and defense band is `1.90x–2.05x`, with chip / pet / set mechanics sharing one budget instead of multiplying four separate 2× bonuses.
- **Visual prototypes**: the initial two built-in image-generation boards establish identity-preserving outfits and dark UI / logo / base language, plus an independent heavy cannon / armor / chip / falcon and six-event combat grammar. Gold behaves as silk / liquid ink behind the hero and along the real upward shot direction; it is not lava, lightning, ice or solid yellow fill.
- **Repository truth**: the proposal, exact Owner decisions, prompt log and source boards are tracked in `design/27_gilded_eclipse_golden_law_phase4_proposal.md` and the production asset index. No data row, runtime asset, entitlement, purchase route, balance value or StoreKit product was added in Step 0.

## Gilded Eclipse / Golden Law Phase 4 Step 1A Detailed Prototype Pack (2026-08-01)

- **Eleven-board contract**: the two initial direction boards now have nine production-detail companions covering full front / rear hero tailoring, eight mechanically distinct free-weapon coatings, the four Golden Law items, enlarged title and native-shape UI / HUD / base pieces, true-grip / recoil, ten combat events, a 9:16 gameplay scale proof and a four-screen unlock / preview / equip flow.
- **Identity and material closure**: Vanguard remains broad and mature, Blaze lean and young, Frost mature and aloof, and Arc agile and young. All content shares a 74 / 18 / 8 mirror-black / warm-gold / white-gold hierarchy; flowing gold reads as silk or liquid light rather than Infernal fire, Neon lightning, Polar ice or solid-yellow trim.
- **Owner direction correction**: a center-only rear firing proof was rejected as incomplete. `golden_law_true_grip_three_direction_master_v2.png` now covers all four heroes in up-left, center and up-right aim. Hands, elbows, shoulders, torso weight, fused cannon, recoil, real muzzle, projectile and rear-only signature move together; rotating a standalone gun is explicitly forbidden.
- **Gameplay readability proof**: the 9:16 composition uses the existing 1.50x character presentation, retains a readable lane / zombie pack / hit point / base / HUD hierarchy and keeps the gold mantle behind the hero. The ten-event board locks charge, shot, impact, status, execution, falcon, counter, active sequence, awakening and reduced-effects semantics to real bottom-to-top directions.
- **Production boundary**: these boards are an acceptance contract, not runtime atlases. No fourth theme row, entitlement, product, localization data, combat value, save schema or StoreKit bridge was added. Step 1B must author transparent assets, twelve fused three-direction character-weapon masters plus attack frames, native controls and real Godot bilingual screenshots.
- **Verification**: asset-pack validation passes with 10,745 files; data validation passes 99 levels / 20 zombies / 8 bosses / 16 skills; 392 `res://` references resolve; level pressure, card director, clean Godot headless boot and M1 smoke all pass. The macOS headless runs emit the existing system CA-certificate warning but exit successfully.

## Gilded Eclipse / Golden Law Phase 4 Local App Integration (2026-08-01)

- **Catalog and gate**: the fourth local-demo series uses the frozen `US$1.99 / US$6.99 / US$4.99` price ladder and appears only after `level_099` is cleared plus any hero reaches Lv.40. Owned non-consumables remain visible through progress rollback; no global debug entitlement bypass was introduced.
- **Runtime art**: the accepted eleven-board contract now resolves through 168 deterministic runtime files. Four identities, 44 rear hero frames, 72 exact-size controls, eight free-weapon coatings, four equipment identities, twelve four-hero / three-direction fused grips, a four-frame rear gold signature and a clean title are indexed and reproducible.
- **Combat**: Golden Law adds target-local Judgment / Verdict, bounded four-target Golden Decree, manual-lock-aware Skyfalcon mark, three-hit Eternal Night counter/repair and rear-layer max-level awakening. Generation is one, feedback is throttled, and generic armor-pierce copy is suppressed for named Golden events to prevent multi-target text collisions.
- **Measured growth**: opening weapon throughput is `1.205x` Railgun. Max Boss / dense / mixed ratios are `1.995x / 2.090x / 2.043x`; weighted output is `2.043x` inside the frozen `1.90x–2.05x` band. Raw max armor HP is `1.888x` Reactive Armor before the extra breach shield and repair.
- **Regression**: ThemeManager covers all four themes and all twelve Golden Law grip files. M1 smoke covers 30 / 50 / 80 / 99+40 visibility, every fourth-series offer, purchase/equip/restore/revoke, and four-series isolation. The Gilded-only real Godot matrix covers 26 routes including bilingual store, four heroes × three directions and six Golden combat events.
- **Visual fix loop**: automated layout audits passed, then manual inspection caught a concept-board crop in the menu wordmark and overlapping generic penetration labels during multi-target Decree. Both were corrected and recaptured at 1080×1920 / 1080×2340.
- **Honest boundary**: this is a complete source/local-demo integration. Apple StoreKit, verified transaction ingestion, refunds, Sandbox, TestFlight real purchase and App Store Connect IAP records remain unimplemented.

## Golden Decree Deep-Rendered Runtime Replacement (2026-08-01)

- **Owner finding**: the live four-target Decree still exposed procedural circles and ruler-straight columns. It communicated target count but failed the accepted black-gold quality bar.
- **New authored layers**: built-in image generation produced an isolated sovereign seal gathering layer and a separate molten judgment impact layer from the accepted Golden Law storyboard and the Owner's runtime screenshot. Chroma sources, transparent masters and final prompts are tracked under the Phase-4 source directory.
- **Animation language**: the seven-frame Decree now gathers a forged mirror-black / liquid-gold crest, drives a white-hot verdict stream into the real target and releases a layered molten pressure wave. The eight-frame single-target Verdict uses the dedicated impact master instead of a procedural starburst.
- **Runtime invariants**: damage, selected targets, target order, independent cooldown, one-generation non-recursion and staggered multi-target timing are unchanged. Source art is fitted into a real atlas guard band after glow and gold-spill grading.
- **Verification**: the rebuilt sequences pass the combat semantic gate across 101 sequences and 845 non-empty frames. Six Golden Law showcase routes pass real macOS Metal capture; the dedicated Decree and Verdict screenshots were manually checked for target readability, HUD clearance, multi-target overlap and alpha cropping. Asset / data / reference / pressure / card-director / clean Godot boot and M1 smoke gates also pass.

## App Store Runtime Placeholder Closure And Menu Logo Expansion (2026-08-01)

- **Menu brand scale**: the shared title envelope is now `1040×560`. Default / Neon titles use their alpha-visible raster bounds, while Infernal and Polar rebuild only the actual wordmark from the accepted source board. Gilded Eclipse keeps its clean standalone title. All five preserve aspect ratio and were recaptured at 1080×1920.
- **Source-board defects closed**: manual inspection of the first enlarged pass found Infernal panel borders and Polar divider / lower-crest fragments. The deterministic Infernal / Polar builders now crop the wordmark itself, preventing the artifacts from returning on regeneration.
- **Complete VFX audit**: `build_vfx_placeholder_audit.py` generated six labeled sheets spanning all 101 referenced sequence IDs. Four remaining Golden Law diagram-like effects plus generic Enemy Enrage and Level Up were rebuilt from six independent image-generation masters; Judgment / Falcon / Counter / Awakening no longer call any procedural geometry builder.
- **Runtime semantics**: Skyfalcon remains top-to-bottom, Eternal Night counter remains bottom-to-top and Sovereign Awakening remains behind the 1.50x fused hero rig. Enrage now renders behind its zombie at peak instead of hiding the actor. Level Up is one coherent upward gold-cyan column rather than disconnected bursts.
- **Permanent regression**: the semantic contract now pins the exact rendered master for all six replacements. The former Level Up hard-edge exception was deleted; every referenced frame must meet the normal alpha guard band. The current gate passes `101` sequences, `850` non-empty frames, six movement contracts and eleven directional projectiles.
- **Scope truth**: dynamic textured trails, target-lock markers and area boundaries remain because their geometry conveys the real travel vector or gameplay area. No static skill, status, hit, character, weapon, enemy, button, panel, logo or environment is allowed to use those constructs as a substitute for final raster art.
- **Collection cleanup**: the hero passive row no longer exposes the hard-coded diamond glyph. It uses the hero's finished elemental badge, with a finished raster talent icon as the invalid-data fallback.

## App Store UI Raster Placeholder Closure And Neon Button Restraint (2026-08-01)

- **Full UI inventory**: `build_ui_placeholder_audit.py` renders five labeled sheets for all 102 unique base runtime UI rasters, excluding only redundant native-size button variants. This exposed a legacy flat-tile icon family and five concept-sheet crop fragments that screen-level overflow checks could not classify.
- **Thirty-two final replacements**: fourteen core HUD icons, thirteen tactical/card/priority icons and five native-ratio HUD surfaces now come from three dedicated deep-rendered extraction masters. Runtime filenames and exact dimensions are unchanged; the regenerated gold icon is a physical minted token rather than a circle drawn inside a gray square.
- **Surface integrity**: level card, combo panel, pill, plate and damage badge no longer contain unrelated neighbouring art. Their separately modelled corners stay at native scale while only the quiet center span extends to the required width.
- **Permanent gate**: `check_app_store_ui_polish.py` pins all three rendered masters and all 32 outputs, requiring exact size, alpha safety, adequate raster color depth and clean chroma edges. The builder, prompt log, manifest and before/after audit sheets keep the pass reproducible.
- **Neon-only restraint**: the existing six native button families remain intact. A deterministic post-grade reduces border luminance/saturation and partial-alpha bloom while leaving child copy, geometry, size routing, character art, weapon art and combat effects untouched.
- **Release verification**: the complete Release Candidate gate passes after the button-only grade, including M1 smoke, `174` base routes, `43` Neon routes, `43` Infernal routes, `43` Polar routes, `64` cross-theme / hero routes and `11` Absolute Zero routes. Manual review of the Neon menu, settings, character, loadout and battle captures confirms that copy now leads the hierarchy without losing the cyan / magenta punk identity.

## App Store Final Bilingual Screenshot Regression (2026-08-02)

- **Final coverage**: the release matrix now contains `954` unique real-Godot captures. It covers five theme states, all four heroes, all twelve weapons in center fire, representative left/right fire, all twenty active-skill/theme combinations, every collection category and detail family, all sixteen skill cast/hint/detail states, twenty zombies, eight bosses, hit/death/status/projectile/pet VFX, ten map chapters, all major interfaces and the full eight-product local-demo catalog in Chinese and English.
- **Copy and layout closure**: this pass corrected privacy/support copy and heights, the `Not Equipped Lv0` artifact, weapon-special spacing, chapter-description edge crowding, long English chapter-title/badge collisions and the raw `element` enum in skill details. Store captures now use an isolated fully unlocked save so every product card is genuinely rendered instead of documenting an empty gated catalog.
- **Visual closure**: Neon button surfaces were restrained without changing their native geometry or text; combat/VFX routes now capture effects after their visible warm-up on clean deterministic stages. Manual review covered all hero/weapon/theme matrices plus full-resolution high-risk screens for maps, store, appearance, settings, skill details, bosses and combat effects.
- **Safe-area fix loop**: the first final run caught the settings stack extending two pixels above and below a simulated compact safe rect. The root keeps a two-pixel settings guard and compact settings margins are `12px`, reducing the minimum stack below the safe-area ceiling. The failing route and the entire affected 318-screen shard were recaptured successfully.
- **Output integrity**: the three final manifests each report `318` screens and zero errors; across them there are `954` unique labels and no missing files. Fifty-five classified contact sheets were generated alongside the screenshots in the external Codex visualization directory. These review artifacts intentionally remain outside the repository and were not committed.
- **Verification**: asset pack (`11,300` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `399` `res://` references, pressure, card director, release strings, bilingual localization (`1,073` runtime Chinese sources), all `32` deep-rendered UI replacements, clean Godot boot and M1 smoke pass. The macOS headless runs emit the known system CA-certificate warning but exit successfully.

## TestFlight Build 44 Internal Speed-Control Delivery (2026-08-02)

- **Internal-test behavior**: Build `44` was exported with the temporary `testflight_speed_unlocked` feature. Its packaged feature probe returned `true`, and packaged M1 smoke verified that stage 1 exposes the complete 1X / 2X / 5X cycle without forcing a non-default speed.
- **Production isolation**: the ordinary iOS preset was restored to exactly `release` after delivery. Builds without the feature retain the progression contract: 2X unlocks at stage 30 and 5X unlocks at stage 50.
- **Package proof**: version `1.0.0 (44)` contains `8,577` PCK files / `4,144` imported resources. The PCK is `692,189,736` bytes with SHA-256 `9711dbf68b9c1063414d1da44708bf7b14bcca89d008cdcece7d967954a418cc`; the IPA is `721,074,655` bytes with SHA-256 `73b1f8c49134a577ed85a783fabf8dc417c292b5ad067b3a7a6fda0dffa88606`.
- **Apple delivery**: delivery `a47bbf50-bb0e-49a3-a572-eeb388fae4c8` reached `VALID`, `APP_STORE_ELIGIBLE`, `IMPORT-STATUS: VALID`, and `IS-ON-APP-STORE-CONNECT: true`. The audited manifest is `build/ios/release/build_44/release_manifest.json`.
- **Gate result**: the full non-windowed Release Candidate suite, exported-PCK battle boot / save integrity / M1 smoke, Xcode archive, code signing, IPA inspection and Apple server validation all passed. The release manifest truthfully records that this package was built from the current tracked dirty working tree.

## Global Boss HUD Title / HP-Rail Separation (2026-08-02)

- **Root cause**: the shared Boss title retained a `28 px` control height after the global readability pass scaled its authored 24 px font to 36 px and its outline to 6 px. Its old rail began only 34 px below the title origin, so glyph descenders and outline were clipped and visually covered by the HP rail in every theme and language.
- **Shared fix**: the Boss HUD now owns a `56 px` vertically centred identity band followed by a hard `10 px` clear gap and the existing `22 px` HP rail. The rail remains at the same absolute battlefield coordinate, while the container grows upward; wave progress, battlefield space and Boss mechanics do not move.
- **Bilingual safety**: the complete Boss name / weakness / percentage remains one line. The common text fitter uses the globally scaled 36 px target and may shrink long English strings to a 28 px floor, preventing wrap, truncation or rail overlap without changing Chinese sizing.
- **Permanent regression**: the static HUD overlap audit now checks wave-to-title and title-to-rail separation plus scaled glyph containment. M1 smoke validates the live label, rail and fill rectangles, vertical alignment, 10 px gap and container bounds.
- **Verification**: asset pack (`11,300` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `399` `res://` references, level pressure, card director, HUD overlap, isolated clean Godot boot and M1 smoke pass. The only headless output is the known macOS system CA-certificate warning; both Godot commands exit successfully.

## Three-Mode Aiming Contract And Guaranteed Automatic Multi-Lane Hit (2026-08-02)

- **Audit finding**: lock and hold-direction priority were already correctly routed, but automatic multi-shot only tested whether an enemy direction fell somewhere inside the fan span. With an even lane count—or a target between two fixed lanes—the entire fan could still pass around the zombie.
- **Automatic correction**: the fan keeps its enemy-centroid composition, then resolves the turret's requested direction back to the nearest still-live battlefield enemy. It rotates the complete fixed-spacing fan until the nearest lane crosses that target centre. This guarantees one real trajectory for every 2–5-lane shot without granting independent per-projectile targeting.
- **Manual target contract**: iPhone uses a deliberate double tap on a zombie, while macOS retains right click. The nearest enemy inside the `180 px` selection radius becomes the persistent `TargetingManager` lock; the same gesture on empty space clears it. Locked targets override automatic scoring and feed preferred homing targets.
- **Direction contract**: a mouse/finger hold becomes manual aim after `0.30 s`; movement updates the direction continuously. Manual direction outranks both automatic selection and an existing target lock while held, retains a `0.18 s` release grace for visual follow-through, then returns to automatic selection.
- **Permanent proof**: M1 smoke now checks touch double-tap emission, live nearest-target lock and empty-space clear, automatic-fire persistence, long-press/drag/release priority, one exact target-crossing lane for every supported lane count, and unchanged adjacent-lane spacing.
- **Verification**: asset pack (`11,300` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `399` `res://` references, 99-level pressure, 99-level card-director simulation, isolated Godot boot and M1 smoke pass. The headless macOS CA-certificate warning remains non-fatal.

## In-Battle Skill Detail Mobile Typography Upgrade (2026-08-02)

- **Owner finding**: the five-row All Levels block and the long skill description were structurally clean but visually too small on a physical phone, despite unused breathing room inside the detail card.
- **Readable hierarchy**: all-level rows move from effective `20 px` to `23 px` with `7 px` line spacing. The long description moves from `23 px` to `26 px`; the tag line moves from `22 px` to `23 px` so the lower information group reads as one deliberate mobile hierarchy.
- **Layout rebalance**: the levels region grows to `226 px`; the description starts twelve pixels earlier and grows to `164 px`; tags and Close move down together. The resulting gaps are `10 px` levels-to-description, `12 px` description-to-tags, `8 px` tags-to-button and `24 px` bottom safety.
- **Bilingual resilience**: Chinese and English both retain the new preferred sizes for the representative five-level Split Shot detail. Runtime text fitting is applied only after localized copy is installed; future longer strings may shrink no lower than their former release size rather than clip or collide.
- **Permanent proof**: M1 smoke switches the live detail card through Chinese and English, asserts the three preferred sizes, measures each rendered multiline string against its label height and retains the existing non-overlap checks through the Close button.
- **Verification**: asset pack (`11,300` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `399` `res://` references, level pressure, card director, bilingual localization smoke, isolated Godot boot and M1 smoke pass. The headless macOS CA-certificate warning remains non-fatal.

## Gilded Eclipse Portrait And Appearance-Action State Closure (2026-08-02)

- **Actual crop source**: Godot was correctly fitting the entire 720×980 portrait canvas. The failure was upstream: the accepted four-column concept board had been mechanically sliced into alpha strips whose real character widths were only `210–233 px`, with outer shoulders, arms and coat edges already absent.
- **Final portraits**: all four black-gold front-view cutouts were identity-preserving horizontal outpaints, chroma-keyed to alpha and fitted into the existing 720×980 runtime contract at `300–392 px` of real occupied width. The selector, collection and showcase keep their existing paths; rear-view battle animation is intentionally unchanged.
- **Three clear actions**: global-theme and per-hero outfit cards share one visual state contract. Applied / Worn is disabled gray, Apply / Wear is the bright primary affordance, and an unowned item is a quieter secondary surface with `未拥有 / Not Owned` status and a full-size `购买 / BUY` action that opens the arsenal.
- **Regression contract**: the deterministic Gilded builder prefers the reviewed V2 alpha masters, preventing a future rebuild from re-slicing the concept sheet. M1 smoke checks both languages, action enablement / semantic metadata / surface hierarchy and the alpha-used width of all four portraits.

## Theme-Wide Character Collection Portrait Alignment (2026-08-02)

- **Root cause**: collection cards scaled the complete transparent portrait canvas by width and then cropped it through a `118×126` upper-body window. Narrow silhouettes therefore appeared smaller than broad heroes, while each asset's internal transparent offsets controlled the apparent head and foot position.
- **Visible-silhouette contract**: the list now resolves each active theme portrait, measures its real alpha-used bounds, scales that visible silhouette to exactly `180 px` high and anchors every hero to the same `194 px` foot baseline. Natural body width remains intact; no hero is stretched to imitate another build.
- **Presentation envelope**: the portrait lane is `148×196`, horizontally centred before the common copy column. Its child art may exceed the former icon crop so hair, shoulder armour, coats and boots remain complete, while the enclosing `224 / 310 px` Chinese / English card is still the final safe clip.
- **Bilingual and theme proof**: real 1080×1920 Godot captures cover all five theme states in Chinese plus default and Gilded English layouts. Manual inspection found consistent head scale and footing with no title, tag, description, action-button or neighbouring-card collision.
- **Permanent regression**: M1 smoke enumerates all four collection rows and requires the authored portrait envelope, unclipped local lane, frameless themed texture, common visible height and common baseline. This makes alignment independent of future source-canvas dimensions.

## Wave Spawn Blue-Noise Distribution (2026-08-02)

- **Root cause**: formation routing was working, but each queued group independently sampled a narrow lane: only `160 px` for centre and `210 px` per flank. Rush deliberately routed every enemy to centre, so valid random numbers still produced repeated visible piles.
- **Formation-preserving spread**: normal-enemy corridors are now `150–430` left, `300–780` centre, `650–930` right and `150–930` spread. Rush remains a broad frontal push, pincer still leaves centre open, escort retains flanks and siege still rotates three channels.
- **Anti-cluster picker**: each spawn rolls nine random X/Y candidates and selects the one with greatest clearance from the last six births plus live enemies still in the top entry band. Immediate X separation receives extra weight and Y receives only a small `158–222` stagger, so depth cannot conceal a horizontal pile.
- **Balance boundary**: boss entrances retain their former narrow corridors and fixed `y=190`. Enemy count, authored interval, HP, damage, speed, reward and total wave timing are unchanged; this pass affects entry composition only.
- **Proof**: M1 smoke uses a fixed seed to sample every corridor 24 times, checks bounds, at least 85% corridor coverage, bounded recent history and no more than three close consecutive pairs while retaining all five formation contracts. A separate 1,000-seed audit measured `88.5%` worst-case coverage and at most two close pairs. A real 1080×1920 Godot capture stages twelve mixed zombies at the campaign median 0.60-second interval and shows full-width, depth-staggered entry without HUD or edge violations.

## Skill Codex Detail Mobile Typography And Compact Upgrade Price (2026-08-02)

- **Scoped readability pass**: only the Skill Codex detail route changes. The skill name, level badge, tags, summary, section headers, core-stat labels, five level rows and tactical notes each move up one local type step; character and equipment detail hierarchies remain untouched.
- **Level-row reflow**: level and effect text now use effective `24 px` / `23 px` sizes with a `64 px` minimum row. Longer localized values retain smart wrapping and the existing vertical scroll contract instead of shrinking back to the former small copy.
- **Compact action copy**: the permanent-upgrade button shows the price-only `150★` pattern (or the real current cost) because the surrounding modal already establishes the action. Its accessibility tooltip still states the true XP cost; the label uses the larger action size and is measured against the native `412×112` bezel.
- **Bilingual screen proof**: standard 1080×1920 Chinese fits without scrolling; English exposes an intentional scrollbar and reveals the complete final line above the fixed actions at scroll end. Tall-screen regression covers all sixteen skill IDs in both languages plus the existing English representative route: `33/33`, zero runtime or image-analysis issues.
- **Permanent regression**: M1 smoke locks the larger level/effect/body sizes, row height, compact current-cost copy and actual rendered button-text width. Review images remain under `/tmp/zf-skill-detail-review/` and are not repository assets.

## Theme-Specific Skill Semantic Tags And Bilingual Baseline Closure (2026-08-02)

- **Root cause**: Skill Codex tags passed a semantic accent into `UiKit.pill`, but the production map-pill texture path discarded both that accent and the requested background. Its faint decorative fragments disappeared against the card texture and made otherwise aligned copy look visually broken.
- **Dedicated component**: Skill rows now use a purpose-built `40 px` semantic tag with a continuous `2 px` border, restrained opaque fill, consistent horizontal padding, subtle dark lift and vertically centred copy. Kind tags and ability tags have separate visual roles while staying quieter than interactive buttons.
- **Five authored palettes**: `themes.json` owns normalized RGBA border / fill / text colors for Default, Neon Tempest, Infernal Dominion, Polar Aurora and Gilded Eclipse. ThemeManager resolves the palette without UI-side theme-ID branches, and data validation rejects missing or malformed channels.
- **Layout repair loop**: the first real capture exposed a title-to-tag collision after the stronger border made the old spacing visible. Tags moved to a dedicated baseline, summaries moved below them, and the summary envelope was restored to `80 px` after the runtime audit caught two bilingual long-copy rows requiring `78 px`.
- **Regression**: M1 smoke checks the dedicated style, border thickness, height, semantic role, centered copy, section gaps, all sixteen row widths and five distinct palettes. A permanent `--skill-tags-only` matrix captured five themes in Chinese and English (`10/10`, zero runtime or image-analysis issues). Review output remains in `/tmp/zf-skill-tag-theme-review/` and is not tracked.

## In-Battle Skill HUD Readability And Active-Skill Gesture Contract (2026-08-02)

- **Left-half skill dock**: owned skills now use `96×120` cards with `66×66` icons and effective `23 px` localized level text. The dock owns only `x=18–530`, fits five cards per row and grows upward from the same `y=1808` bottom edge as the active-skill button.
- **Explicit inspection**: passive skill details require an intentional tap or long press and remain readable after release. Desktop hover still provides scale / native tooltip feedback but no longer opens the large combat overlay unsolicited.
- **Ready / cooldown semantics**: a ready active-skill short tap casts immediately and closes any old explanation. The cooling button remains interactive; the next short tap opens its description without changing cooldown or cast count. A long press opens the same description in any state and suppresses exactly the release-generated Button press, preventing accidental casts.
- **Bilingual resilience**: visible level badges and native tooltips both route through localization (`等级1 / Lv.1`); the active-skill native tooltip now localizes its name and description as well.
- **Permanent proof**: M1 smoke verifies the bounded anchor, enlarged geometry, phone-readable type, sixth-card wrap, passive single-tap detail, active cooldown inspect and long-press non-cast behavior. Five real 1080×1920 Godot captures cover Chinese / English, two-row / maximal legal rows, ready / cooling detail and Gilded Eclipse styling with zero runtime or image-analysis issues. Review output remains in `/tmp/zf-battle-skill-interaction-review/` and is not tracked.

## Gilded Eclipse Frost Full-Body Three-Direction True-Grip Repair (2026-08-02)

- **Root cause**: the approved 4×3 direction master contains Frost's complete legs and boots, but the deterministic exporter ran GrabCut and then retained only the largest connected alpha component. Her mirror-black greaves were close to the dark runway and became disconnected, so all three runtime PNGs expanded an upper-body / coat fragment to the full `380×520` canvas.
- **Reviewed source**: a dedicated three-cell chroma master preserves the exact mature Frost identity, silver ponytail, black-gold tailoring, Golden Law cannon, rear camera and two-hand shoulder brace. Up-left, centre and up-right now share one scale and boot baseline with complete head-to-foot anatomy and safe muzzle / foot padding.
- **Deterministic output**: `build_gilded_eclipse_runtime_assets.py` routes only Frost's three Golden Law exports through the keyed source, keeps all valid character components, removes resampling spill and leaves every runtime path and weapon presentation value unchanged. The other nine approved hero/direction masters are untouched.
- **Permanent proof**: the builder rejects undersized silhouettes and insufficient two-sided lower-body coverage. `check_visual_assets.py` also requires at least `150 px` of visible span in the lowest 8% of every Frost direction, directly distinguishing two grounded boots from the former narrow coat endpoint.
- **Real runtime closure**: the first capture correctly exposed stale Godot imported textures still showing the old half-body files. After a forced headless import, all three 1080×1920 captures show complete legs / boots and direction-matched muzzle / projectile lines; the routed visual audit passes `3/3`. Final review files remain under `/tmp/zf_gilded_frost_fullbody_final/` and are not tracked.

## Neon Blaze Raised Side-Aim True-Grip Revision (2026-08-02)

- **Portrait-field correction**: the accepted Blaze side pose looked strong but held the Thunder Apocalypse cannon too close to horizontal for a 1080×1920 battlefield, where meaningful targets overwhelmingly remain above the fixed hero dock.
- **Scoped art revision**: a reviewed chroma-key right master raises the complete cannon and naturally re-articulates both hands, shoulders, elbows, wrists, torso and sight line while preserving the exact Blaze identity, Neon Tempest suit, rear three-quarter camera and wide full-body stance. The left sprite is a deterministic mirror; the centre master is untouched.
- **Runtime contract**: only `char_blaze_apocalypse_attack_left.png` and `char_blaze_apocalypse_attack_right.png` change. Both remain `380×520`, retain safe hair / muzzle / boot margins and measure a `38.2°` neon barrel rise. The centre output retains its prior SHA-256 `340a2762ee6719295c69cfedebacfcc9aae35726f17e37d21c18c47ac3f07dfc`.
- **Permanent proof**: `check_visual_assets.py` isolates the upper-cannon neon axis for Blaze + Thunder and rejects either side below `35°` or pointing toward the wrong upper corner. The source, alpha, prompt and deterministic builder route remain under `premium_neon_tempest_thunder_true_grip_2026_07_29`.
- **Real runtime closure**: after refreshing Godot imports, two 1080×1920 captures verify the raised left / right poses with the complete Neon firing aura and direction-matched battlefield corridor. Runtime and image audits pass `2/2`; review files remain under `/tmp/zf_neon_blaze_raised_aim_final2/` and are not tracked.

## Global In-Battle Anatomical Character Normalization (2026-08-02)

- **Root cause**: every battle pose was fit from its complete `380×520` composite. Long cannons, coats, skirts, wings and muzzle glows therefore participated in scaling, and character level added another visual growth multiplier. Two equally tall people could render at visibly different sizes—even for the same hero after changing equipment.
- **Anatomical ruler**: `character_body_metrics.json` defines head-to-foot height, foot anchor and horizontal body centre independently for the standard animation set and all four premium true-grip weapon sets, covering all four heroes and all three firing directions. Weapons, trailing cloth, theme wings and muzzle energy are explicitly outside the measurement.
- **Runtime transform**: each pose is normalized to the common `420 px` source-body target before the approved `1.50×` presentation scale, yielding an exact `322.56 px` design-space body height and common `y=1752` boot line. Character levels retain stats and badges but no longer alter human scale.
- **Attachment integrity**: changing left / centre / right pose reapplies both scale and anatomical anchor. Authored true-grip muzzle coordinates pass through the same transform, so projectiles remain attached to the real barrel while independent Neon, Infernal, Polar and Gilded signatures can extend beyond the body envelope.
- **Permanent proof**: data validation requires exact profile / hero / pose coverage and valid measurements; M1 smoke verifies identical effective body height, foot line and transformed muzzle coordinates. The HUD audit covers `944` character / weapon frames and retains a `6.9 px` minimum bottom-resource gap.
- **Runtime visual closure**: five themes × four heroes × three directions produced `60` real 1080×1920 Godot captures. A second human-review sheet overlays a common cyan head line and magenta foot line: bodies share the ruler while guns and VFX deliberately overflow it. Runtime and image audits pass `60/60`; review files remain under `/tmp/zf_body_scale_regression_v1/` and are not tracked.
- **Verification**: asset pack (`11,314` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `400` `res://` references, 99-level pressure, 99-level card simulation, `1,092` battle visual assets, `944` HUD frames, isolated Godot boot and M1 smoke all pass. The known macOS CA-certificate warning is non-fatal.

## Appearance Selector Alpha-Bounds Full-Body Repair (2026-08-02)

- **Finding**: `Follow Global` under an active Gilded Eclipse theme and the explicit `Gilded Eclipse` outfit correctly resolved to the same Vanguard portrait. Both looked like a clipped narrow strip because the selector fitted the complete `720×980` transparent authoring canvas, allowing unused alpha margins to shrink the actual person.
- **Full-person fit**: every selector portrait now derives an `AtlasTexture` from the real alpha-used bounds plus a `10 px` safety gutter. Only transparent canvas is discarded; hair, shoulder armour, arms, coats and boots remain inside the displayed region and retain the source aspect ratio.
- **Shared contract**: hero showcase, global theme cards, hero rows and outfit cards all use the same resolver and fitting path. Follow Global and an explicit outfit therefore remain pixel-identical whenever they resolve to the same theme, while all five themes gain a more stable visual presence.
- **Permanent proof**: M1 smoke checks all six outfit choices in both languages, requires an alpha-fitted atlas, and proves the display region encloses the complete source-visible person on every side. A real 1080×1920 Gilded Vanguard capture confirms the two duplicated portraits are complete, equal-sized and clear of copy / button borders.
- **Verification**: asset pack (`11,314` files), data, all `400` resource references, 99-level pressure, 99-level card simulation, clean Godot boot, M1 smoke and the routed screenshot audit pass. Review output remains under `/tmp/zf_appearance_portrait_fix/` and is not tracked.

## Appearance Selector Theme-Wide Hero-Bust Normalization (2026-08-02)

- **Owner follow-up**: removing transparent canvas fixed accidental shrinkage, but a complete slender figure still looked materially smaller than broad armored heroes inside a `150×172` phone thumbnail. That is a framing problem, not a reason to distort body width.
- **Shared crop contract**: all outfit rows now frame the top `62%` of the source-visible silhouette, while the large hero showcase uses a roomier `70%`. Both retain `10 px` head / hair safety, centre on the real alpha body and crop to the viewport aspect before `TextureRect` fitting, producing one head scale and one upper-body depth across every source canvas.
- **Natural variation preserved**: no portrait is stretched. Vanguard remains broader than Volt, mounted guns and armour can occupy more width, and the five themes retain their silhouette language; only the human presentation ruler and camera crop are normalized.
- **Screenshot coverage**: four Gilded-active hero pages were captured at the top and scrolled lower section, covering Follow Global, Default, Gilded Eclipse, Infernal Dominion, Neon Tempest and Polar Aurora in `8` real 1080×1920 frames. The routed UI audit reports zero issues, and manual inspection found no clipped hair, off-centre face, row overflow, copy collision or button intrusion. Files remain under `/tmp/zf_appearance_bust_fix/` and are not tracked.
- **Permanent proof**: M1 smoke switches the selector through Chinese and English, checks all six portrait cards plus the large showcase, and locks the bust mode, `62% / 70%` framing ranges, complete headroom, source-body centre and exact viewport aspect. The screenshot harness now prioritizes the active high-layer appearance modal when a scroll offset is requested.
- **Verification**: asset pack (`11,314` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `400` resource references, 99-level pressure, 99-level card simulation, isolated Godot boot, M1 smoke and `8` routed screenshots pass. The known macOS CA-certificate warning remains non-fatal.

## Gilded Eclipse Loadout Presence And Golden Law Weapon Framing (2026-08-02)

- **Character root cause**: the loadout still assigned every `720×980` portrait a fixed `378 px` canvas width. Gilded Eclipse uses intentionally leaner alpha silhouettes, so all four humans occupied materially less of the Hero Unit frame even though the outer source dimensions matched the other themes.
- **Alpha-aware bust normalization**: only Gilded loadout portraits now target `260 px` of real visible silhouette width, clamped to a `460–540 px` source-canvas display range. The existing 10 px hair safety, natural aspect ratio and upper-body crop remain intact; broad and lean heroes keep their authored builds instead of being stretched.
- **Weapon source repair**: the former Golden Law inventory image carried a lower-right concept inset / duplicate reflection and edge-contacting geometry. A new clean single-cannon master reconstructs the complete muzzle, rear housing and grips on a guarded `384×384` transparent canvas, with no text, border, reflection or secondary object.
- **Loadout fit contract**: the V2 Golden Law icon is cropped only to its full alpha bounds plus a 12 px transparent gutter before keep-aspect display. It now fills the Weapon Panel while every extremity remains visible; no other weapon or theme code path changes.
- **Visual proof**: real 1080×1920 captures cover all four Gilded heroes and a same-hero comparison against Default, Neon Tempest, Infernal Dominion and Polar Aurora (`8/8`, zero routed audit errors). Manual review confirms comparable face / shoulder scale, consistent headroom, complete weapon geometry and no copy, panel or button collision. Review files remain in `/tmp/zf_gilded_loadout_fix/` and are not tracked.
- **Permanent regression**: M1 smoke locks the four-hero Gilded display-width envelope, V2 weapon path, source alpha guard band and atlas region that must enclose the complete cannon. The generated source and exact prompt live under `assets/production/source_refs/generated/premium_black_gold_golden_law_phase4_2026_08_01/` and are registered in the production asset index.
- **Verification**: asset pack (`11,319` files), data (`99` levels / `20` zombies / `8` bosses / `16` skills), all `401` resource references, 99-level pressure, 99-level card simulation, `1,092` battle visual assets, isolated Godot boot, M1 smoke, routed screenshots and `git diff --check` all pass. The known macOS CA-certificate warning is non-fatal.

## Direct Normal / Challenge Stage Actions And Card Recomposition (2026-08-02)

- **Interaction hierarchy**: `Normal` and `Challenge` are now the two direct stage actions. The former duplicate `Enter` and `Challenge Mode` buttons have been removed, reducing every card from four mode-related UI elements to two unambiguous choices.
- **Progress inside the action**: each mode button owns its three-star history. Locked Challenge stays in the same location with a lock mark and subdued secondary surface; a normal three-star clear enables that exact button without moving any content or bypassing the existing route guard.
- **Native art and touch contract**: both actions use the existing theme-resolved `412×88` native button raster. The `192 px` card holds two real 88-pixel controls plus an 8-pixel gap, so visual bounds and expanded mobile hit regions never overlap and no theme button is stretched.
- **Information architecture**: the left lane owns the three-digit gate number and Current state, the middle lane owns the title / power / weakness, and the right lane owns mode selection. Widening the weakness capsule removes the English `Weak: Frost / Physical` collision without reducing type size.
- **Bilingual and theme proof**: routed 1080×1920 captures cover Default Chinese, Default English, Gilded Eclipse English and a populated normal/challenge progress state. All labels, stars, lock state, card edges and mode surfaces remain clear with zero runtime or image-analysis issues. Review files remain under `/tmp/zombie_fire_level_modes_final/` and `/tmp/zombie_fire_level_modes_review_v2/` and are not tracked.
- **Permanent regression**: M1 smoke asserts the new node identities and mode metadata, embedded star rows, locked-Challenge marker, absence of both legacy buttons, shared minimum touch height and non-overlapping hit geometry.

## Infernal Dominion Main-Logo Second Enlargement (2026-08-02)

- **Root cause**: the `1040×340` Infernal title source retained a concept-board dark stage on both sides. The runtime dark-key shader removed its color, but keep-aspect layout still measured the whole source, so the actual metal emblem occupied too little of the phone width.
- **Scoped presentation fix**: the Infernal theme now supplies a symmetric `[210, 0, 620, 340]` authored region. This discards only unused side staging, keeps the full mechanical crown, sparks and molten lower point, and changes no other theme or source asset.
- **Visual proof**: the final 1080×1920 runtime capture keeps roughly 145 px horizontal guard space and 80 px above the subtitle while materially increasing the emblem's presence. No edge clipping, subtitle collision or downstream button movement is visible. Review output remains `/tmp/zf_infernal_logo_review/menu_logo_infernal_final.png` and is not tracked.
- **Permanent regression**: M1 smoke verifies both the data-owned region and the exact AtlasTexture presentation crop, preventing the removed concept staging from silently returning to layout measurements.
- **Verification**: asset pack (`11,319` files), data, all `401` resource references, 99-level pressure, 99-level card simulation, isolated Godot boot, M1 smoke, final screenshot image analysis and `git diff --check` all pass. The known macOS CA-certificate warning remains non-fatal.

## Character Collection Large-Portrait And Copy-Lane Recomposition (2026-08-02)

- **Card-space correction**: the collection already devoted substantial vertical space to each hero, but the former `148×196` portrait envelope left most of it empty. Character rows now share a `310 px` presentation height and a `220×282` portrait lane whose real silhouette is normalized to `250 px` high.
- **One human ruler**: every active-theme portrait is alpha-measured, scaled without changing its natural body width and grounded to the same `y=276` foot line. Hair, armour, coats and weapons may keep their authored width while the four people read at one deliberate scale.
- **Independent copy lane**: title, semantic tags and description begin at `x=260`, at least 20 px beyond the portrait lane. The action control remains in the right rail, so neither longer English names nor enlarged character art can intrude into another role.
- **Deliberate bilingual wrapping**: Chinese character facts are authored as separate `定位 / 元素 / 下级成长` lines instead of relying on automatic glyph wrapping; English follows `Role / Element / Next Lv.`. This removes the former orphaned final glyphs while keeping the phone-readable font size.
- **Visual and permanent proof**: five themes in Chinese and English produced `10/10` real 1080×1920 Godot captures with zero routed audit issues. M1 smoke locks the row height, enlarged envelope, common visible height / foot baseline and the minimum gap between the portrait and all three copy nodes. Review files remain under `/tmp/zombie_fire_character_cards_final/` and are not tracked.

## Loadout Hero-Bust And Clean-Weapon Presentation Contract (2026-08-02)

- **Reference-preserving human ruler**: the approved default Steel Vanguard remains exactly at its former `378 px` authored display width. Its real `845 px` alpha height becomes the common loadout ruler, so every Default, Neon Tempest, Infernal Dominion, Polar Aurora and Gilded Eclipse portrait is measured and scaled by its visible silhouette rather than its transparent source canvas.
- **Consistent half-body composition**: all twenty hero / outfit combinations keep `10 px` of head safety and centre their real visible bounds inside the `336×282` crop. Natural build differences remain; no portrait is horizontally stretched and weapon, coat or shoulder width no longer changes the human-height contract.
- **Clean weapon source**: the loadout now prefers each weapon's unframed `handheld` art and resolves the active theme's matching clean skin for the eight standard weapons. Premium weapons remain theme-independent. Golden Law declares a data-owned `loadout_art` override to use its complete single-cannon V2 cutout instead of the staged handheld concept containing a duplicate inset.
- **One weapon ruler**: every source is cropped to its actual alpha bounds plus a `5.5%` proportional safety gutter and fitted inside the common `296×296` viewport. The rule preserves complete muzzles, stocks and grips while eliminating baked icon frames and transparent-canvas size drift.
- **Proof**: a focused five-theme × twelve-weapon matrix captured `60` real 1080×1920 loadout screens and cycles all four heroes three times per theme. Routed layout and image analysis report `60/60` clean; manual hero and weapon contact sheets confirm aligned busts, clean single-gun renders, consistent long-axis occupancy and no clipping or copy collision. Output remains under `/tmp/zf_loadout_presentation_20260802/` and is not tracked.
- **Permanent gate**: M1 smoke measures all twenty portrait layouts plus every standard, themed and apocalypse loadout gun. It locks the Steel Vanguard reference, exact headroom / vertical ruler, alpha-centred silhouette, complete display region, clean Golden Law override and bounded `246–276 px` weapon occupancy.

## Challenge Result Aligned Hero-Bust Showcase (2026-08-02)

- **Composition upgrade**: the challenge / campaign / endless result showcase no longer shrinks a complete hero into a `108×108` inventory-like thumbnail. It now owns a `170×144` clipped hero lane inside a `164 px` outcome card, producing a readable head-to-waist victory portrait without disturbing rewards, stars or actions.
- **Same ruler as the approved UI direction**: every Default, Neon Tempest, Infernal Dominion, Polar Aurora and Gilded Eclipse hero is alpha-measured and scaled to one `280 px` visible human height, with exact `8 px` head safety and visible-bounds centring. Transparent canvas, weapons, coat tails and effects do not influence size; natural body width remains untouched.
- **Copy safety**: the enlarged portrait and the copy own independent lanes. Chinese and English hero names, outcome state and combat highlight remain complete, including the longest `Steel Vanguard · Defense Complete` line.
- **Proof**: five themes × four heroes × Chinese / English produced `40` real 1080×1920 challenge-victory result captures. Routed layout and image audits report `40/40` clean; manual contact-sheet review confirms common head line, crop depth and human scale with no clipping, frame collision or text intrusion. Output remains under `/tmp/zf_result_portrait_review_20260802/` and is not tracked.
- **Permanent gate**: M1 smoke traverses all twenty resolved portrait assets and locks the half-body mode, exact headroom, human-height ruler, visible centre and actual crop relationship.

## Result Reward / Standing-Power Icon Safe Lanes (2026-08-03)

- **Reward-card rhythm**: the two decorated reward cards now reserve `30 px` horizontal safe lanes. Gold and XP share one `54 px` icon ruler, a `16 px` copy gap and vertically centred icon / copy groups.
- **Standing-power strip**: the baseline / final / recommended-power explanation reserves `28 px` on both sides around its `50 px` star icon and wrapped copy, keeping the icon away from the luminous border in both languages.
- **Permanent proof**: M1 smoke checks left-edge clearance, reward-icon size parity and the shared vertical baseline; the focused result matrix covers all five themes, four heroes and Chinese / English.

## Premium Store Cross-Theme Semantic Merchandising Grid (2026-08-03)

- **Root cause**: the store previously treated each product's `art` as a ready-to-ship card image even though those files served different authoring purposes and aspect ratios. Neon showed a battle capture, Infernal showed a style board, and some arsenal masters contained selection layouts, duplicate details or copy.
- **One grammar across series**: theme products now resolve the four real themed hero portraits into the same `2×2` roster. Complete and upgrade arsenals resolve their data-owned weapon, armour, chip and pet into the same ordered `2×2` equipment grid. Every preview uses the same `330×330` frame, `152×152` cells and theme-accent border hierarchy.
- **One ruler inside each grammar**: outfit portraits share a `212 px` alpha-visible human-height ruler and `8 px` headroom before the deliberate card crop. Equipment shares a `124 px` alpha-visible long-axis ruler and remains completely inside its cell. Natural body and item proportions are preserved without non-uniform scaling.
- **Composite-source cleanup**: the Gilded chip and Skyfalcon retain their accepted source icons everywhere else, while a data-owned `store_preview_region` removes inset variants and source-board labels from the store-only view. Eternal Night Aegis now uses a clean, complete single-object full-body source everywhere and no longer needs a store crop. The Golden Law weapon already uses its clean V2 single-cannon icon.
- **Data and regression contract**: every product declares `theme_roster` or `arsenal_grid`; validation enforces the role mapping and presentation-region shape. M1 smoke builds the fully revealed fresh catalog and checks eight visible cards, four production assets per card, exact geometry / scale metadata and the absence of any `source_refs` path.
- **Bilingual visual proof**: four scroll stops in both Chinese and English cover Neon, Infernal, Polar and Gilded theme + arsenal cards. All `8/8` real Godot captures pass runtime and image analysis; manual review found no cropping, copy overflow, mixed imagery or cross-theme layout drift. Review output remains under `/tmp/zf_store_preview_review_20260803/` and is not tracked.

## Apocalypse Armor Prototype Completeness and Regression Gate (2026-08-03)

- **Complete catalog audit**: all ten armor sources were inspected outside the UI. The six standard items are intentional, complete isolated vest / chest / hazmat equipment icons. The four Apocalypse sources were the defects: Neon was torso-only, Infernal and Polar were headless suits, and Gilded combined a torso with a duplicate inset.
- **Four production replacements**: Sky Conductor Armor, Molten Starplate, Permafrost Crystal Wall and Eternal Night Aegis now use clean transparent `384×384` full-body armor renders with an enclosed helmet, both arms, both legs and both boots. They remain weaponless, keep the frozen IDs and paths, and carry no labels, frame or secondary concept inset.
- **One source in every consumer**: armor list cards, armor detail panels and Premium Arsenal grids now consume the same clean production images. Eternal Night Aegis no longer declares `store_preview_region`; only genuinely composite Gilded chip / pet sources retain store-only presentation regions.
- **Bilingual proof**: `tools/check_visual_screens.py --armor-prototypes-only` captures the Chinese and English catalogs plus every armor detail, producing `22/22` real 1080×1920 screens. Runtime layout / image analysis and manual review confirm complete silhouettes, readable names, clear level / close controls and safe frame margins. Output remains under `/tmp/zf_armor_prototype_review_20260803_v4/` and is not tracked. The affected store cards were additionally rechecked in the eight-screen bilingual store regression.
- **Permanent semantic gate**: `validate_asset_pack.py` now rejects any of the four premium armor assets unless it is a transparent non-empty `384×384` render with four-edge safety, full-body vertical occupancy, a plausible helmet-to-shoulder ratio and no large detached duplicate component.

## Character Detail Mobile Secondary-Type Recomposition (2026-08-03)

- **Hierarchy instead of indiscriminate scaling**: the hero name, section titles and bottom actions keep their accepted hierarchy, while the genuinely small information layers move up one step: affinity summary, role / element / build pills, stat labels and growth values, passive / signature descriptions, signature level and per-level rules.
- **Readable signature investment block**: the old horizontal row forced the longest progression copy into the remaining space beside a `286 px` button. The new two-tier block keeps level and a native `286×112` upgrade action on top, then gives the growth rule the full card width below. This also closes the previously inherited 80px touch-target exception.
- **Responsive vertical rhythm**: stat tiles now reserve `98 px` in Chinese and `112 px` in English; skill rows reserve at least `118 px` and grow naturally with wrapped copy. On shorter tall phones the detail body scrolls independently while the four bottom actions remain fixed and unobscured.
- **Bilingual device proof**: every hero detail was captured in Chinese and English at both the Owner-reference `1080×2340` viewport and a tighter `1080×2046` viewport. All `16/16` real Godot screens pass runtime layout and image analysis; manual review confirms readable secondary text, complete long English copy and stable borders / buttons. Output remains under `/tmp/zf_character_detail_readability_20260803_final/` and is not tracked.
- **Permanent gate**: M1 smoke names and inspects the affinity, pill, stat-secondary, skill-description and signature-growth controls, enforces their minimum effective font sizes and requires the growth line to own more than twice the upgrade-button width.

## Character Detail Whole-Page Type Scale Pass (2026-08-03)

- **Full hierarchy, not one isolated paragraph**: the second readability pass raises the hero name and level badge, section headers, affinity line, semantic pills, stat labels / values / growth, skill names / kinds / descriptions and signature progression copy together. Effective minimums now span `24–50 px`, while the accepted four native bottom actions keep their existing geometry and hierarchy.
- **Scoped shared builders**: the reusable pill and stat-card builders accept an explicit character-detail delta. Only the character surface requests it, so weapon, armor, chip and pet detail layouts do not inherit an accidental font or height change.
- **Content owns height**: Chinese / English stat cards gain `8 px` only on this surface and skill rows reserve `132 px`; longer English descriptions wrap at their natural size inside the independently scrolling detail body. The fixed action grid remains fully visible at every tested height.
- **Three real phone heights**: the permanent matrix now captures all four heroes in both languages at `1080×2340`, `1080×2046` and the Owner screenshot's `1080×1920`, for `24` real Godot screens. Runtime bounds, image analysis and manual review confirm complete names, labels, wrapped descriptions, panels and action controls. Final review output remains outside Git under `/tmp/zf_character_detail_readability_20260803_v2_final/`.
- **Regression contract**: M1 smoke names the hero-name and level controls, checks at least five section headings, and enforces effective `50 px` hero names, `28 px` levels, `29 px` section titles, `24 / 32 / 22 px` stat layers, `30 / 24 / 26 px` skill layers and `23 px` signature-growth copy.

## App Store Full Screenshot Review Pack (2026-08-03)

- **One explicit release matrix**: `tools/check_visual_screens.py --full-review` now de-duplicates and combines the final release suite with the three-height character-detail audit, five-theme / twelve-weapon loadout presentation audit and bilingual result-portrait audit.
...
## Challenge Campaign Simulation Gate (2026-08-03)

- **Runtime-aligned data path**: `simulate_balance.py --challenge` resolves each level's chapter row from `data/challenges.json`, applies authored HP to enemy durability / clear time, exact breach damage to breach pressure, and speed plus mechanic frequency to the expected control-window pressure. No challenge multiplier is duplicated in Python.
...
## Gilded Eclipse Fully Rendered Native Buttons (2026-08-10)

- **Placeholder source removed**: the old black-gold generator constructed every button from flat polygons, thin rails and repeated diamond nodes. Weapon-detail actions therefore read as geometric scaffolding rather than a finished App Store material.
- **Two authored action tiers**: a dedicated primary master uses mirror-black stone, deep gunmetal, polished flowing gold and restrained warm energy; the secondary master uses quieter steel / antique-gold relief. Both keep a broad dark text-safe lane and remain visually distinct without changing button copy or interaction.
- **Native geometry preserved**: all `36` registered sizes per tier continue to resolve through their existing asset paths. The builder independently scales rendered end armor and cross-fades only a calm centre strip, so a `170×84` compact control and a `980×58` rail retain the same material identity without stretching their corner hardware.
- **Focused production proof**: the button-only build validates exact dimensions, corner transparency and real material variance for all `72` outputs, refreshes hashes in the full Gilded runtime manifest, and records five representative aspect ratios in `gilded_eclipse_button_runtime_contact_sheet_v2.png`. A routed weapon-detail capture with an enabled primary action beside both secondary actions passed at `/tmp/zombie_fire_gilded_button_review_20260810_v4/collection_detail_gilded_buttons.png` (local QA only, not committed).

## Fixed Collection Title-to-Tag Rhythm (2026-08-10)

- **One bilingual vertical contract**: character, weapon, armor, chip and pet cards now share the same title lane and metadata anchors. The title-to-tag interval is exactly `8 px`; the tag-to-description interval is exactly `6 px` in both Chinese and English.
- **Visible spacing follows authored spacing**: titles bottom-align inside the fixed lane, removing the language-dependent dead area that previously made `Balanced / Physical` and equivalent tags appear detached from English names.
- **Readability is protected**: the title lane remains `60 px` high, preserving the approved mobile font size for normal names. Width fitting remains available only for genuinely long localized catalog titles.
- **Permanent regression coverage**: M1 smoke traverses both languages across all five families and asserts the exact spacing, bottom alignment and semantic tag containers. Ten routed 1080×1920 captures passed layout, adaptive-text and image audits at `/tmp/zombie_fire_collection_title_tag_spacing_20260810_v2/`.

## Infernal Dominion Fully Rendered Native Buttons (2026-08-10)

- **Confirmed root cause**: the Infernal button builder still drew every control from polygons, nested outline strokes, repeated circular nodes and a center rail. There was no previously accepted rendered button family to reconnect, so the owner-requested skip condition did not apply.
- **Two real material masters**: the new primary bezel uses charred forged steel, heat-tempered copper and restrained molten seams; the secondary bezel uses quieter gunmetal / antique-copper relief. Both keep a calm black text well and avoid the old diagram-like construction.
...
- **Area semantics, not a dome**: the former semicircular silhouette is replaced by a full-width horizontal cryogenic threshold plus a continuous subdued frost wash from the data-owned `y_min` to the real defense line. Even the 70% level reads as one occupied battlefield region rather than an enlarged arc.
- **No visual stretching with growth**: the authored boundary remains `1080×240` at every skill level and only moves upward. The quiet interior texture tiles at fixed density while its clipped container grows, so ice detail, edge thickness and particle scale stay consistent from 30% through 70% coverage.
...
## Adaptive Wave Pressure + XP Cost Recall (2026-08-16)

- **First ten levels are object-for-object protected**: the rerunnable generator records and checks the canonical complete level-object SHA before touching any campaign data. Only target-wave `spawns.count` rows from levels 11–99 are eligible; support, Boss and interval authoring remain unchanged.
- **Pressure follows the star boundary**: each level solves the highest `0.001` scale up to `+20%` that preserves at least two leak percentage points from its current star boundary. The checked result is exactly 77 full, seven partial and five unchanged levels, with no per-level star regression.
- **Economy returns to the approved feel**: additional kills raise three-clear XP to `271,084`; the two L5 skill costs move to `8,500 / 11,000`, producing `335,000` total cost and `80.92%` coverage. Gold supply remains above the free-growth midline without inversion.
- **Power remains comparable**: all 99 contracts are regenerated. The 2–98 corridor stays in `[0.9700,1.3800]`; Stage 99 moves from `5322/4546` to `5443/4650` at the same `R≈1.1706/Boss`, and the Stage-80 replay moves from `4268/2431` to `4439/2528` at the same `R≈1.7558/Boss`.
- **Challenge remains winnable**: only Chapter 10's mechanic multiplier needs a minimal `1.24→1.21` callback. Stage 99 falls from a 100% predicted breach to 99%; every challenge level remains below its phase cap and total challenge stars stay at 164.

## Boss Slow Resistance + Repeated-Copy Tempo (2026-08-17)

- **Control keeps its crowd identity**: small enemies retain the authored 80% Slow Field ceiling, while Bosses cap received slow at 40%. Runtime movement and the player-facing line axis both read `economy.boss_pacing`; the skill's five authored levels are unchanged.
- **Repeated models stop building a linear HP wall**: same-ID copies consume `1.0 / 0.82 / 0.45 / 0.35` durability multipliers from one data source. The second-copy prototype of `0.60` produced a 52.9-second Stage-75 phase, so it was raised to `0.82` to restore the approved 60–100-second band without weakening later-copy attenuation.
...
## Late-Boss Curve + Power 5.0 Runtime Alignment (2026-08-20)

- **The Stage-85 spike is removed at its source**: the late campaign now teaches one, two, three and four full-health Necrotitans before graduating to one, two and four Apex Overlords. Stage 85 is `69.0M` total Necrotitan HP instead of one premature `93.24M` Apex, and its authored weakness is fire again.
- **A Boss model has one campaign durability**: every campaign copy consumes the model's literal `fixed_hp`. Repeated-copy coefficients survive only inside Endless, where they distribute an already-generated loop budget and do not alter campaign identity.
- **Power now values delivered hits**: a rerunnable generator converts checked-in Godot collider benchmarks and audited relative weapon DPS into per-weapon crowd / Boss axis profiles. Runtime GDScript and offline Python read the same economy table, and Release Candidate rejects stale profiles.
- **Boss pressure is counted once**: roster size, resistance and mechanics live in the Boss capacity contract; the headline recommendation no longer multiplies the same roster a second time. The Stage-80 pressure replay is `1038/989, R=1.05/Boss`, and the literal four-Apex finale is `1667/1437, R=1.16/Boss` at `183.1s`.
- **Contracts remain guarded**: the normal campaign stays at `13/86/0` stars, the 2–98 corridor remains `[0.9700,1.3800]`, the Apex single-phase audit remains `116.6s`, and a same-recommendation physical graduation build remains below the `460s` battle cap.
- **Endless consumes the same measured axis**: regenerated loop budgets use a `2%` graduation bridge in loops `7–10`, then cross-fade linearly over loops `11–20`. The level-080 Owner fixture reaches its wall at loop `11`, graduation covers loop `20`, the existing `[10,14] / [16,20]` release bands remain unchanged, and the approved +12% gold curve stays strictly increasing per minute.

## Campaign Rebuild v2 · Phase A Contract Sources (2026-08-21)

- **Targets are explicit but not prematurely frozen**: the complete 99-level grade sequence, four target bands and ten chapter quotas now live in `data/campaign_pacing_targets.json` with `frozen=false`. The audit reports every delta but does not turn an unreviewed target into a release failure.
...
- **Final-repair Chapter 2 is closed**: Stage 018's final-wave durability is 0.90. Its tier-B/v2.1 sweep is 10/10 at 66.71% median progress, 100% median base and 50.55% worst base; the regenerated-fixture three-seed sweep of Stages 019–099 introduced no new failing stage.
- **Stage 059 handoff mismatch is isolated**: the delivered solution did not match the Process-B evidence-generating level. The evidence state (W4/W5 HP 1.30/1.70 and two spitters) is restored, but the regenerated final fixture still loses 10/10, so the remaining drift is a real Chapter-6 repair rather than a merge omission.
- **Final-repair Chapters 4–5 are closed**: Stages 040 / 045 / 049 / 050 each pass 10/10 under Tier B / v2.1 and their frozen pressure bands. Stage 040's one-extra-Boss composition measures 42.30 seconds with 16.49% worst base; Stage 045 measures 55.84 seconds; Stage 049 measures 90.82% progress and 86.22% base.
- **Stage 050 preserves the frozen family model and topology**: Plague Mother remains 450k / bd 1.5 / poison resistance 50%, with the effective stagger, 15-spitter screen and trailing shambler unchanged. Only W4/W5 mob durability moves to 4.8 / 3.5; the ten-seed result is 10/10 at 87.50% progress, 67.42% base, 58.85-second Boss median and 58.44% worst base.
- **The downstream checkpoint is explicit**: after fixture regeneration, Stages 051–099 produce 131/147 wins and zero host timeouts over three seeds. Existing targets 058/059/060/068/077/080 remain visible; new fixture regressions at 062/069 are queued in numeric order after Stage 060 and before Stage 068.
- **Final-repair Chapter 6 is closed**: Stage 058's final-wave HP is 2.7 and passes 10/10 at 95.08% progress / 89.32% base with 3.69% worst base. Stage 059's local difficulty is 6.0 and passes 10/10 at 97.47% / 92.39%. Stage 060 uses four total Void Phantoms and passes 10/10 at 95.42% / 86.81% with a 41.78-second Boss median.
- **The second downstream checkpoint narrows the tail**: the regenerated fixture yields 112/117 wins and zero host timeouts across Stages 061–099. Stages 068 / 069 / 080 recover to 3/3 through progression propagation; only Stage 062 (0/3) and Stage 077 (1/3) remain failing and are handled in numeric order.
- **Stage 062's fixture regression is closed in order**: its level-local difficulty moves from 10.5 to 2.1 without changing global curves. The ten-seed result is 10/10 at 55.62% median progress and 100% median base, and the regenerated-fixture Stages 063–099 sweep is 109/111 with zero timeouts; Stage 077 is the only remaining loss.
- **The Chapter-7 tail is closed**: Stage 068's Wave-3 HP moves from 0.75 to 0.65 and passes 10/10 at 89.04% progress / 83.57% base with a 23.38% worst blade margin. After fixture regeneration, Stages 069–099 are 91/93 with zero timeouts and Stage 077 remains the sole loss.
- **Final-repair Chapter 8 is closed**: Stage 075 moves W4 HP 1.35→1.2 and passes 10/10 at 63.56% base / 78.13-second Boss with 9.60% worst base. Stage 077 moves W5 HP 1.7→1.1 and passes at 85.60% / 86.80%. Stage 080 moves W4 HP 2.12→1.9 and passes at 100% / 57.82% with an 85.10-second Boss median and 6.72% worst base.
- **Chapter-8 fixture propagation is clean**: after regeneration, the Stages 081–099 three-seed sweep is 57/57 with zero host timeouts and no new failing stage.
- **Final-repair Chapters 9–10 are closed under one explicit Owner exception**: Stages 088 / 090 remain in their authored contracts. Stage 095 freezes the no-guarantee `charger×2 + spitter×4 / W5 HP 17` composition and passes 10/10 under the regenerated fixture at 100% median progress, 76.88% median base, 5.96% worst base and 175.72 seconds maximum Boss phase. After three bounded equal-pressure attempts failed, Owner approved only Stage 095's high-grade base-median ceiling from 65% to 80%; every other grade, Boss band and safety floor remains frozen. Stages 096–099 then pass 12/12 downstream runs.
- **The Chapter-6 frozen digest follows the already-committed final repair data**: `898de3… → 17dfe2…`; the contract checker again confirms the 16/43/33/7/0 Tier-B distribution and 99/99 max-free graduation clears.
- **Stage 080's approved star is restored after the final convergence audit**: the regenerated progression fixture had reduced its median base from the approved 82.17% to 57.82%, which would have downgraded the stage. With Owner authorization, only Wave-4 local HP moves from 1.90 to 1.30. The fresh ten-seed result is 10/10 at 70.48% median base, 64.73% worst base and an 86.22-second Boss median, preserving 3★. The accepted strict-high-envelope conflict remains report-only, and the regenerated-fixture Stages 081–099 sweep is 57/57 with zero timeouts.
- **Chapter 1's remaining Boss-band medians are closed without model changes**: Stage 005 switches its data-owned offer floor to crowd and passes 10/10 at 62.23% progress, 100% median base and a 21.43-second Boss median. Stage 010 staggers its already-authored second Tank Titan by 10 seconds and raises only Wave-4 mob HP to 1.5; it passes 10/10 at 59.45% / 100% with a 32.26-second Boss median. The regenerated progression fixture then passes all 267 three-seed downstream runs across Stages 011–099 with zero host timeouts.
- **Chapter 3's Boss-band closure is stable under the regenerated fixture**: Stage 025 receives a five-offer XP cadence and passes 10/10 at 55.72% progress, 100% base and a 35.73-second Boss median. Stage 030 reduces only its final-wave spitter support from five to three and passes 10/10 at 95.84% progress, 91.01% median base, 52.69% worst base and a 44.21-second Boss median.
- **The resulting progression propagation is closed locally**: Stages 047 / 068 / 069 use level-local difficulty coefficients 12.5 / 32 / 13, while Stage 070 moves only Wave-4 HP from 1.7 to 1.1. All four pass 10/10; Stage 070 retains a 2.34% worst base margin and 65.77-second Boss median. Its 86.52% progress remains an accepted safety-first strict-high-envelope report item, consistent with Stages 039 / 084 and without relaxing any runtime gate. After fixture regeneration, Stages 071–099 pass 87/87 three-seed runs with zero timeouts and 21.53% minimum base.
- **The 99×10 convergence sweep has one isolated tail repair**: the first full pass is 989/990 with zero host timeouts, and only Stage 078 / seed 8849 loses after reaching 100% progress. Moving only Stage 078 Wave-5 HP from 1.25 to 1.10 restores 10/10 at 88.27% median progress, 83.86% median base and 14.11% worst base, preserving both its pressure grade and approved 3★. After fixture regeneration, Stages 079–099 pass 63/63 downstream runs with zero timeouts and 21.53% minimum base.
...
# 2026-08-31 · 付费套商业文案新口径

- **价值承诺改为数据可证**：前三套统一写明满配 `1.25×` 纯物理输出，黄金法则写明 Lv65 `1.556×`；四套均披露全元素弹药适配、各自专属机制、配套外观与追赶期逐级半价升级。
- **被推翻口径已清除**：不再把火弱 / 雷弱 / 冰弱、元素主宰或“终局 Boss 只认物理”当作购买理由；通用商店详情标题同步从“主宰区间”改为“整套优势”。
- **揭示门跟随 Owner 新表**：炼狱 / 雷霆 / 绝对零度 / 黄金法则依次为 30 / 50 / 70 / 90，黄金法则不再要求任意角色 Lv40。
- **审阅边界明确**：`design/audits/premium_commercial_copy_old_to_new_20260831.md` 逐条列出 72 个中英字段的旧→新对照；localization 文件未动，等待 Owner 批准。
# 2026-08-31 · 挑战曲线 Tier B 数值落地

- **单一来源**：`data/challenges.json.curve` 保存分段 smoothstep K、逐段三指数与参考 fixture 路由；`ChallengeRules` 和 Python 审计共用同一算法。K 从 1.25 单调升到运行时反解的 5.0，相邻增幅仍受静态连续性门禁约束。
- **体验合同改正**：挑战不再要求每关机械地升一档或参考样本 10/10。每章第 1/5/10 关代表点按十种子聚合验收，ch1–6 为 70–100%，ch7–10 为 60–90%；个别关允许处于普通同档的更高带内位置。
- **099 锚点**：黄金法则一档满级 9/10 通关，胜局 Boss 中位 201.017 秒，命中 60–90% 与挑战专属 150–220 秒。基地余量继续决定实际星级，但不再被误用为 K 的反解目标。
- **证据与回归**：99×10 最终证据共 990 条，十章代表点全部入带；第 9–10 章免费反例低于付费样本带。普通 099 前后十种子记录逐位一致，`levels.json` 未改，挑战星供给上限固定为 297★。

# 2026-09-01 · 付费主题枪口与宠物战场可读性

- **枪口根因与修复**：四套 `true_grip` 的旧枪口点仍停在肩部或枪身中段，与 380×520 的实际成图不一致。现已按四角色 × 左/中/右逐张重测 48 个枪管出口，并继续让单发、多弹道和武器多弹丸共用同一 `_weapon_fire_origin()`。
- **错误姿势纠正**：炼狱 `char_volt` 的左射源图实际朝右。新增数据化 `mirror_x_by_character`，运行时同步镜像人物、alpha 身体锚点和枪口空间，不对其他角色或主题做硬编码分支。
- **宠物存在感**：四只付费宠物由统一画布缩放改为按 alpha 可见高度归一至 148px，光环用世界空间 `0.58` 比例与 `-34px` 偏移，避免父节点放大造成二次缩放。宠物属性、技能、碰撞与战斗范围保持不变。
- **视觉证据**：五主题 × 四角色 × idle/left/center/right 共 80 路真实 1080×1920 战斗截图通过；人工逐张复核 48 个付费射击姿势与四只付费宠物，枪口、弹道起点、人物落脚、HUD 避让与双主体层级均正常。

# 2026-09-01 · 战区地图装备入口与章节卡收口

- **入口主体放大**：角色导航使用 alpha 主体对齐后的 118px 膝上裁切；其余装备图标在运行时裁掉透明画布留白，并把展示区从旧的窄内框扩至卡片主体。规则按当前选中数据行工作，覆盖免费装备及雷霆、炼狱、绝对零度、黄金法则四套。
- **左栏边缘解耦**：章节内容左边距从 32px 增至 48px，内部路线轨从 14px 收为 8px 并独立放在 x=24；“关卡”与“击破战区首领”不再落到轨道或外框上，双线也不再挤成一条脏边。
- **Boss 徽章与 CTA**：Boss 胶囊由 82×66 调为 92×68，图标、关卡号拥有独立内边距；章节按钮文字按扣除右侧箭头装饰后的有效内容区光学居中，保持既有 286×80 触控尺寸和滚动手势。
- **高屏证据**：四套付费主题分别以完整后期进度和对应整套装备生成 1080×2340 战区截图；四次运行时 UI 审计均为空，人工复核 065/070、075/080、085/090、095/099 全部无压框、裁切或按钮偏心。

# 2026-09-01 · 商店购买弹窗字号与按钮居中

- **排版层级**：商店长文案弹窗使用 700px 内容面板与独立的标题 / 正文 / 操作字号，避免全局 `FONT_SCALE` 把商品说明放大成连续展示标题；普通确认弹窗继续使用原默认值。
- **按钮居中**：取消与确认动作从受 StyleBox 内容边距影响的 Button 内建文字，改为覆盖完整按钮面的只读 `ActionLabel`，水平与垂直方向均按按钮外框中心对齐，触控目标和点击回调保持不变。
- **双语证据**：黄金法则升级包的中英文 1080×2340 购买确认截图均通过运行时 UI 审计；长文案完整显示，两个按钮文字无裁切、重叠或偏心。

# 2026-09-01 · 商店系列标题、主题与完整包归组

- **系列起点明确**：每套付费内容以“系列 / SERIES”标签、系列色竖轨和独立标题条开始；当前战区克制徽标移至第二行，避免英文长标题与状态争抢横向空间。
- **商品角色明确**：每张销售卡先显示“主题 / THEME”“完整包 / COMPLETE SET”或“军械升级 / ARSENAL UPGRADE”，再显示商品名；卡片左侧重复同一系列色竖轨，形成标题到商品的连续归属线索。
- **系列边界明确**：不同系列之间加入 34px 闭合分隔带，解决下一套标题被误读为上一套尾标的问题；商品卡仍是滚动内容的直接子项，购买、整卡详情点击和拖动取消合同不变。
- **视觉证据**：完整解锁状态下的黄金律中英文 1080×2340 截图均通过运行时 UI 审计，系列标题和两种商品类型完整可辨，无横向溢出。

# 2026-09-01 · iPhone 16 Pro Max 设置页溢出修复

- **根因**：攻速实验按钮由导出特性控制，场景初始隐藏；旧流程先按仅两个可见兄弟给舒适度按钮写入 440px 最小宽度，随后才显示第三个按钮，三者被固定横排后把 880px 内容栏和整张面板一起撑出安全区。
- **响应式结构**：舒适度容器改为 `HFlowContainer`。普通发行态仍是两个 432px 开关同排；实验态为两个 432px 开关同排，880px 攻速实验按钮自然换到第二行。入口可见性在布局和样式计算前确定，不再携带过时尺寸。
- **纵向与触控边界**：三个舒适度控件均为 88px 高；iPhone 高屏实验态把返回键从 96px 收到仍合规的 88px，为新增一行腾出空间，中英文操作说明保持原 144/148px，不通过裁字换空间。
- **真机比例证据**：`1080×2340` 加 `44/132/44/102` 调试安全边的中英文实验态均返回 `UI_AUDIT_JSON issues=[]`；无实验入口的 `settings_tall_safe_area` / `settings_safe_area` 与中英字体两路截图也全部通过。

# 2026-09-01 · 防线围城自动瞄准与弹道恢复

- **根因**：普通僵尸进入基地攻击线后，个别中心单位的碰撞体中心会与付费主题的真实枪口坐标重合。威胁评分仍把它选作自动目标，但炮塔把长度小于 1px 的方向判为不可发射，于是出现“人物朝正中、左右都有敌人、没有新子弹”的稳定死锁。
- **目标选择修复**：自动模式先计算敌人实际圆形碰撞半径与 24px 弹丸可见余量；只要场上还有枪口外目标，就在这些真实射线上按原威胁规则选敌。玩家手动拖动和点名锁定仍拥有原优先级，所有目标都贴身时也会回退到原最高威胁。
- **近身补射修复**：炮塔保留最近一次有效方向；目标与枪口完全重合时仍按该方向发射，战斗层继续用实际武器枪口和目标方向解析命中，避免仅剩中心敌人时永久停火。
- **复现证据**：黄金法则 `1080×2340` 左中右贴线场景修复前目标偏移为 `(0,0)`；修复后自动选择 `(-250,-4)` 的左侧真实目标并新增 14 枚弹丸。随后只传入中心贴身目标仍新增 14 枚弹丸，最终截图中人物左射姿势、枪口闪光和左向弹道完全对齐。

# 2026-09-01 · 战区逐关进度段可读性

- **语义确认**：章节卡“战区进度”下方的十段对应本章十个关卡的通关状态；上方 `10/10` 是汇总，短条是逐关分布，不是星级徽章。
- **根因**：旧实现把用于大面板的斜角九宫格压缩成十个 18px 高的小段，左右角饰占满可用宽度并相互覆盖，最终只显示为一排被截断的小三角。
- **修复**：逐关状态改用专用 14px 圆角短条，已通关使用章节强调色实心与亮描边，未通关使用暗色底与低亮描边；章节卡尺寸、进度计算、星数和点击行为均保持不变。
- **同屏溢出收口**：首轮截图暴露右侧底栏需要 486px、实际仅约 444px，导致“回顾战区”按钮越过卡片内框。最终按 890px 实际卡宽并计入按钮皮肤外圈重算：主按钮保持 280×80、两枚 Boss 徽章为 88×68、缩略图区为 300px、双栏 / 底栏间距为 12px / 4px；底栏改为左对齐，把全部富余宽度留作按钮右侧保护边，并继续满足 280×80 触控门禁。

# 2026-09-01 · 设置页开关文字安全边

- **问题**：开关控件沿用 28px 左内容边距，在 iPhone 高屏截图中“总声音”“减弱闪烁震动”“触感反馈”紧贴按钮左侧金属角，与其他居中按钮的留白节奏明显不一致。
- **修复**：统一开关表面的左内容安全边增至 48px；右侧 124px 开关轨预留、旋钮位置、按钮尺寸、响应式换行和触控区域均不改变。

# 2026-09-01 · 战区卡片左侧色条减法

- **问题**：章节缩略图本身已有一根章节色短条，卡片框层又绘制一根贯穿上下的 `ChapterRouteRail`，两根同色竖条并排错位，形成重复且突兀的视觉信号。
- **修复**：删除卡片级长色条，并在缩略图内部保留唯一一根 8px 宽、与缩略图等高的章节色短条；章节卡外框、内容边距、章节色文字、进度段和交互区域全部保持不变。

# 2026-09-02 · 战区头卡与章节内容精排

- **无限尸潮外框**：旧压暗层把一张自带边框的 `ui_panel_skin` 非等比拉到 920×184，再与正式 `EndlessFrame` 重叠，四角错位后读成斜框。压暗层现改为无边框纯色遮罩，动画背景仍为纹理，外框只由单一九宫格负责。
- **缩略图节奏**：新增 300×140 独立槽位，图片以 292×132 放在 `(-12, 8)`，利用左侧空当并在右侧为标题列多留 20px；短章节色条随图移动。
- **Boss 徽章**：底层 ID 继续使用 `level_065` 等三位格式，展示层转为整数 `65`；徽章内图标 40px、数字区 24px，在 88×68 外框内留下稳定空隙。

# 2026-09-02 · 出战配置防线完整度可读性

- **门槛框放大**：三星结果尺的三只门槛框由 42px 拉高至 58px，阈值文字由 14 提至 17，星标由 18px 放大至 22px；三框仍保持同排等宽，星级阈值和计算来源不变。
- **纵向节奏**：“防线完整度”标题字号由 15 提至 17，标题与门槛行的间距由 4px 增至 7px；战术摘要继续按渲染内容自适应增高，不侵占底部主操作区。
- **高屏复核**：1080×2340、44/132/44/102 安全边截图中三档门槛清晰可读，摘要外框完整，且与“开始战斗”按钮保留独立净空。

# 2026-09-02 · 在途伤害预判与多弹道覆盖

- **提前换靶**：每枚主武器弹丸在生成时记录目标、原始直伤、元素与穿透；自动瞄准以目标实时生命 + 护甲 + 护盾为尺，并通过敌人的只读伤害预估复用抗性、弱点和阶段闪避口径。当在途直伤已经覆盖剩余耐久，下一轮立即选择其他威胁，不再等命中动画结束。
- **安全释放**：预留只存在于活跃弹丸节点，命中、落空、闪避或超时销毁后自然消失；若场上所有目标都已被预计击杀，炮塔短暂停火等待结果，避免把额外子弹浪费在必死单位。玩家手动方向和点名锁定仍可强制集火。
- **多弹道覆盖**：多重射击继续使用原固定弹道数、等夹角和伤害衰减。自动模式枚举“至少一条弹道穿过主目标”的合法刚性扇面，按不同几何命中目标数择优；不会把每条弹道改成独立追踪，也不改变追踪弹自身合同。
- **验证**：确定性场景确认 100 耐久目标在第一发 50 伤害后仍被选中、第二发后切换，三弹道同时覆盖左 / 中 / 右三个目标且相邻夹角相等；Godot 载入、M1 smoke、资源包、数据、引用、压力与选卡检查全部通过。

# 2026-09-02 · 自爆僵尸基地伤害半径与反馈语义

- **音效没有回退**：`enemy_breach` 仍指向已重做的钢板挠曲 / 沙袋闷响资源。玩家听到旧式“撞防线”反馈，是因为死亡爆炸错误复用了物理接触分支，不是 WAV 被覆盖。
- **根因**：死亡爆炸视觉使用数据半径，但基地伤害用固定 `y > 1080`。标准防线位于 y=1500，这等价于给 160px 自爆圈偷偷扩成最多约 420px 的基地射程，造成远处爆炸、基地无动画却掉血。
- **范围单一来源**：自爆与毒云分别读取 `mechanic_params.radius` 的 160px / 190px；只有 `爆炸中心 y + 半径 >= 运行时防线 y` 才能进入基地伤害路径。视觉圈、邻近敌人判定和基地判定共用同一个半径。
- **因果反馈**：真实相交时显示向下传导轨迹、基地核心闪光、冲击环、HUD 震动和明确的“爆炸波及 / 毒云波及”浮字，并分别播放爆炸 / 腐蚀冲击音；物理僵尸或 Boss 真正接触防线时才播放 `enemy_breach`。
- **边界不扩张**：保持原 `bd_coef`、伤害比例、基地减伤、单次伤害上限、敌人数据和音频文件不变，仅修复可达性与反馈语义。定点验证覆盖 160 / 190px 边界，Godot 载入、M1 smoke 与两项音频门禁通过。

# 2026-09-02 · 角色详情滚动尾部与关闭键精修

- **底部裁切根因**：专属技能成长说明只有紧贴字体的最低高度，滚动内容末端又没有越过视口裁切线的净空；最后一行的字脚因此和区块底框相碰，看起来像被切掉。
- **可读尾部**：成长说明拥有 52px 最低行高且明确禁用文字裁切，专属升级布局增高至 178px；滚动内容末端再保留 44px 空白，使最后一行能够完整滑到固定操作栏上方。
- **真触摸滚动**：详情卡内所有非忽略控件使用 `MOUSE_FILTER_PASS`，升级按钮保留短按，同时将拖拽交给父级 `DetailScroll`；触发死区与商店详情统一为 12px，解决有滚动条却像拖不动的问题。
- **关闭键去斜框**：旧实现把为横向胶囊设计的 `ui_map_pill_skin` 压进 96px 方框，造成上下角视觉错位。现改用生产级方形图标框，按钮为 104×104、叉号 68px，交互态只切换同族激活框。
- **证据**：1080×2340 / 1080×2046 / 1080×1920 三高度，中英双语、四角色共 24 张运行时截图和 UI 审计全部通过；人工复核高屏钢铁先锋截图，成长说明完整、下一技能可见、关闭框端正。Godot 载入与 M1 smoke 通过。

# 2026-09-02 · 三选一技能卡完整文案与等高比较

- **根因**：卡片正文虽声明自动换行，但正文仍允许裁切，三张卡又各自按瞬时测量值独立定高；字体与容器尺寸稳定前，长句的第二行可能落在卡框之外，而短卡和长卡高度也不一致。
- **两阶段布局**：先让每张卡按当前语言、等级和实际字体测出自然高度，再把同一组三张卡统一为最大自然高度；正文保持完整测量空间，标签贴齐共同下基线。
- **裁切收口**：正文禁用裁字，智能换行不限行；推荐标签宽度由 152px 增至 164px，标题可用宽度由 286px 增至 292px，同时仍保留右侧安全边。
- **证据**：1080×2340 中英文常规长文案组合与包含“蓄能重击”的中英文定点组合共 4 张截图均通过 UI 审计；蓄能重击中文完整显示到“主弹威力”，英文完整显示到“primary-round power”，三卡等高且操作按钮未被挤压。

# 2026-09-02 · 全装备战术说明数据化

- **根因**：武器、护甲与芯片详情共用按类别写死的占位说明，宠物也只覆盖六只免费基础角色；因此实际数据中已经存在的射速、连锁、燃爆、碎冰、裁决、屏障反击和付费套联动没有传达给玩家。
- **单一事实来源**：详情页现在直接读取 `weapons / armors / chips / pets` 数据行生成“定位、核心特性、成长方式、使用建议”四段说明。元素、基础射速、弹丸 / 穿透 / 连锁数量、范围、触发次数、护甲修复、芯片附加机制、宠物冷却等均来自运行时同源字段，不另建会漂移的平行文案表。
- **覆盖范围**：12 件免费 / 付费武器、10 件护甲、12 枚芯片和 10 只宠物都进入同一生成合同；黄金法则额外说明 1-50 标准成长和 51-65 伤害延伸段，维修 / 经济宠物明确无攻击及其适用场景。
- **长文可达**：战术正文关闭裁切、增加行距，详情滚动接入 12px 触摸拖拽传递并保留 44px 底部净空；固定操作栏不移动，长说明可完整滚到其上方。
- **验证**：Godot 工程载入和 M1 smoke 通过；1080×2340 武器、护甲、芯片、医疗宠物、五类战斗 / 经济宠物及主题按钮共 10 张路由截图运行时 UI 审计为零问题，人工复核四段说明换行完整且未撑出安全区。

# 2026-09-02 · 散弹炮 3→4→5 弹丸成长

- **根因**：`weapon_scattergun.special.pellets` 同时被当作当前值和上限，战斗层没有读取永久武器等级，因此 Lv1 直接生成满级五弹扇面。
- **数据合同**：保留 `special.pellets = 5` 作为硬上限，新增 `pellet_growth` 阶梯：Lv1 三枚、Lv25 四枚、Lv50 五枚。通用解析器对未声明阶梯的武器保持原值，其他 11 把武器逐位不变。
- **叠加合同**：当前等级散弹数先在每条外层弹道内展开；多重射击和先锋弹幕齐射继续增加外层弹道。确定性断言确认 Lv1 裸散弹为 3 枚、满级散弹 + Lv1 多重为 10 枚、再叠先锋齐射为 15 枚。
- **界面同步**：收藏卡显示 `3→5 弹丸`，详情显示当前值与满级值，Lv24→25、Lv49→50 的升级提示增加“弹丸+1”，战术说明列出三个等级节点并说明可叠加。
- **验证**：`validate_data`、`check_localization`、`check_release_strings`、Godot 工程载入和完整 M1 smoke 全部通过；已知沙盒 `user://` 写入警告不影响最终通过结果。

# 2026-09-02 · 散弹高密度弹道等角排布

- **根因**：旧实现先生成 7° 间隔的多重射击外层扇面，再在每条外层弹道上展开 18° 散弹扇面。两套角度直接相加，在 7 / 9 / 10 / 15 枚等组合中会生成重合或仅差 2° 的弹道，表现为两发黏在一起、其余间距忽大忽小。
- **合成规则**：总弹数仍为“当前散弹数 × 外层弹道数”，最外侧覆盖范围仍为“外层跨度 + 散弹跨度”；运行时只把最终全部弹丸按该完整覆盖角一次性等角展开，不改变任何弹丸数量、伤害衰减、射速或机制叠加。
- **瞄准保持**：先沿用原自动覆盖优化 / 手动锁定取得一条命中目标的来源弹道，再把等角扇面中最近的一发精确旋转到该来源弹道；整组只做刚性旋转，所有相邻间距继续完全相等。
- **验证合同**：7 发与 9 发扇面逐相邻夹角必须一致；散弹 × 多重的 9 发组合也必须全程等距，且奇数扇面中央弹丸保持在瞄准方向。
- **验证结果**：Godot 工程载入、完整 M1 smoke、资产包、数据、资源引用、关卡压力、选卡模拟、本地化与发布字符串门禁全部通过；`git diff --check` 无格式问题。

# 2026-09-02 · TestFlight 1.0.0 (67)

- **发布前门禁恢复**：完整 RC 首轮拦截 7 处发行态 `ColorRect / StyleBoxFlat`，地图尸潮压暗层、章节进度段及商店系列轨道 / 分隔线 / 标签改为现有生产纹理和语义样式；次轮拦截散弹 `pellets` 未被战斗文件直接消费，战斗层现读取作者上限并约束等级解析结果。两类问题均在改 Build 号前安全停止并修复。
...
# 2026-09-02 · 散弹炮组合扇面均匀收势

- **实机根因**：Owner 截图中的十发齐射来自两条外层 lane 各自嵌套五发散弹；两组角度相加后，中间出现约 2° 的近重合间距，两侧则拉到 4.5°，因此视觉上成对拥挤、边缘松散。
- **等角合成**：保留总发数与“外层 lane 跨度 + 原生散弹跨度”的完整包络，把最终全部弹丸一次性均匀铺开；2×5 的 25° 包络现为九段约 2.78° 等距。
- **瞄准不漂移**：最终扇面只做整体旋转，将最接近原命中 lane 的弹丸重新锚定到目标方向；内部等距关系不会被破坏。
- **边界不扩张**：未修改伤害、射速、追踪强度、弹丸成长、总弹数、多重射击逐 lane 衰减或自动目标选择；完整 M1 smoke 通过。

# 2026-09-02 · 属性弹单局一次选择合同

- **规则**：武器初始属性不占用选择次数；属性弹尚未选择时继续沿用该武器原有候选范围，首次选择后则锁定该属性到本关结束。之后只会出现同属性升级卡，不会再用另一张属性弹替换。
- **根因与修复**：普通物理武器已有互斥候选过滤，但终焉武器的 `ammo_cards_override_base_element` 提前返回，绕过了已选属性检查。候选器现先处理“同属性升级 / 其他属性屏蔽”，再判断武器的首次换弹权限。
- **不变项**：没有修改四张属性弹的等级、伤害、状态、元素克制或武器初始兼容范围；规则随战斗技能实例自然按关重置。
- **验证合同**：终焉雷霆在未选择时仍可看到多种属性；选择冰霜弹后仅冰霜弹可继续升级，燃烧 / 感应电弧 / 剧毒弹全部退出候选池。

# 2026-09-02 · 技能选择卡正文与标签安全间距

- **实机问题**：全局字号 1.5 下，中文说明换成两行时，末行文字描边与下方小标签视觉重叠；原本共用的 8px 文案间距不足以形成清楚分区。
- **布局修复**：数值到正文仍用原 8px 紧凑节奏，正文到标签改为独立 16px 安全带；同时将每张卡底部无信息留白由 28px 收至 20px，因此没有扩大三张卡的最坏总高度或侵入英雄 / 基地区域。
- **回归合同**：M1 smoke 枚举 16 个技能的全部等级及中英文，检查真实换行高度、正文—标签安全带、标签—卡底净空和最坏三卡走廊上限。
- **视觉证据**：追踪弹 Lv2、冰霜弹 Lv2、伤害穿透 Lv1 的 1080×2340 中文定点截图通过运行时 UI 审计，正文末行与标签清楚分离。

# 2026-09-02 · 角色主动技能重复释放演出修复

- **根因**：角色专属 14 帧序列和开场闪光沿用战斗缩放时钟；在 5× 下约 0.6 秒的完整动画被压到约 0.12 秒真实时间，第二、第三次释放又叠在持续伤害反馈中，肉眼近似只剩掉血。
- **时钟分层**：仅角色主动技能的专属序列与开场闪光改用真实时间；伤害、冷却、脉冲 / 连击间隔、普通武器与敌人反馈仍随 1× / 1.5× / 5× 正常加速。
- **回归合同**：四名角色的首次和第二次释放都必须各自生成新专属序列；在 5× 时间缩放下，传入 0.5 秒战斗增量只推进 0.1 秒演出时间。

# 2026-09-02 · 散弹炮 3→4→5 最终弹丸成长

- **Owner 最终修订**：取代阶段 226 的 2→3→5 临时裁决；保留均匀等级分段，Lv1–17 为 3 枚、Lv18–34 为 4 枚、Lv35–50 为 5 枚。
- **运行时边界**：`special.pellets = 5` 仍是作者硬上限；等角收势、多重射击、先锋弹幕齐射、追踪和伤害参数全部不变。
- **验证口径**：M1 smoke 覆盖 Lv1 / 17、18 / 34、35 / 50 六个边界；遵照 Owner 指示不重算战力尺子。

# 2026-09-03 · F-X1 设置隐私正文

- 同步本地 main 至 648b040e；基线完整 release candidate 通过。只读素材例外经 Owner 同意，未读取主仓库在制代码。
- 英文隐私原需 215px，被 204px 固定盒裁切；改为自然高度（227px，含行间距），短屏按测量回收行间留白。字体和中英文原文不变。
- 六张复拍全部 issues=[]；同一 M1 断言覆盖三尺寸、双语、隐私 / 支持 / 帮助共 18 个状态。证据：`design/audits/visual_fixes_2026_09_03/x1/`。
- 测试配置记录：首次把截图只读环境带入存档写入测试导致失败；移除该配置后完整基线通过，原失败日志保留。

# 2026-09-03 · F-X2 武器卡长标题

- 标题与等级分开，英文名固定授权 37 号、中文名固定授权 43 号；不改任何名称、等级或属性值。
- 标题区由全武器的中英文正文 / 等级徽章测高，卡片、描述、行动按钮随同一高度增长；原有双语同高及标签间距合同保留。
- 16 张复拍均 issues=[]；极光英文 Lv.50 III 长名原先 37→29 且仍超宽，现可完整阅读。证据：`design/audits/visual_fixes_2026_09_03/x2/`。

# 2026-09-03 · F-X3 角色详情首屏与底部空间

- 信息顺序改为基础属性 → 被动 → 专属技能 → 升级预览 → 流派倾向，优先在首屏说明角色能力，未删改任何正文或成长数值。
- 等待容器完成本地化测量，再在首个放不下的完整信息块之前补分页留白；已开始滚动则不再挪动读者位置。
- 固定操作栏 202px，滚动尾部补 246px；M1 新增四角色 × 中英 × 四档高度的首屏完整区块和末块可达断言。证据：`design/audits/visual_fixes_2026_09_03/x3/`。

# 2026-09-03 · F-X4 配装摘要列宽
...
# 2026-09-08 · 全主题 UI 审阅（只报告，不修产品）

- 在 `codex/visual-fixes` 固定 `12e6e354c35a`，采集2236张逐像素离屏图；核心17状态×五主题×中英×三尺寸完整交叉，持枪240组合、人物详情120、已拥有装备详情64、本地演示购买完成24等另设专项。代码含Build70记录后的StoreKit提交，不称为已装手机Build70二进制实拍。
- 28项图证发现：11项优先显示/状态问题、17项表达/素材/排版优化，另1项真实StoreKit英文反馈风险。摘要、可视报告、102张五主题对照和全部原图索引在 `design/audits/ui_full_review_2026_09_08/`；分清普通布局和人工安全区压力、确认问题与审美建议。
- 完整非窗口 `check_release_candidate.py` 通过，m1零ERROR，包含纹理样式、文案、本地化等检查。2236份原图HEAD/尺寸/哈希一致；运行时仍79图226条提示、像素规则零提示，不能用静态绿代替人工UI结论。
- 本轮未改产品、文案、数值或确定性路径，未提交/推送；四段免费指纹与本地main Git对象一致。仅按授权只读主仓库历史资产源图；Godot串行、离屏静音、独立测试HOME，真实用户存档未读取/写入/删除。

# 2026-09-08 · UI 首轮自主修复候选

- 修复 UI-01/02/03/04/05/16/17/19/23/24/25/27，分为 12 个独立提交：`03f74180..01a5ee25`。范围为商店/地图/设置/收藏/结算、暂停布局及技能单位格式化；其余审阅项未擅自扩展。
- 520 张同配置复拍齐备，运行时提示由 58 图/187 条降至 4 图/4 条，原生像素检查零提示，四条残留与基线逐字一致。保留英文隐私/支持页人工安全区越界风险，不称全 UI 无问题。
- 最终非窗口 `check_release_candidate.py` 完整通过，含 M1 零 ERROR；新增显示回归断言零失败。报告与前后原图为本地交付材料：`design/audits/ui_first_fix_2026_09_08/`。
- `FONT_SCALE=1.5`、data 数值、存档/付费、弹道/瞄准不变；battle.gd 仅改 `_pause_skill_card`、`_pause_metric`、`_setup_pause_overlay_layout`，确定性路径逐函数核对未动。免费侧四段指纹仍与本地 main 一致。
- 真实存档未读写；所有捕获与测试使用独立 HOME。未 push、未制作 TestFlight，交 Owner 目视复核后再决定后续优化。

# 2026-09-13 · UI 原审阅剩余16项收尾

- Owner 要求全部修完，9月13日补充批准删除过时的“没有内购”及三项技能历史叙述、使用常规图像处理整理黄金芯片/天隼。当前19个分项提交 `02505b24..722fedc6`；旧图逐字节归档，素材源图/prompt/处理清单入索引。
- 完成标题和等级分组、按钮组合居中、霓虹可见框、武器展示清晰度、短详情、图鉴摘要、长英文选卡、商店状态归组、人物绿边及波次条避让。英文选卡正文从20px提高到26px，正文宽584→644；未改FONT_SCALE或其它授权字阶。
- 首次全量试拍虽无机器提示，人工发现霓虹可见框仍受透明残像影响，主动中止final01。补修单层纹理绘制及可见alpha测量，12张中英三尺寸pilot07通过；最终聚合60项与m1零ERROR通过，专用回归零失败。
- 现于固定 `722fedc6` 进行final02全量2234图/2236逻辑用例复拍；报告在 `design/audits/ui_finish_2026_09_08/`。整批未结束前不计作最终视觉通过。
- 44个确定性相关函数、战斗数值、弹道/瞄准、core/save及core/commerce均未改；四段免费指纹与本地main Git对象一致。DEVICE-01仅代码与消息断言已修，真实交易仍待iOS沙盒。Godot串行、离屏静音、隔离存档；未push、未合并主仓库或打包。

# 2026-09-14 · 武器保留原样与UI最终续验

- 按Owner最新决定撤回本轮新加的武器主题染色，复用既有高清展示图；原型、数值、弹道、已验收套装与局内武器皮不改。另补商店已拥有卡18px留白和宠物真实冷却小数，23个分项提交终点`b32841b7`。
- 发现旧`check_res_refs.py`将路径硬编码到主仓库，导致以往聚合中的引用项查错目录。该调用只读但越过预期隔离边界，已记录并改为工具所属仓库，当前隔离树454个引用通过；没有降低检查标准。新版本完整非窗口60项及m1零ERROR通过，补充UI断言零失败。
- 基础截图批被主动暂停并封存源码；1007张合格记录中793张不受追加修改影响可保留，26张退出资源清理错误排除重拍。最新固定版本续拍1441张，覆盖全部商店/宠物/配装及未完成场景，最终目标2234图/2236逻辑用例；源码/函数范围证明组合图证等价，不称同一HEAD整批。
- 最终1441张续拍全部完成，源码守卫通过；与793张未受影响合格图组合为2234张/2236逻辑用例，原79张/226条运行时提示归零，原生像素规则0提示。生成28项前后报告、126张同屏对照及原图检索目录，机器全量与人工抽样分开记录。证据、桌面副本校验及文档HEAD见`design/audits/ui_finish_2026_09_08/`。
- 44个确定性函数和四段免费侧指纹仍一致；未接触真实存档、未push/合并主仓库/打包。23项代码修复另加文档证据提交，真机支付仍待iOS沙盒。

# 2026-09-14 · UI-13选卡状态补修

- Owner在SE人工复看发现红色世界名牌跨选卡面板顶边；原UI-13避让范围漏了弹窗状态。新增整层隐藏/关闭恢复，覆盖新名牌、重抽和嵌套详情，不改敌人本体及LOD判断。
- 顶部边界以波次条实际底边加48画布像素计算；长卡组合使用1020宽紧凑布局及真实卡高，收回等高填充，避免按钮推到防线。保留字号、文字和卡内/操作间隔；宽裕高屏仍用原972宽。
- 完整非窗口聚合发布门禁通过，m1新增名牌/顶部间距断言且零ERROR；三尺寸中英专项零失败。全部78张选卡/详情复拍通过；2张退出资源清理失败归档后重拍通过。证据design/audits/ui13_card_modal_2026_09_14，桌面17目录。
- 5个战斗UI函数变更，44个确定性函数逐字一致，免费侧四段指纹未变；无数值/武器/瞄准改动，独立测试存档，未push或打包。旧大包保留历史版本，不冒称自动检查此前已覆盖此缺陷。

# 2026-09-17 · 战区首领徽章可读性与长按解释

- Owner指出两枚首领徽章的图标/数字挤边且含义不明。徽章从88×68增到112×80，图标保比例收紧占位、内部间距4→8；章节主操作右移，继续保留原生286×80和字号。缩略图原图/尺寸不变，只收回其布局槽未用右空位；小屏压力验证按钮距卡片右缘≥30。
- 新增局部`chapter_boss_badge.gd`手势观察器：长按0.5秒显示对应首领与关卡编号，PASS保留滚动；移动12px或开始滚动取消。提示复用纹理样式、按内容测高且定位安全区，测量期间透明防闪；松手可继续阅读约4秒，点击/切页关闭。
- 专项以真实SubViewport输入验证短按、触摸长按、触屏模拟鼠标流实际拖动、锁定大首领第99关、鼠标长按、切页隐藏；三尺寸双语零失败。首轮发现窄安全区宽度分配需改进，已修复并保留失败迭代，最终42张截图零审计问题。
- 文案只新增一条中英说明；未改已有英文语义。免费侧四段哈希与本地main及688fc89d均相同，gameplay/完全未改。所有Godot测试使用隔离HOME/只读存档，没有碰Owner共享存档。证据：`design/audits/map_boss_badges_2026_09_17/`。
- 最终完整非窗口发布门禁60项通过，包含纹理原语、资产、对比度、本地化、release strings及m1冒烟零ERROR；42张最终图全部无ERROR/审计问题。桌面18_战区首领徽章_2026_09_17提供中文前后对照入口、五主题同屏和逐图校验清单。

# 2026-09-18 · 战区环境图加高

- Owner要求删除每卡重复口号并把空位让给图片。移除ChapterObjective，缩略图槽140→182、图片132→174，宽度与图片资源不变，继续使用KEEP_ASPECT_COVERED。独有摘要、关卡范围、状态、首领长按及主按钮不变。
- m1新增十卡无重复说明、292×174图片、保持比例裁切断言。截图工具仅补地图LevelScroll定位，让审阅图可从第一战区展示，不改变游戏默认滚动定位。
- 五主题三尺寸双语30张加前段战役6张，36张最终复拍零审计问题、日志无ERROR。完整非窗口聚合60项通过，m1通过；重点人工复看中文高屏与英文SE，未见新增裁切或压边。
- 证据`design/audits/map_environment_art_2026_09_17/`，桌面19_战区环境图优化_2026_09_17。日期保留任务开始日；未改数值/战斗/字体，隔离只读存档，未push或打TestFlight。

# 2026-09-18 · 关卡列表紧凑信息行与固定返回

- 根据Owner截图，关卡标题改为统一30基准字号（全局1.5后47），智能换行而不再fit缩字；推荐战力/弱点在HBox同行并由内容决定宽度。操作区按卡片真实安全宽度定位，中文尽量维持左右结构，英文超宽时按钮另起一行并撑高卡片。
- 页顶新增96×96钢青纹理返回箭头；标题两侧等宽占位保持居中，长战区名换行。返回不随列表滚动，原章节卡中的返回仍保留。
- 三尺寸双语专项覆盖99关的完整标题、字号、同行徽章、模式按钮范围、变体标签、滚动返回和总览隐藏，594个组合零失败；m1新增固定返回与标题/信息行断言。
- 第一轮发现安全区导致中文卡片多余堆叠，经实拍改为动态右侧操作区后复核；保留iteration01，不冒充最终图。最终五主题章节页30张、末战区/新玩家/总览18张，共48张零审计问题，完整非窗口发布门禁60项与m1零ERROR。
- 证据`design/audits/map_level_layout_2026_09_18/`，桌面20目录含原生截图、历史参考与中文复看说明。无英文改写，无战斗/数据修改，免费四段指纹与main一致；独立测试存档，无push、合并主线或TestFlight。

# 2026-09-18 · 闪电少女从空手待机改为持枪准备姿势

- Owner指出停火张手、开火突然出现枪。定位为普通武器组合idle/hurt仍沿用空手放电底图，并非射击武器丢失。加载时对Volt复用各武器attack_01无枪焰准备姿势，保持4拍待机/3拍受击接口，呼吸和受击运动仍由原rig驱动。
- 新增实例表现标记，仅将其idle/hurt身体定位映射到center；未改中心/左右枪口、射击8帧和F2起火、瞄准及其它角色/付费持枪母版。旧资产原位保留，无新图、无资产索引/数据改动。
- 八普通武器×三瞄准方向×三次射击结束/受击切换72轮零失败，枪口坐标严格一致；五外观两状态80图＋散弹枪小屏/高屏4图，84张原生截图零审计问题。源代码边界工具证明44个确定性相关函数未变，四段免费指纹一致。
- 完整非窗口门禁60项通过，含m1新增断言且零ERROR。证据`design/audits/volt_armed_idle_2026_09_18/`；桌面21目录附84张原生图、HTML状态放大对照与中文修补说明。静默离屏、独立只读存档，无Owner存档写入；不push、不打包。

# 2026-09-18 · 其余三角色同类持枪待机问题

- Owner要求排查另外三人；原素材确认钢铁先锋/火焰少年握拳但未握枪、冰霜少女摊手，受击亦回空手。将已验证的Volt规则统一应用普通武器组合，复用各自attack_01无枪焰持枪准备姿势；八帧开火、呼吸/受击运动、付费持枪母版保留。
- 运行时只有`_load_character_animation_frames`的角色限定条件改动。44个确定性相关函数逐字一致，data/和assets/无差异，免费侧四段哈希与本地main一致；不改数值、弹道或瞄准。
- 四角色八普通武器、左中右三方向、连续三轮开火/停火/受击，共288轮零失败，枪口位置对比一致。m1将上一轮Volt专用断言扩展至全部普通组合。
- 三个新修角色八普通武器两状态48张、四额外外观24张、高屏/SE12张、Volt回归2张，86张最终截图零问题；另有3张修前对照。不是主题×武器×尺寸的全笛卡尔积。完整非窗口发布门禁60项及m1通过，日志无ERROR/check failed。
- 证据`design/audits/heroes_armed_idle_2026_09_18/`，桌面22_全角色持枪待机_2026_09_18。页面用CSS放大原截图，附中文说明及原图清单；隔离HOME、只读捕获，无Owner共享存档写入或删除。无英文改写，无push、主线合并或TestFlight。

# 2026-09-18 · 武器跨主题保持原貌

- Owner指出旧全量包霓虹武器仍有青紫涂装。9月13日仅撤了新增配装shader，历史主题icon/handheld/turret解析仍在；本次统一原始路径，移除收藏/共享图标/配装及独立战斗武器surface材质，不改人物战衣/合成动画。
- 商店八武器预览取消主题素材和10%accent混色；主题描述删除武器涂装承诺，主题SKU副标题展示改取主题目录。价格/权益/商品数据未改，完整包/升级说明未改。Owner新决定显式追加到设计主方案，历史资产不删除。
- 主题测试五主题十二武器四类路径240项原貌一致，共享图标无shader/混色；m1增加商店逐武器原图/原色断言。完整非窗口聚合60项通过；66张中英三尺寸代表截图，五主题首把武器原生内部像素一致。首轮商店16图命名带tall但尺寸为标准屏，已归档并正确命名重拍。
- 人工另发现商店详情在左右各44px安全区压力测试下固定四列和底部操作撑宽，记录待单独修补，不把自动审计为空当作全界面无缺陷；本轮只收敛武器外观。
- 证据`design/audits/original_weapon_identity_2026_09_18/`，桌面23_武器保持原貌_2026_09_18。运行时battle只取消独立武器surface，44个确定性函数不变；data仅themes描述改写，免费四段指纹一致。隔离只读存档，不push、不打包；中英文案清单见修补说明。

# 2026-09-18 · 主菜单三主题中英标题留白

- Owner指出霓虹、极光、黄金标题文字贴框。文字与框均烘焙在PNG内，单纯放大TextureRect不能修复相对留白；本轮通过内置图像编辑扩大暗色牌面和框内空间，保留中英标题内容及各主题材质方向，六张原路径替换、旧图逐字节归档。
- 黄金中英中央菱形仍贴字的候选未收用，经实拍追加修正后重新拍摄完整矩阵；初轮截图保留iteration01。最终标题图含真实透明alpha；黄金裁切从旧1040×340窗口更新为1536×1024全画布，再由既有光学裁切代码处理。
- 无菜单代码/控件、slogan、FONT_SCALE、按钮、武器、人物或战斗修改。图像编辑导致字形材质有细微变化，并非逐像素保留原字再机械扩框；不宣称烘焙标题文字实际显示像素高度完全不变。
- 五主题×中英×1080×1920/1320×2868/750×1334，修前后各30张、零审计问题及原始日志ERROR。人工复看小屏中英、标准屏和高屏代表组合；六资产归档/透明度/源一致性专项通过。完整非窗口聚合60项与m1通过。
- 证据`design/audits/menu_title_clearance_2026_09_18/`；桌面24_主菜单标题留白_2026_09_18含六张最终速览、60张原生前后图、中文说明及哈希清单。四段免费指纹与本地main逐字相同；只读隔离存档，不push、不合并主线、不打包。无中英文案改写。

# 2026-09-18 · 第一战区当前布局复查与收紧

- Owner从9月14日旧全量包指出稀疏关卡卡片。核实55cbff28已含a80b2820的大标题/推荐弱点同行/固定返回，但介绍卡仍固定400且故事目标间有弹性空白，英文因元信息较长将两个模式按钮整体下沉；本轮从当前版本重新拍修前48图。
- 介绍卡消除页顶已展示的重复标题，故事与目标整宽自然换行，去掉行数截断；进度与286×80原生返回在底部同行，内容撑高，第一战区310。既有导航>=172顶部、>=56底部断言保留。英文长元信息改全宽底行而不是整个动作区下移，变体徽章在标题下方；中文保留紧凑双栏。
- 不缩全局/局部字号，不变触控尺寸、普通/挑战语义或解锁规则，无原文改写。m1增加完整正文位于页脚之上、进度/返回同行且不相交的断言，以及容器换行稳定所需帧。
- 最终专项按结构识别节点，硬断言2970关卡组合（99×5主题×3尺寸×中英）零失败；初版以名字前缀漏计重复打开首章的300项，已明确归档为非全覆盖。修前后48张均零审计问题与原始日志ERROR，人工复看黄金中英、小屏/高屏及默认标准屏。完整非窗口聚合60项及m1通过。
- 证据`design/audits/chapter_layout_recheck_2026_09_18/`；桌面25_战区布局复查_2026_09_18含96張原生前后图、黄金三尺寸中英对照与中文复看说明。免费指纹四段与本地main一致，data/assets/gameplay/core未改；独立只读存档，不push、不合并主线、不打包。
