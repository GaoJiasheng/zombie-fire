# 19 · 经济·难度·经验技能 三轴重平衡（已实现，星价于 2026-07-27 再平滑）

> 本文记录当前实现。Owner 于 2026-07-27 明确要求取消 100–230 星的陡峭单件价格；现行门禁以 §2.3 的 8–16 星舒适带为准。

## 0. 已锁定决策
1. 金币、经验、星 **获取随关卡线性增长**;武器/装备/角色 **升级费随等级线性**(取代 1.15 指数)。
2. 金币 **线性但仍有压力**(收紧系数,见 §2.1)。
3. 星 = **消费货币**（购买扣除），商品总价 **318★**；普通战役 297★ 后只需 21 颗挑战星完成全收藏。
...
### 2.3 星：消费制 + 平滑档位（总 318★）
解锁逻辑改为**购买扣星**(去掉 `save_manager.gd:168-188 _refresh_star_unlocks` 的"阈值自动解锁",新增 `purchase_item()`)。**默认免费仅 vanguard + autocannon**;其余全部按下表:

| 类 | 物品:★ |
|---|---|
| 主炮（autocannon 免费） | flamethrower 8 · cryocannon 8 · scattergun 9 · teslacoil 10 · venomlauncher 10 · railgun 14 · plasmacannon 16 |
| 角色（vanguard 免费） | blaze 10 · frost 12 · volt 16 |
| 护甲（全付费） | kevlar 8 · thermal 8 · cryo 9 · faraday 10 · hazmat 11 · reactive 14 |
| 芯片（全付费） | attack 8 · haste 8 · crit 9 · health 9 · guardian 10 · pierce 11 · greed 11 · element 14 |
| 宝宝（全付费） | turret_drone 8 · fire_imp 9 · frost_wisp 10 · volt_orb 11 · medic_drone 13 · collector 14 |

合计 **318★**（主炮 75 + 角色 38 + 护甲 60 + 芯片 80 + 宝宝 65），最贵单件 16★。
- 每个品类的最高付费价不超过最低付费价 2 倍；价格表达强弱与稀有度，但不制造上百星收藏墙。
- 战役上限 297★ → 可买到几乎全部收藏；额外 21 颗挑战星即可全收。
- 第一件护甲 + 芯片为 16★，前几个章节即可形成完整配装。
...
### 4.4 挑战模式补星（已实现）
- 普通战役 297★ 后只需 21 颗挑战星完成全收藏；挑战是完成度奖励，不再承担 593 星的长期战力缺口。
- `data/challenges.json` 与 `save.challenge_progress` 已独立记录每关挑战最高星，重复完成只补历史差额，不可无限刷星。
...
| 星 | 单件 8–16；普通 297 后再取 21 挑战星全收 | ✅ 平滑尝试、挑战收尾 |
...
5. 5 个收藏 JSON：`unlock_cost_star` 使用 §2.3 平滑档（含 kevlar/chip_attack 不免费）。
...
- 星价只允许在 8–16 舒适带内微调，并保持同品类最大/最小不超过 2 倍、全收藏约 318 星。
