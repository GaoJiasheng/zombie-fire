...
- [ ] 主题不再含武器涂装 → ASC 四个主题内购描述(“gun colorway/枪械涂装/赤焰涂装”等)与 `design/32` 需同步改掉;`data/store_products.json` 主题行旧副标题已不再显示,可择机清理。
- [ ] 已知非阻断:商店主题详情仅在左右各 44px 安全区压力配置下右溢(竖屏真机左右 inset 为 0)。
- [ ] 提审时 ASC 版本页构建需从 70 换成 72;Owner 真机沙盒支付五步与 DSA 信息仍未做。
