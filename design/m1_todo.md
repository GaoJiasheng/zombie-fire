...
## 一级跳弹首目标重撞修复（2026-09-01）

- [x] 弹射子弹继承首个已确认命中的目标 ID，避免出生在 70px 敌人碰撞体内时再次撞回原目标。
- [x] M1 smoke 增加真实战斗生成链回归：一级跳弹只生成 1 枚子弹、首目标不二次受击、邻近目标实际受伤。

## 战斗卡牌详情弹窗 FONT_SCALE 1.5 适配（2026-08-30）

- [x] 详情遮罩改为复用已经受战场走廊约束的选卡面板，不侵入英雄、基地或底部 HUD。
- [x] 全等级、描述、标签和关闭按钮按本地化文字的真实高度顺次布局，移除详情文字的自适应缩字路径。
- [x] M1 smoke 零 ERROR；16 技能 × 中英双语详情视觉门禁 32 路全绿，三尺寸双语证据图 6 张零审计问题。

## 2026-08-30 · C 阶段战力 6.0 / 付费套 v2（部分完成）

- [x] 战力 6.0 纯构筑恒定读数、B2 反解推荐值、走廊校准器只读化与黄金测试落地。
- [x] 付费武器弹药卡单通道覆盖；切换后仅本命元素机制自然停用。
- [x] Tier B 中性纯物理合同重标：三套 1.25x；黄金法则 Lv50 1.251x、Lv65 自然成长实测 1.556x。
- [x] 通用 `level_growth_segments` 接入运行时、SaveManager 镜像与 Python 标尺；黄金法则 51–65 使用标准 `+0.08/级` 攻击成长且基础攻速停在 50 级，免费武器无 segments 精确旧公式护栏。
- [x] 揭示阶梯 30/50/70/90、黄金法则上限 65、完整包/升级包 US$8.99/US$6.99。
- [x] v3 老存档迁移后一次性显示中英文战力标尺说明；v4 新存档不误弹，确认后不重复。
- [x] 1080×1920 / 1320×2868 的中英文关卡、配装、结算 12 张离屏截图通过尺寸、UI 审计、图像检查与高屏人工抽看。
- [x] 四套普通 L99 各 10/10、确定性 A/B 逐位一致、12 张视觉矩阵与非视觉 RC 主链归档；免费 99×10 零泄漏对照使用付费改动前的同合同提交 `4be071fb`。
- [x] 黄金法则二次返工以当前 4a fixture 同输入重跑修前/修后免费 99×10，原始 990/990 完整 run 对象逐位一致、0 timeout。
- [x] 黄金法则 1.556x 新终值重新跑普通 level_099 固定十种子，10/10 胜利、0 timeout。
- [x] 非视觉 RC 除两份 B2 已缺失的美术源 manifest 外通过；基线提交 `f8361dfb` 同样不存在这两份文件，按历史资产缺口披露。
- [x] Owner 方案 B 追赶折扣：付费装备仍从 1 级起步，权益到账时冻结玩家最高既有武器等级；追赶段逐级成本乘 `0.0005`，超过冻结等级恢复全价，免费 1178 项成本向量逐项不变。
- [x] 商业化投影改用追赶等级并显示整套追平金币；等价分位恢复为配装 34/69（49.3%，旧 47.2%）、结算 17/69（24.6%，旧 25.0%），阈值从 economy 单一来源读取。
- [x] 经济影响实测：四套在揭示点追平整套分别需 83 / 206 / 214 / 249 金币，占奖励到账后金币 1.74%–5.04%；`check_economy_loop` 与 XP 覆盖门禁保持原值。

## 2026-08-31 · 付费套商业文案重写

- [x] 四套 `premium_sets.json` 中英商业字段统一改为纯物理输出倍率、全元素弹药适配、专属机制、外观与追赶期半价升级；删除元素弱点 / 主宰区间 / 终局 Boss 只认物理等旧承诺。
- [x] 解锁阶梯对齐 30 / 50 / 70 / 90；黄金法则移除旧角色 Lv40 附加门槛。
- [x] 逐字段中英旧→新清单归档；localization 文件保持不变，等待 Owner 审批。
- [ ] Owner 批准后同步 localization 对应条目。

## 属性弹丸视觉同步（2026-08-24）

- [x] 普通物理武器选择火焰、冰霜、闪电或毒素弹后，同步切换弹丸模型、颜色、尾迹、枪口光与命中特效。
- [x] 未选择属性弹时保留原物理弹外观；原生属性武器及付费武器专属弹丸外观保持不变。
- [x] M1 smoke 覆盖四种物理转属性组合，以及物理原貌、原生属性和付费专属外观三类反向保护。

## 关卡配装返回子战区（2026-08-24）

- [x] 从子战区关卡进入配装页时保留当前章节上下文，点击返回后恢复原子战区，不再跳到总战区地图。
- [x] 配装页统一按 `return_to + return_payload` 返回，保留结算页返回与直接打开配装页的原有语义。
- [x] M1 smoke 锁定“子战区 → 配装 → 返回原子战区”链路及十关列表恢复。

## 战役真实成长与战线推进基线（2026-08-21）

- [x] 新增资源受限免费账号推演：只使用普通关 2★ 首通所得金币、经验与星星，并复用运行时升级成本
- [x] 新增 99 关战线推进审计，以 50% / 80% / 基地边缘和基地掉血划分四档，不读取推荐战力定难度
- [x] 固化 `design/audits/campaign_frontline_baseline.csv` 并接入 RC 新鲜度检查；本阶段零战斗 / 数值改动
- [ ] 阶段 2：Owner 确认章节目标档位后，先平滑真实难度，再重标有效战力 / 推荐战力

## 商店推荐位跟随当前战区（2026-08-17）

- [x] 从最高已解锁关所在章节的十关 `primary_weakness` 动态计算众数，匹配的已揭示、
  未拥有付费系列置顶并显示中英文「当前战区克制」角标；未硬编码章节元素映射
- [x] 毒弱等无付费覆盖章节保持原排序且无角标；已拥有系列保留原有可见路径，但不会被推荐
  或重复渲染
- [x] 第 6 章雷霆 / 第 8 章炼狱两组中英文 1080×1920 截图布局审计零问题；M1 smoke
  锁定推荐切换、无覆盖、已拥有排除与商店滚动点击回归

## 绝对零度卖点与 55 关揭示门（2026-08-17）

- [x] 「主宰区间」中英文改为碎冰连爆、减速控场及首领减速上限下仍有效的机制卖点，
  不再暗示 80 关后的冰弱覆盖
- [x] 绝对零度商店揭示门从 80 调整为 55；价格、付费数值、主题外观与真实难度均未改动
- [x] 54 / 55 关显隐与动态 30 / 50 / 55 / 99 空态档位均由 M1 smoke 锁定；五张中英文
  1080×1920 截图布局审计零问题，发布文案与本地化门禁绿色

## 战败 / 1★ 结算页量化购买引导（2026-08-17）

- [x] 战败与 1★ 结算复用配装页同一个付费套装战力投影，只在已解锁未拥有且投影
  `R >= 1.20` 时追加内嵌量化建议；2★ / 3★ 与无尽结算绝不展示
- [x] 文案明确「升级到与现役同级后」并展示完整有效战力变化，点击提示卡直达对应商店系列，
  返回时保留原结算状态
- [x] 中英文战败 / 1★ / 2★ 六态 1080×1920 截图均无裁切；M1 smoke 与本地化门禁覆盖显隐、
  数值门槛、已拥有排除及路由
...
## 阶段 167 · 雷霆完整套合同上限发布余量（2026-08-15）

- [x] 按 Owner 数值决策，仅将 `set_apocalypse_thunder.target_full_set_ratio_max` 从 `1.58` 放宽为 `1.60`；雷霆武器、护甲、芯片、宠物与套装机制数值零改动。
- [x] `audit_character_endgame_dps.py` 继续从 `premium_sets.json` 同源读取上下限，不保留第二份硬编码带宽；炼狱 / 绝对零度仍为 `[1.52,1.58]`，黄金法则仍为 `[1.90,2.05]`。
- [x] 雷霆完整套实测 `1.579x` 落在新合同 `[1.52,1.60]` 内，上限余量从 `0.001` 恢复到 `0.021`，且对玩家实际战斗零影响。

## 阶段 168 · 英文结算英雄行右缘收口（2026-08-15）
...
## 阶段 171 · 图标式弹窗关闭按钮可读性统一（2026-08-16）

- [x] 盘点所有右上角图标式关闭入口，统一覆盖商店商品详情、图鉴人物 / 技能 / 装备详情与角色外观选择页；底部“完成 / 返回”文字按钮保持原规格。
- [x] `UiKit` 新增共享关闭按钮合同：可视按钮至少 `96×96`，叉号统一 `56 px` 并保持水平居中；各主题原有装甲边框与交互色不变。
- [x] M1 smoke 锁定三类页面的图标、最小按钮尺寸与叉号字号，防止后续主题或局部页面退回过小规格。
- [x] 商店、人物图鉴和外观页实图通过 `1080` 宽运行时审查，标题未被挤压、叉号不贴右缘。

## 阶段 172 · 商店滑动与点击手势彻底分流（2026-08-16）

- [x] 修复 iOS 滚动序列中 `ScreenDrag` 已发生、但最终抬起坐标仍接近按下点时，商品整卡被误判为点击并打开详情的问题。
- [x] 商店页面级手势裁决完整跟踪触摸 / 鼠标的按下、移动、拖动与抬起；位移一旦超过与滚动容器一致的 `12 px` 阈值，本轮所有商品与操作按钮激活均作废。
- [x] 商品购买、详情购买、整套装备、撤销演示购买和装备升级统一走防误触入口，滚动不再触发弹窗、详情或资源操作。
- [x] M1 smoke 新增 iOS“拖动事件移动、抬起坐标回到起点”的反例，并验证同一位置静止点击仍能正常进入商品详情。

## 阶段 173 · 出战人物入口与战术摘要安全边（2026-08-16）

- [x] 出战配置的战术摘要新增独立内容安全区，左右分别保留 `34 / 30 px` 内边距，标题、属性格、配装行、建议与星级规则不再挤贴装甲边框。
- [x] 人物卡取消“头像只换外观、底部名字才换人物”的分裂点击区；整张人物卡统一进入人物图鉴，并以“人物 / 外观”明确标示入口用途。
- [x] 从出战页进入人物图鉴后继续保留人物详情里的“选定”与“外观”两个动作，返回时仍保留当前关卡、挑战模式与结算来源。
- [x] 中英文入口文案入库；M1 smoke 锁定摘要双侧净空、人物卡全尺寸点击区、无旧换装覆盖层，以及人物详情同时具备选择与换装动作。

## 阶段 174 · 技能三选一图标净空与放大（2026-08-16）

- [x] 技能卡图标框左侧净空由 `16 px` 增至 `32 px`，不再压住卡片左侧强调条与装饰边框。
- [x] 图标框由 `176×176` 放大至 `196×196`，图标本体由 `156×156` 放大至 `178×178`；保持等比居中，不裁切技能图案。
- [x] 右侧文字栏同步右移至 `x=252`，与图标框保留 `24 px` 间隔；中文数值、标题、说明和标签均未被挤压。
- [x] 英文长标题保留自适应字号，英文说明使用均衡双行排版；中英文技能三选一实图与运行时裁切审查通过。
- [x] M1 smoke 锁定图标左侧净空、放大标尺、图标内嵌关系、图文间隔与右缘安全边。

## 阶段 175 · 配置图鉴列表统一大图标体系（2026-08-16）

- [x] 武器、护甲、芯片、宠物四类配置列表统一使用 `860×256` 宽卡片、`176×176` 图标与 `x=248` 文字轴线；不再保留宠物 / 护甲 / 芯片的 92 px 小图标窄卡片分支。
- [x] 技能图鉴同步使用 `860×256` 卡片，技能图案由 `80×80` 放大到 `156×156`，外框统一为 `176×176`，图文间隔保持至少 `30 px`。
- [x] 所有列表标题、语义标签与说明文字共用右移后的起始轴；购买 / 装备 / 详情按钮保留独立安全区，不与放大图标或长文案重叠。
- [x] 中英文武器、护甲、芯片、宠物、技能共十张运行时截图通过裁切与手工视觉复核。
- [x] M1 smoke 锁定五类列表的卡片宽度、图标尺寸、左侧净空、统一文字轴和图文间隔。

## 阶段 176 · 人物详情技能图标统一放大（2026-08-16）

- [x] 人物详情的被动、专属主动与弹种技能统一改用 `120×120` 图标框，技能图案由 `64×64` 放大到 `104×104`，不再沿用旧的小图标规格。
- [x] 技能行高度同步增至 `148 px`，图标与文字间隔增至 `20 px`；标题、类型、说明字号与文案保持不动，长说明继续在原有滚动详情内自然换行。
- [x] 所有技能图案继续等比居中显示，缺图兜底也使用相同大图标标尺，避免不同技能类型出现大小跳变。
- [x] M1 smoke 锁定所有人物详情技能行共享同一图标组件、放大标尺、图文间隔与等比居中规则。

## 阶段 177 · 角色列表膝盖裁切与全主题模型对齐（2026-08-16）

- [x] 角色列表卡片扩展至 `860×330`，人物展示区由 `220×282` 放大至 `320×310`；人物主体按 `465 px` 统一标尺渲染，不再为了露出完整脚部而缩小。
- [x] 四名角色统一采用头顶线 `y=6` 与画布中心线对齐，展示区在膝盖附近裁切下半身；头部、肩甲和上身成为主要视觉层级。
- [x] 主体边界使用稳定透明度阈值，忽略电弧、火焰、披风尾迹等低透明特效对缩放的干扰，避免不同主题和姿势把人物拉偏。
- [x] 默认、霓虹、炼狱、极地与黑金人物资源共用同一对齐算法；文字栏同步右移并保留 `26 px` 图文净空。
- [x] M1 smoke 锁定卡片尺寸、膝盖裁切、主体高度、头顶线、画布中心线与四角色一致性。
## 阶段 178 · BOSS 贴墙近距离瞄准稳定（2026-08-16）

- [x] 人物左右姿态选择改用固定的人体中心参考点，不再用当前姿态的枪口位置反推下一姿态，消除 BOSS 贴墙时左右互相触发的反馈振荡。
- [x] 左右姿态使用分离的进入 / 退出阈值；真实子弹仍从当前姿态的枪口直指目标，命中与伤害逻辑不变。
- [x] M1 smoke 从左 / 中 / 右三种初始姿态反复瞄准同一个贴墙目标，必须稳定收敛为同一方向。

## 阶段 179 · 基地屏障技能与护甲拦截视觉解耦（2026-08-16）

- [x] 开局大蓝色基地屏障只跟随已获得的“防线屏障”技能，不再因为护甲带有 `breach_shield` 拦截次数而提前出现。
- [x] 护甲原有的一次性漏怪拦截数值保持不动；触发时仍有瞬时命中 / 破碎反馈，但不会持续冒充尚未获得的技能屏障。
- [x] M1 smoke 覆盖“护甲拦截为 2、技能未获得”组合，锁定持续屏障必须隐藏；获得技能后仍正常显示。

## 阶段 180 · 暂停页大字信息与三栏操作（2026-08-16）

- [x] 战场状态、出战配置与已带技能统一提高标题、字段、数值、技能名和等级字号；技能图标同步增大，保持信息层级一致。
- [x] 暂停弹窗二次真机可读性收口：信息区标题、字段、数值、技能名与等级再次放大，信息 pill 和技能 chip 同步增高；底部三按钮增高至 154px，改为大图标 + 短文案横排，并按五套主题最厚边框统一保留上下左右安全区，避免图标或文字压住装饰边框。
- [x] 信息内容区左右各收回 `24 px` 空白，卡片阅读宽度由 `836 px` 扩展至 `884 px`；板块间距与内部纵向留白同步收紧。
- [x] 底部操作改为左 / 中 / 右同排的高触控按钮，短文案统一为“继续 / 重开 / 退出”，英文为 `Resume / Restart / Exit`。
- [x] M1 smoke 锁定三按钮同轴顺序、宽高、短文案、装甲框安全区和信息区宽度。

## 阶段 181 · 局内技能栏四列换行（2026-08-16）

- [x] 底部被动技能栏由每行五个改为每行最多四个，第五个起自动进入第二行；后续技能继续按四列向上增加行数。
- [x] 单卡 `96×120`、技能图标、等级字号与触控范围保持不变，只把技能栏实际宽度由 `512 px` 收窄为 `408 px`，避开人物主体区域。
- [x] M1 smoke 锁定四列宽度、第五张换行、跨行左对齐和人物中间安全区；当前八技能上限与战斗数值均未改变。

## 阶段 182 · Boss 百分比抗性与亡骸泰坦回血修复（2026-08-16）

- [x] 8 个 Boss 移除属性 / 物理硬免疫，统一改为数据驱动 `resistances` 百分比减伤；当前抗性减伤 50%，弱点增伤 50%，所有合法伤害属性都能实际扣血。
- [x] 钢铁泰坦装甲由“前 30 次物理零伤害”改为“破甲前物理减伤 50%”；每次命中仍推进破甲，破甲后恢复全额物理伤害。
- [x] 修复亡骸泰坦隐式继承每秒 2.5% 最大生命回血的问题：改为显式每秒 0.4%，任意伤害暂停 0.8 秒，火焰弱点伤害暂停 3 秒，持续火焰伤害也会刷新压制。
- [x] Boss 血条显示弱点 `+50%`，抗性命中显示“减伤 50%”；血量刚开始下降时使用自适应小数精度，不再因整数四舍五入长期显示 100%。
- [x] GD / Python 战力投影、终局审计、全 99 关合同检查与 schema 同步百分比抗性；两冻结锚点比值和短板零漂移，旧硬免疫反例由 `0.50x 抗性 / 1.50x 弱点` 元素倍率合同替代。
- [x] M1 smoke 覆盖亡骸泰坦雷电正常伤害、火焰 1.5x、毒素半伤、再生压制，以及钢铁泰坦装甲半伤 / 破甲与终焉霸主多抗性。

## 阶段 183 · 火焰少年主动技由全屏回收为大范围爆区（2026-08-16）

- [x] 根据 Owner 实机反馈撤销“每段无条件命中全场”的过强覆盖，`sig_blaze_meltdown` 改回数据驱动 `local`，锁定高威胁目标并围绕目标连续爆燃。
- [x] 基础半径由旧局部版 `260` 提高到 `340`，兼顾尸群命中数量与空间判断；爆区外的远处敌人不再同步掉血。
- [x] 单体伤害、段数、冷却与灼烧倍率保持不动，覆盖类型和半径继续不进入 Boss 单体战力，因此既有“有效战力 / 推荐”口径不漂移。
- [x] 中英文技能说明同步改为“大范围爆区”；M1 smoke 锁定目标与邻近敌人受击、远距敌人不受击，以及覆盖模式不改变显示战力。

## 阶段 184 · 重装 Boss 双层装甲与幽影轻量化（2026-08-16）

- [x] 钢铁泰坦、亡骸泰坦与终焉霸主改为真实装甲耐久：原 `hp_coef` 总耐久按 `30% 装甲 / 70% 本体` 拆分，不在原生命之外额外堆血。
- [x] 无伤害穿透时先扣装甲；“伤害穿透”按当前百分比越过装甲直击本体，装甲击破时溢出伤害继续进入本体；旧 `armor_hits` 次数制只保留兼容路径。
- [x] Boss 头顶与全局 Boss HUD 均显示金色装甲条 + 红色本体条；全局标题同步显示 `甲 / 血`（英文 `AR / HP`）百分比。
- [x] 虚空幽帝 `hp_coef` 由 `46` 降至 `36`，让闪现 / 相位成为主要威胁而不是与厚血叠压；重生成 55 / 60 / 65 关合同后，55 关 Owner 构筑为 `271/287, R≈0.9430, 短板防线`。
- [x] 数据校验限制装甲占比范围；M1 smoke 锁定总耐久不膨胀、普通伤害先破甲、50% 穿透双路分伤、破甲溢出、双血条布局和幽影生命系数。

## 阶段 185 · 战力 4.0 真实漏怪防线标定（2026-08-16）

- [x] 删除按推荐等级三次方增长、与实际漏怪无关的防线门槛；改为由 `simulate_balance` 同源预计漏怪伤害、参考基地生命和二星剩余生命阈值生成逐关防线合同。
- [x] 高输出缩短基地接触时间只按小怪 / Boss HP 份额给予 `0.85–1.15` 有界修正，避免旧模型错杀高输出构筑，也禁止输出无限重复折算成防御。
- [x] 重生成全 99 关 `bottleneck_v4` 合同；2–98 关节奏族保持 `R=[0.9700,1.3800]`，本阶段验收时 99 关免费毕业锚点为 `2724/2340, R=1.1643, Boss`；阶段 186 的固定 Boss 数量编队已接管后续绝对刻度。
- [x] 80 关截图构筑按永久技能 L1/L2/L3/L4 复算为 `902/1037/1157/1626`，推荐固定 `926`；最低档进入兜底带而非严重不足，L2 起正常过线。
- [x] 极低等级无防具反例仍为 `110/926`，85 关半速反例仍为 `R=0.4850`；M1 smoke 与 Python 合同校验锁定新公式和 `+15%` 上限。
- [x] 完整公式、锚点退役理由与验证结果记录于 `design/34_power_v4_runtime_line_calibration.md`；真实战斗难度与付费数值零改动。

## 阶段 186 · Boss 固定身份耐久与数量编队（2026-08-16）

- [x] 8 个 Boss 全部增加 `fixed_hp`；同型号在普通战役的单只生命 / 装甲总耐久固定，不再随关卡、晚波或局内卡压力暗涨。
- [x] 20 个 Boss 关以显式数量表达进阶：前五族按 1 / 2，只以机动见长的虚空幽帝按 1 / 2 / 3，亡骸泰坦按 1 / 2 / 3，终焉霸主按 1 / 2 / 3 / 4。
- [x] 多 Boss 通过 `runtime_bosses` 左 / 右 / 分散通道错峰入场；99 关不再混入钢铁泰坦，改为四只完全同规格终焉霸主。
...
## 阶段 188 · 角色详情技能图标方形放大与左缘安全区（2026-08-16）

- [x] 被动、专属主动与弹种技能统一使用 `148×148` 方形框和 `128×128` 等比素材，替代会随文字行纵向拉伸的窄框。
- [x] 每条技能行增加 `12px` 左侧内容安全区；图标从安全起点向右扩展，与文案保持 `22px` 间距，不再压住面板装饰边框。
- [x] M1 smoke 固化方形尺寸、左缘净空、纵向居中和等比缩放；四名角色中英文 1080×1920 共 8 张详情截图全部通过视觉门禁。

## 阶段 189 · 配置图鉴全宽卡片与二次图标放大（2026-08-16）

- [x] 武器、护甲、芯片、宠物和技能目录卡片由 `860` 扩展至 `896 px`，完整利用 `904 px` 内容安全区并为滚动条保留 `8 px`，不再把卡片撑出屏幕。
- [x] 五类目录图标框统一由 `176×176` 放大至 `204×204`；技能图案由 `156×156` 放大至 `184×184`，其余装备图标继续等比居中。
- [x] 文字轴统一由 `x=248` 右移至 `x=272`，标题、标签和说明共用同一轴；购买、装备和详情按钮同步右移并保留右缘安全区。
- [x] M1 smoke 锁定 `896 px` 宽卡、`204 px` 图标、`x=272` 文字轴和按钮边界；中英文五类目录共 10 张 1080×1920 截图通过运行时与图片审计。

## 阶段 190 · 散弹枪多重射击追踪生效（2026-08-16）

- [x] 根因确认：普通追踪弹统一直飞 `1.0s` 后才导引；散弹枪运行时弹速较低，外侧 pellet 往往在导引启动前已命中、越过目标或飞出侧边，实机等同于“追踪不生效”。
- [x] 普通武器继续保留 `1.0s` 直飞合同；只有带原生 pellet 的散弹追踪使用 `0.35s` 起导，仍遵守 `460px` 最小转弯半径，先看得见散射扇形，再及时开始转向。
- [x] 自动射击按追踪技能等级限制同时覆盖目标数：Lv1–5 最多稳定覆盖 1–5 个不同存活目标；弹丸在这些目标间按原方向和威胁轮转，目标失效后沿用既有最近目标重寻逻辑。
- [x] 手动锁定继续拥有最高优先级，所有追踪弹集中攻击玩家点名目标；伤害、散弹 pellet 数、多重射击 lane 数及既有逐 lane 衰减均未修改。
- [x] M1 smoke 锁定散弹前 `0.20s` 保持扇形、`0.35s` 后开始导引、Lv3 覆盖 3 个不同目标及手动锁定全弹集中；gameplay polish 增加静态合同护栏。

## 阶段 191 · 战区主操作按钮纵向扩容（2026-08-16）

- [x] 战区总览卡片的“继续推进 / 回顾战区 / 进入战区”由 `268×54` 次级规格提升为 `284×80` 主操作规格，利用右侧状态栏的空余宽度并显著增加可见触控面积。
- [x] 主操作文字提升至 `20px` 并保持全框垂直居中；未解锁提示仍使用弱化配色和较小字号，不改变战区门控逻辑。
- [x] 章节详情“返回战区地图”经 Owner 复核后同样提升为 `284×80` 主导航规格；它与上方进度框、章节卡片底边分别保留明确净空。
- [x] M1 smoke 固化总览与详情两类主按钮尺寸、卡片边缘净空、文字居中及滚动手势透传。

## 阶段 192 · 技能卡自动换行与动态高度（2026-08-16）

- [x] 局内三选一卡片由固定纵坐标改为“数值实测高度 → 说明实测高度 → 标签实测高度”顺排；自动折行和手写换行走同一套布局，不再只识别 `\n`。
- [x] 卡片及三选一面板按真实内容增高并限制在战场顶部控件与基地线之间；中文最长三卡和英文最长三卡同时出现仍完整保留按钮净空，不遮挡人物 / 基地。
- [x] 技能图鉴的高等级效果摘要使用卡片剩余文字区，角色详情与武器 / 护甲 / 芯片 / 宠物语义说明同步加入实际字体高度检查。
- [x] M1 smoke 遍历 16 个技能的全部 76 个已编写等级和中英文，共 152 张局内卡；锁定数值、说明、标签顺排、卡片包裹及最坏三卡组合容量。中英文 1080×2340 实际截图通过视觉审计。

## 阶段 193 · 11–99 关波次自适应增压与 XP 回调（2026-08-16）

- [x] 新增可复跑波次生成器：前十关完整哈希冻结；11–99 关只调整目标波次 `spawns.count`，结果为 `77` 关全额、`7` 关部分、`5` 关不动。
- [x] 普通模式保持 `13×3★ / 86×2★ / 0×1★`，并把逐关增压后不降星、2pp 边界保护和前十关零改动接入 Release Candidate。
- [x] 基础 / 专属技能 L5 成本回调至 `8500 / 11000`；首通 XP `154,881`、三次通关 XP `271,084`、全满成本 `335,000`，覆盖率 `80.92%`。
- [x] 重生成全 99 关三短板合同；走廊 `R=[0.9700,1.3800]`，099 与 080 实战锚点比值 / 短板保持不变。
- [x] 第十章挑战机制倍率仅从 `1.24` 最小回调至 `1.21`，消除 level_099 的 100% 破线；挑战模式无不可胜关、无超 cap。

## 阶段 194 · Boss 减速抗性与同型叠只提速（2026-08-17）

- [x] `economy.boss_pacing` 统一登记普通怪 / Boss 减速上限 `80% / 40%`；减速力场五级数据不动，战斗运行时按 Boss 身份夹取，战力防线轴按小怪 / Boss 暴露份额加权镜像。
- [x] 11–98 关同型 Boss 耐久按第 1 / 2 / 3 / 4 只使用 `1.0 / 0.82 / 0.45 / 0.35`；第二只从初值 `0.60` 回调到 `0.82`，避免 level_075 跌出已批准的 60 秒下限。前十关始终 `1.0x`。
- [x] 099 由生成器按 `180s` 目标反解四只 Apex 为统一 `0.983097x`，散弹枪毕业族全程 `180.0s`；自动机枪 / 磁轨炮 / 散弹枪均在 `460s` cap 内，Apex 单体 `116.6s` 合同不动。
- [x] Boss 里程碑体验带全部进入 CI：45–70 关 ≤60s、75–85 关 60–100s、90–95 关 100–170s、99 关 `[150,185]s`。
- [x] 重生成全 99 关战力合同；走廊保持 `R=[0.9700,1.3800]`，099 锚点保持 `R=1.1706/Boss`，080 Owner 锚点保持 `R=1.7558/Boss`；普通星级、挑战可胜性、XP 与金币合同零漂移。

## 阶段 195 · 配装页付费克制量化推荐（2026-08-17）

- [x] 只对主弱点匹配、系列已解锁、军械未拥有且实算提升 ≥15% 的付费套展示推荐；已购、未解锁和属性错配全部隐藏。
- [x] 四件套逐槽按 `min(现役同槽等级, 套装上限)` 投影，并通过现有有效战力管线输出升级前后完整数字，不引入营销估值。
- [x] 免费建议与付费建议可同时显示；付费行明确「升至现役同级」前提，点击直达对应商店系列并保留配装返回路径。
- [x] M1 smoke 锁定三项触发条件、已购短路、逐槽折算、装备恢复、完整数字和商店路由；中英文有推荐 / 无推荐四张 1080×1920 截图通过视觉复核。

## 阶段 196 · 局内三选一空白区回归修复（2026-08-18）

- [x] 移除高屏幕对三选一弹层的额外纵向拉伸；弹层高度只由三张技能卡的实测内容决定，高屏幕仅参与战场可用区内的上下居中。
- [x] 卡片容器同步收缩到真实卡片总高，第三张卡与重抽 / 跳过按钮固定保留 `62px` 操作净空，不再出现整段无内容黑区。
- [x] M1 smoke 新增内容高度、卡片容器高度、操作净空与战场居中四项护栏；全量 smoke 通过。

## 阶段 197 · 暂停页后期技能溢出与三按钮常驻（2026-08-18）

- [x] 修复携带 7–10 个技能时“已带技能”区突破旧固定高度、覆盖“继续 / 重开 / 退出”操作行的问题。
- [x] 暂停内容区改为按真实最小高度二次定版；技能按每行 3 个动态增加到 4 行，面板与操作行同步下移并保留 `32px` 净空。
- [x] 三个暂停操作按钮强制保持可见且绘制在动态内容之上，避免后创建的内容层再次遮挡退出路径。
- [x] M1 smoke 新增 10 个数据技能的后期压力态，锁定四行换行、内容容量、操作净空、绘制层级及面板包裹。

## 阶段 198 · 无尽模式三线解耦与难度重整（2026-08-18）

- [x] Phase 1：Boss 改用生成式轮次总预算，完全脱离战役 `fixed_hp` 与小怪复利；预算按既有同型折减权重分摊，池升档、首轮抗性宽限和首轮护甲上限不变。
- [x] Phase 1：小怪改为 `20% / 28% / 38%` 分段复利，第 8 轮 `4.348×`；Boss 数量改为每 4 轮增加一只、上限 6，全部从 economy 单一来源读取。
- [x] Phase 1：可复跑生成器、RC stale-check 与 M1 运行时护栏入库；战役 `levels.json` 字节零改动。
- [x] Phase 2：击杀金币按显示轮数线性 `+12% / 轮`；每 5 轮一次性发放
  `50 × 里程碑序号` 金币，并复用金币飞行 / 资源脉冲 / 中英 toast。经济审计锁定
  20 轮金币 / 分钟严格递增且第 10 轮不超过第 1 轮 3 倍。
- [x] Phase 3：双 fixture 无尽离线模拟、存活轮带与 Boss 阶段体验带接入 CI，并完成 1080×1920 手测证据。

## 阶段 199 · 局内三选一偶发纵向拉伸修复（2026-08-20）

- [x] 根因确认：重抽或连续弹出三选一时，旧卡仅调用 `queue_free()`，在帧末真正删除前仍参与 VBox 高度统计，导致新旧两组三卡被同时计入并把面板偶发拉到战场高度上限。
- [x] 卡片换代改为“先从容器移除、再延迟释放”，布局阶段始终只看当前三张卡，不再依赖 Godot 帧末删除与 deferred 重排的先后顺序。
- [x] M1 smoke 增加同一帧连续两次重建压力态，立即及下一帧分别核对卡片数量、内容高度、按钮净空与战场居中。

## 阶段 200 · 近战线追踪穿透弹反向开花修复（2026-08-20）

- [x] 根因确认：多弹道追踪弹汇聚命中近战线目标后，若没有可用后排目标，剩余穿透仍沿各自入射方向越过目标，视觉上反向散成贯穿全屏的扇形弹道。
- [x] 追踪穿透改为仅在存在真实后续目标时继续飞行；无后续目标时在已确认命中点消散。普通直线穿透和可实际命中后排的追踪穿透均保持不变。
- [x] M1 smoke 同时锁定“无后续目标必须收束”和“存在后排目标必须继续追踪”两种路径。

## 阶段 201 · 后期 Boss 曲线与战力 5.0 对齐（2026-08-20）

- [x] 70–99 关 Boss 改为稳定单体耐久的数量阶梯：亡骸泰坦 `1/2/3/4` 只后，终局霸主按 `1/2/4` 只升级；85 关不再提前出现单只终局 Apex 尖峰。
- [x] 战役同型 Boss 全部使用完整 `fixed_hp`；原同型折减只保留给无尽轮次总预算分摊，99 关四只 Apex 毕业战保持 `183.1s`。
- [x] 新增可复跑武器真实碰撞产能生成器，清群 / Boss 轴由 economy 单一来源供 GDScript 与 Python 共用；推荐标题删除重复 Boss 数量计价。
- [x] 重生成 99 关合同：80 关回放 `1038/989, R=1.05/Boss`，99 关 `1667/1437, R=1.16/Boss`，走廊仍为 `[0.9700,1.3800]`。
- [x] 新口径、编排表和历史取代关系记录在 `design/38_late_boss_curve_and_power_v5_alignment.md`。
- [x] 战力 5.0 实测轴同步重生成无尽 Boss 预算；第 `7–10` 轮用 `2%` 桥接、第 `11–20` 轮线性接力，中期 / 毕业撞墙轮为 `11 / 20`，且 +12% 金币/分钟合同继续严格递增。

## 阶段 202 · 战役难度重建与战力 6.0 方案冻结（2026-08-21）

- [x] 依据资源约束免费首通基线，完整记录当前 `82 轻松 / 7 略有压力 / 0 压力 / 10 高难` 的锯齿问题与现有逐关走廊掩盖模型失配的证据。
- [x] 在 `design/40_campaign_difficulty_rebuild_and_power_ruler_v6_plan.md` 冻结“真实难度先行、战力只描述已验收难度”的实施顺序、算法边界和停工条件。
- [x] 提出首轮 `16 轻松 / 43 略有压力 / 33 压力 / 7 高难 / 0 不可胜` 目标、章节配额、逐关档位序列、Boss 时长带及关键节点模拟目标；全部明确标记为待实施验收数字。
- [x] 战力 6.0 方案改用资源成长三轴的连续单调标尺；玩家能力与关卡要求彻底解耦，逐关走廊降级为只读门禁，`R≈1.0` 固定为“有压力但稳定可过”。
- [x] 拆出合同冻结、真实难度生成、战力重标、运行时复核四阶段，并列出数据、连续性、随机种子、既有 RC 和视觉门禁；本阶段未改关卡 / 经济 / 装备 / 战力数据。

## 阶段 203A · 战役节奏目标与成长样本合同固化（2026-08-21）

- [x] 将 design/40 v2 的逐关目标档、档位带和章节配额写入 `campaign_pacing_targets.json` 单一来源；保留 `frozen=false`，B1 Owner 验收前只报告差值、不判红。
- [x] 将资源约束免费首通成长样本抽到 `campaign_progression_fixture.json`；购买、升级、武器选择、固定卡牌种子和 10 个校准关由审计与后续运行时探针共用。
- [x] 战线审计把“难度高但可过”与“不可胜”拆开，当前基线如实记录为 `82/7/0/4/6`，并输出 99 关当前 / 目标 / 差值表。
- [x] 导出逐关运行时注入构筑，记录 `levels.json` SHA-256 `7429c0ac…d3f3e`；本提交未修改关卡、经济、战力或星级数据。

## 阶段 203B · 战线模型固定帧运行时校准（2026-08-21）

- [x] 新增真实 `Battle` headless 探针，固定 1/60 秒物理步，复用阶段 203A 的逐关构筑与三卡牌种子，自动选卡、施放主动技能并记录推进 / 基地 / 胜负 / 用时。
- [x] 卡牌随机流与战斗随机流显式隔离；相同关卡与种子重复运行产物逐字节一致，不再受无渲染环境墙钟与界面帧差异影响。
- [x] 完成 003 / 008 / 013 / 030 / 040 / 055 / 062 / 075 / 084 / 095 共 10 关 × 3 种子校准；旧模型推进误差中位 22.91pp、档位一致 6/10。
- [x] 裁决前三关矛盾：真实推进中位 24.55% / 30.65% / 26.40%，均 3/3 满血通关；问题来自抽象 capacity 被误当逐帧 DPS，并非前三章数据异常。
- [x] 将固定帧真实战斗中位定义为 B1 前的权威折算，种子范围作为方差带；旧解析模型降级为差值筛查器。校准报告保持待 Owner 认可，目标分布继续不冻结。

## 阶段 203C · A2 攻速实验档工程落地（2026-08-21）

- [x] 在 economy 单一来源落地 `control / tier_a / tier_b` 三档；正式包恒为 control，TestFlight 实验 feature 才显示切档入口。
- [x] control 保留原乘区与运算顺序，M1 smoke 锁定参考构筑射速 `3.740233728`、普通弹丸 DPS `314.179633152`，确保默认档零行为漂移。
- [x] tier_a / tier_b 分别封顶 `1.8× / 2.2×`，削减 DPS 以 50% 折回单发伤害；齐射、芯片、角色弹幕、宠物与过载均按同一 profile 缩放。
- [x] 逐发状态用 control/current 射速比归一期望触发量，减速与点燃参考覆盖率相对 control 偏差 0%，门禁保持 ≤5%。
- [x] GDScript、战力模型、平衡模拟和终局 DPS 审计支持同一 profile；多重射击夹角、战役数据、经济和星级口径均未改动。

## 阶段 203D · A2 三档并列审计基线（2026-08-21）

- [x] 复用唯一资源成长 fixture 生成 Tier A / Tier B 两份并列 99 关 CSV；Control 正式基线不替换、RC 新鲜度语义不变。
- [x] 输出三档 99 关群体 / Boss DPS 曲线、战线档位分布及 055 / 075 / 095 节点对照，供 B1 决定最终攻速档。
- [x] 将并列报告 `--check` 接入 RC；生成器连续运行结果稳定，避免 profile 或 fixture 改动后审计产物过期。
- [x] 如实登记 Tier A 的 099 清场时间约 `486.9s`、超过当前 `460s` 的实验风险；未放宽门禁、未修改关卡或正式档。

## 阶段 203E · B1 类别池保底与 051 结构停工（2026-08-22）

- [x] 落地数据门控的 `offer_category_floor`：候选缺少压力类别时替换最低优先级项，
  类别映射复用 `economy.probe_card_policy.category_tags`；未启用关的三选一序列逐位不变。
- [x] GDScript / Python 战力投影同步支持类别池最弱兼容卡保守期望；M1 smoke 覆盖启用与
  未启用两条路径。
- [x] Tier B 固定三种子完成 051 类别池诊断，确认发牌类别和节奏已达标，但现有三个调节
  杠杆无法同时满足 W1 `≤42%`、最终 L 档与末两波定档。
- [x] Owner 后续批准波级 `hp_coef` 与现有敌人 / lane 的纯数据组合包络；运行时、离线模型、
  校验器与生成器已完成同源候选，051 十种子达到 10/10，051-054 Tier B 三种子中位进入 L 带。
- [x] 055 完成 Boss+支援与 Boss-only 裁决：三固定种子均失败；删除全部支援后种子 1103
  仍为推进 95.56%、基地归零、Boss 未击杀，证明阻塞来自当前冻结的固定 Boss 本体合同。
- [x] 资源样本武器策略升级为 v2：按下一章众数弱点购买星星武器、逐关直接调用
  `power_for_build` 选出战武器、换枪追级最多使用当关金币预算 40%；99 关新旧时间线已归档。
- [x] 055 样本策略 v2 正式裁决：真实样本为散弹枪 Lv43 / 特斯拉 Lv12，Tier B
  `power_for_build` 为 `909 / 145`；特斯拉强制追到 Lv43 仍只有 `508`，两把满级时仍为
  `1012 / 620`，因此规则不会选择雷电武器。
- [x] 055 Tier B 十种子门禁：`0/10`，推进中位 `95.54%`、基地中位 `0%`；Boss 阶段
  中位 `43.68s`（`42.88-44.78s`）已在 `40-60s` 内，故没有改 055 独立预算，也没有重解
  虚空幻影模型基础血量。
- [ ] B1 第 6 章重建、派生合同重钉与 TestFlight：等待 Owner 在“让免费雷电武器的成长 /
  战力在 055 matchup 真正超过散弹枪”与“重新授权 055 基地压力杠杆”之间裁决；10/10 与
  40-60s 门禁保持不变。

## 阶段 203F · B1 第 6 章沙盒收尾（2026-08-24）

- [x] 收拢按波 `hp_coef`、类别池、资源样本 v2、固定帧提速与并行 sweep 工具链；未写
  `hp_coef` / 未启用类别池的关卡保持零行为漂移。
- [x] 虚空幻影完成模型级三参数联合解：`380k / bd_coef 0.85 / 物抗 25%`；055/060 Boss
  阶段分别为 `50.5s / 46.0s`，065 只变易且无尽 11/20 轮合同保持。
- [x] Tier B 051-060 全十关、十种子全部 10/10，档位序列命中
  `L/L/L/L/P/L/P/P/P/P`；Boss、基地余量与时长四合同全部通过。
- [x] 重生成 ch6 clear requirement、星级、走廊、wave-pressure、level-pressure 与 smoke
  钉值；新增十个 3★ 仅来自 ch6，走廊继续处于 `[0.97,1.38]`。
- [x] 完成 Control / Tier A 信息复核与 051 三组类别池对照；Tier A 的 059 `2/3` 作为
  TestFlight 试玩红旗保留，不静默放宽实验档。
- [x] 重写试点报告、归档最终证据并清理 scratchpad；目标表继续 `frozen=false`，等待 Owner
  试玩确认压力爬坡、055 运气墙消失与 Tier A/B 选档。
- [ ] B2 前产品决策：免费雷电武器在 055 matchup 中的成长收益仍不具竞争力；B1 未改武器。

## 阶段 204A · B2 冻结合同与毕业族门禁（2026-08-24）

- [x] 按 Owner 验收结论冻结 Tier B、目标分布 `16/43/33/7/0`、章节配额与第 6 章数据哈希；
  `campaign_pacing_targets.json` 置为 `frozen=true`，后续生成器不得改动 051-060。
- [x] 新增“满级免费毕业族”单一 fixture：所有免费槽位满级、逐关调用正式 matchup 战力择优，
  与按节奏样本并列扫描 99 关。
- [x] 冻结合同与毕业族扫描接入 RC；B2 重建前 095 / 099 的不可胜结果保持可见，待 061-099
  重建完成后将扫描切为永久硬门禁，要求 99/99 在时长上限内通关。
- [ ] B2a：按冻结的 Tier B 流水线完成 061-099、Boss 主场三参数反解与免费元素武器后段重标。

## 阶段 204B · B2a 第 7–10 章收口（2026-08-26）

- [x] Tier B 固定帧探针完成 061-099 共 39 关 × 10 种子，390/390 通关、0 超时；第 6 章与 001-060 关卡数据未继续调整。
- [x] 恢复死灵泰坦主场模型解；075/080/085 不再追改共享 Boss 参数，099 仅用关卡编排保持 153.367 秒终局带。
- [x] 重钉 061-099 的 clear requirement、星级期望、走廊、wave-pressure、level-pressure 与 M1 smoke，并生成逐关旧→新对照。
- [x] 输出刀锋关表；095 最差种子基地余量 1.41%，登记为“B2b 终验复查项：余量 <2% 定点加厚”，B2a 不再调参。
- [x] 清理调参中间件，仅保留终版 390 场证据、合同对照与报告；完整非视觉 RC、无尽 CI 与 TestFlight build 63 纳入发布收口。
- [ ] B2b 前置：001-050 重建后必须全量重生成成长 fixture；因击杀/金币变化造成 061-099 少量档位漂移时，仅做计划内定点复验与微调。

## 阶段 204C · B2b 成长样本全量重生成（2026-08-26）

- [x] 基于 build 63 的当前 99 关数据幂等重生成按节奏成长 fixture，并输出 99 关旧→新槽位、等级与资源对照表。
- [x] 用新 fixture 完成 061-099 的 Tier B 三种子快扫；仅登记 26 个终验修补候选，不在第 1-5 章重建前追调后期关卡。
- [x] 明确同种子失败集中在 066 / 067；068 / 080 / 085 / 095 / 099 的基地余量变化纳入 B2b 全量终验复查。
- [ ] 按冻结章节配额重建 001-050；随后再次全量生成 fixture，再处理 061-099 的最终修补清单。

## 阶段 204D · B2b 第 5 章与母体家族收口（2026-08-29）

- [x] 041-050 在 Tier B / v2.1 / 十种子下达到 100/100 通关并命中冻结章节档位。
- [x] 瘟疫之母冻结为 450k / bd 1.5 / 抗毒 50%；045 与 050 均为 10/10，Boss 阶段中位 55.35s / 55.37s。
- [x] 050 用有效约 11s 双母体错峰、15 个喷吐者与末尾 1 个基础步行者达到推进 94.10%、基地 83.84%，最差基地 9.67%。
- [x] 无尽 Boss 预算 current，无尽模拟通过；041-050 clear requirement 与 runtime-owned wave/star 合同已重生成。
- [ ] 完成全量合同重生成与攻速默认 control→tier_b；随后生成星表和消费口盘点，停等 Owner 审批。

## 阶段 204E · B2b 工序 B 合并裁决（2026-08-29）

- [x] 合并工序 B 的 34 关有效 repairs；以 Tier B / v2.1 对 34 关各抽验 3 种子，累计 102/102，且强制覆盖 066 / 067 / 089。
- [x] 作废 010 / 015 / 020 的错误 40 秒下限修补：010 / 020 原值分别 21.03s / 30.47s 保持，015 按第 1–2 章 20–35s 合同重解为 29.68s。
- [x] 040 刀锋关小步加厚至十种子最差基地余量 2.06%；039 / 084 保留“十种子安全优先、严格包络超限留档”的既定取舍。
- [x] 下一链全量重生成 clear / star / corridor / pressure 合同、重钉 smoke，并把正式默认攻速从 control 切为 tier_b。

## 阶段 204F · B2b 全量合同与 Tier B 正式默认（2026-08-29）

- [x] 001–099 clear requirement 全量重生成；固定帧证据拼回六个章节证据源，功率走廊 97/97、毕业免费族 99/99 通过。
- [x] 正式运行时和默认平衡模拟从 control 切到 tier_b；control 继续作为不变的算术对照，付费装备数据与既有付费发布合同未改。
- [x] wave-pressure / level-pressure / smoke 钉值按 B2b 合并结果重钉；固定帧星级汇总为 96×3★ / 3×2★ / 0×1★。
- [x] 生成逐关星表旧→新对照和消费口盘点：普通战役 280→294★，全收藏缺口 36→22★，无星级回退。
- [x] **审批闸门**：Owner 于 2026-08-29 批准并冻结星表；终验仅允许星级持平或上升，任何降星立即停工重报。

## 阶段 204G · B2b 最终终验（2026-08-29）

- [x] §3.7 Boss 分章时长带写入合同单一来源；065 `61.5s` 与 090 `91.8s` 按正确章节直接入带，无需改关。
- [x] 015 以 tier_b / v2.1 十种子独立复验：10/10、Boss 中位 `27.5s`、推进中位 `60.53%`、基地中位/最差 `100%`；现有编排已经回到第 2 章 `20–35s`，未再改数据。
- [ ] 完成 001–099 × 10 最终固定帧终验、刀锋余量复核、双族门禁与 build 64。
- [x] 终验修复第 2 章块：018 末波耐久 `1.00 → 0.90`，十种子恢复 10/10；推进中位 `66.71%`、基地中位 `100%`、最差余量 `50.55%`。重生成 fixture 后 019–099 三种子快扫未新增失败关。
- [x] 059 合并事故诊断：交付 solutions 与 B 的 10/10 证据生成状态不一致；已恢复证据对应的 W4/W5 耐久与 spitter×2，但最终 fixture 下仍为 0/10，留待第 6 章块按序修复。
- [x] 终验修复第 4–5 章块：040 / 045 / 049 / 050 均恢复十种子 10/10 并入档位带；040 Boss 中位 `42.30s`、最差基地余量 `16.49%`，045 Boss 中位 `55.84s`，049 推进/基地中位 `90.82% / 86.22%`，050 推进/基地/Boss 中位 `87.50% / 67.42% / 58.85s`。
- [x] 050 保留冻结的双母体错峰、15 个喷吐者、末尾步行者与 `450k / 1.5 / 抗毒 50%` 模型，仅将 W4/W5 小怪耐久定点收敛为 `4.8 / 3.5`；十种子最差基地余量 `58.44%`。
- [x] 第 4–5 章闭合后重生成成长 fixture，并完成 051–099 三种子下游快扫：`131/147`、0 超时；下一升序块仍为 058/059/060，另登记新 fixture 下的 062/069 回退，须在 060 后、068 前处理。
- [x] 终验修复第 6 章块：058 末波耐久 `3.0→2.7` 后 10/10（推进/基地中位 `95.08% / 89.32%`，最差 `3.69%`）；059 关卡局部系数 `33.2→6.0` 后 10/10（`97.47% / 92.39%`）；060 总 Boss 数 `6→4` 后 10/10（推进/基地/Boss 中位 `95.42% / 86.81% / 41.78s`）。
- [x] 第 6 章闭合后重生成 fixture，061–099 三种子快扫 `112/117`、0 超时；068/069/080 随成长传播恢复 3/3，剩余失败仅 062 `0/3` 与 077 `1/3`，继续按数字升序处理。
- [x] 新 fixture 回退 062 已按升序修复：仅将关卡局部系数 `10.5→2.1`，十种子 10/10，推进中位 `55.62%`、基地中位 `100%`，命中 light_pressure；重生成 fixture 后 063–099 快扫 `109/111`、0 超时，唯一失败仍为 077。
- [x] 终验修复第 7 章尾块：068 W3 耐久 `0.75→0.65` 后十种子 10/10，推进/基地中位 `89.04% / 83.57%`、刀锋最差余量 `23.38%`；重生成 fixture 后 069–099 快扫 `91/93`、0 超时，唯一失败仍为 077。
- [x] 终验修复第 8 章块：075 W4 `1.35→1.2` 后 10/10（high 档基地中位 `63.56%`、Boss `78.13s`、最差 `9.60%`）；077 W5 `1.7→1.1` 后 10/10（pressure `85.60% / 86.80%`）；080 W4 `2.12→1.9` 后 10/10（high `100% / 57.82%`、Boss `85.10s`、最差 `6.72%`）。
- [x] 第 8 章闭合后重生成 fixture，081–099 三种子快扫 `57/57`、0 超时，无新增失败。
- [x] 第 9–10 章闭合：088 / 090 复判通过；095 采用无保底 `charger×2 + spitter×4 / W5 hp17`，新 fixture 十种子 `10/10`、推进/基地中位 `100% / 76.88%`、最差 `5.96%`、Boss 最大 `175.72s`。Owner 批准仅 095 的 high 档基地中位上限 `65%→80%`，其余档位带不变；096–099 下游快扫 `12/12`。
- [x] 终验星表回退 080 已按 Owner 授权定点恢复：W4 局部耐久 `1.90→1.30`，十种子 `10/10`、基地中位 `70.48%`（保持 3★）、最差 `64.73%`、Boss 中位 `86.22s`；081–099 fixture 下游快扫 `57/57`、0 超时。严格 high 包络超限继续留档，不放宽合同。
- [x] 最终收敛第 1 章 Boss 带回收：005 改用 `crowd` 编排后十种子 `10/10`、推进/基地 `62.23% / 100%`、Boss 中位 `21.43s`；010 将第二只泰坦错峰 10 秒并把 W4 局部耐久钉为 `1.5`，十种子 `10/10`、`59.45% / 100%`、Boss `32.26s`。重生成 fixture 后 011–099 快扫 `267/267`、0 超时。
- [x] 最终收敛第 3 章与成长传播回收：025 增加至 5 次卡牌节奏后十种子 `10/10`、Boss `35.73s`；030 将末波喷吐者 `5→3` 后十种子 `10/10`、推进/基地 `95.84% / 91.01%`、Boss `44.21s`；047 / 068 / 069 / 070 仅用关卡局部系数与 W4 耐久吸收下游漂移，均十种子 `10/10`。
- [x] 070 定稿最差基地余量 `2.34%`、Boss 中位 `65.77s`；推进 `86.52%` 的严格 high 包络冲突按 039 / 084 既有裁决留档，不放宽种子安全门禁。重生成 fixture 后 071–099 快扫 `87/87`、0 超时，最差基地余量 `21.53%`。
- [x] 全 99×10 首轮为 `989/990`、0 超时，唯一缺口 078 / seed 8849；将 W5 局部耐久 `1.25→1.10` 后十种子恢复 `10/10`，推进/基地中位 `88.27% / 83.86%`、最差余量 `14.11%`，保持 pressure 档与已批 3★。重生成 fixture 后 079–099 快扫 `63/63`、0 超时。

## 阶段 205 · 挑战曲线 Tier B 重解（2026-08-31）

- [x] 挑战 K 改为 `data/challenges.json.curve` 单源分段 smoothstep，逐关单调从 `1.25` 升至 `5.0`；生命、推荐战力与三条防线指数由运行时和 Python 共用同一求值规则。
- [x] Owner 裁决将“逐关严格升一档 / 10 次全胜”改为章节分布与通关率带：每章 001/005/010 节奏代表点（第十章为 091/095/099），ch1–6 `70–100%`，ch7–10 `60–90%`。
- [x] 最终 99×10 Tier B / v2 固定帧证据完整 990 条；十章代表胜率依次为 `100/100/100/100/100/96.7/90/73.3/83.3/86.7%`，全部入带。
- [x] 099 黄金法则一档满级为 `9/10`，胜局 Boss 中位 `201.017s`，命中挑战专属 `60–90% / 150–220s` 锚点；旧 `1.556/0.92` 与基地余量锚点作废。
- [x] 第 9–10 章免费毕业族代表点分别 `15/30` 与 `1/30`，均低于付费参考样本带；现有 `<0.85` 严重不足警告与二次确认继续生效。
- [x] 普通 099 改动前后十种子运行记录逐位一致，canonical SHA 均为 `b11d47c39700d24a8d0d679ee0749dee4bfd67781d7ba7d633a6a59a8d973a19`；`data/levels.json` SHA 保持 `5c95e196…dc8`。
- [x] 挑战理论星供给永久门禁为 `297★ / 297★`；专项运行时门禁、数据、经济、资源引用、压力、选卡、挑战静态模拟、Godot headless 与 M1 smoke 均通过。

## 阶段 206 · 付费战斗主体可读性修复（2026-09-01）

- [x] 逐张校准四套付费主题 × 四角色 × 左/中/右共 48 个真实枪口坐标；单发与所有多弹道/多弹丸组合继续共用同一枪口原点，修复弹丸从人物或枪身中部出现的问题。
- [x] 修正炼狱主题 `char_volt` 左射成图方向错误；镜像规则、人物锚点与枪口坐标均由 `true_grip` 数据统一驱动。
- [x] 四只付费宠物按 alpha 可见高度统一为 148px 战场主体，并将光环改用世界空间比例，提升存在感而不改变属性、碰撞、技能范围或战斗数值。
- [x] 完成五主题 × 四角色 × 四姿势 80 路真实战斗截图门禁，并人工复核四套付费主题 48 个射击姿势与四只付费宠物。

## 阶段 207 · 战区地图入口与章节卡布局修复（2026-09-01）

- [x] 战区顶部角色入口改用统一主体高度的膝上裁切，武器 / 护甲 / 芯片 / 宠物 / 技能入口先裁透明留白再放入扩大后的展示区；免费与四套付费装备共用同一规则。
- [x] 章节卡左栏由 32px 退让到 48px，彩色路线轨缩为独立 8px 线，关卡范围与击破目标不再覆盖双边缘。
- [x] 两枚 Boss 徽章扩大并加入内容内边距，065/070、075/080、085/090、095/099 的图标与关卡号均不再压框。
- [x] “回顾战区”按按钮右侧装饰扣除 18px 后光学居中；四套付费主题 1080×2340 后期地图截图运行时审计 4/4 无问题。

## 阶段 208 · 商店购买弹窗排版收口（2026-09-01）

- [x] 商店确认与购买完成弹窗改用独立紧凑字号层级；保留全局移动端字号，不影响其他页面的可读性标准。
- [x] 装甲按钮改由覆盖完整按钮面的 `ActionLabel` 绘制操作文字，取消 / 确认购买不再受贴图内容边距影响而偏离视觉中心。
- [x] 黄金法则升级包中英文 1080×2340 实际弹窗截图 2/2 通过运行时 UI 审计，长文案完整、按钮无裁切或偏心。

## 阶段 209 · 商店系列归属层级（2026-09-01）

- [x] 四套付费系列标题增加独立“系列 / SERIES”标识与系列色竖轨，不再像上一张商品卡的尾标。
- [x] 商品卡增加“主题 / 完整包 / 军械升级”类型标识，并沿用对应系列色竖轨，让同一系列的标题与商品形成连续视觉语法。
- [x] 系列之间加入明确闭合分隔带；中英文黄金律完整解锁高屏截图运行时 UI 审计均为零问题。

## 阶段 210 · iPhone 高屏设置页舒适度布局（2026-09-01）

- [x] 修复攻速实验入口在 `_apply_style()` 之后才显示、继承双按钮宽度并把整张设置面板撑出 iPhone 安全区的问题。
- [x] 舒适度区域改为响应式流式布局：普通发行态保持两个开关同排；攻速实验可见时两个开关同排、实验入口独占第二行。
- [x] 舒适度三控件统一满足 88px 最小触控高度；实验态紧凑布局仅把返回键从 96px 收至 88px，不压缩中英文说明正文。
- [x] iPhone 16 Pro Max 比例 `1080×2340`、安全边 `44/132/44/102` 的中英文实验态审计均为 0 问题；正式态 tall / safe-area 与中英字体回归共 4 张全部通过。

## 阶段 211 · 防线围城零向量瞄准修复（2026-09-01）

- [x] 复现贴线僵尸中心与付费武器枪口重合时的 `(0,0)` 自动瞄准死锁：炮塔允许开火但发射器不产生新弹丸。
- [x] 自动瞄准在存在其他同场威胁时避开枪口已埋入碰撞体的目标，改选有可见飞行距离的左/右真实目标；手动瞄准与点名锁定不受影响。
- [x] 只剩贴身目标时使用上一有效射向完成近距离补射，不再因为零向量停火，也不会把该敌人永久留成攻击死角。
- [x] 黄金法则左右夹击确定性复现从中心偏移 `(0,0)` 修复为 `(-250,-4)`，一轮新增 14 枚弹丸；贴身单目标路径同样新增 14 枚，人物侧向姿势、枪口和弹道一致。

## 阶段 212 · 战区逐关进度段可读性修复（2026-09-01）

- [x] 确认“战区进度”下方十个图形代表本章逐关通关状态，并非章节装饰或星级图标。
- [x] 移除被压缩到 18px 后只剩斜角残片的大面板九宫格，改为专用十段短条：章节色实心表示已通关，暗色描边表示未通关。
- [x] 完成 1080×2340 后期战区真机比例截图与运行时 UI 审计，确认十段完整、章节色区分明确且不挤压 Boss 徽章。
- [x] 修正截图复核时发现的章节底栏宽度溢出：主按钮保持 280×80 合规触控规格，左侧缩略图区收为 300px、双栏间距收为 12px、Boss 徽章保持 88×68，确保按钮皮肤最外圈与卡片内框之间仍有清晰余量。

## 阶段 213 · 设置页开关文字安全边（2026-09-01）

- [x] 将“总声音”“减弱闪烁震动”“触感反馈”等统一开关控件的左内容边距由 28px 增至 48px，避开按钮贴图左侧高亮金属角。
- [x] 完成 iPhone 16 Pro Max 比例中英文截图与运行时 UI 审计，确认长文案不挤压右侧开关。

## 阶段 214 · 战区卡片左侧色条减法（2026-09-01）

- [x] 移除章节卡最左侧贯穿上下的 `ChapterRouteRail`，并将唯一章节色短条明确归入缩略图，消除同色双条叠放。
- [x] 完成后期章节高屏截图与地图冒烟，确认卡片层级更干净且章节识别色仍清楚。

## 阶段 215 · 战区头卡与章节内容精排（2026-09-02）

- [x] 移除“无限尸潮”压暗层携带的第二套面板边框，只保留正式九宫格卡框，消除四角错位形成的斜框。
- [x] 章节缩略图向左 12px、向下 8px，右边缘收窄 20px，改善左侧空当与右侧标题间距。
- [x] Boss 徽章显示自然关卡号 `65/70/75/80…`，图标收为 40px、数字区收为 24px，避免内容骑框。
- [x] 完成高屏截图和地图冒烟，逐项确认头卡四角、缩略图节奏与 Boss 徽章。

## 阶段 216 · 出战配置防线完整度可读性（2026-09-02）

- [x] “防线完整度”标题字号由 15 提至 17，并增加标题与三档门槛之间的垂直间距。
- [x] 三档星级门槛框由 42px 拉高至 58px，阈值文字由 14 提至 17，星标由 18px 放大至 22px。
- [x] 完成 1080×2340 安全区截图复核；战术摘要完整包裹新高度，且与“开始战斗”按钮保持独立净空。

## 阶段 217 · 在途伤害预判与多弹道覆盖（2026-09-02）

- [x] 自动瞄准按目标当前生命、护甲、护盾、元素抗性及在途弹丸直伤建立只读预留；预计致死后，下一发提前切换目标。
- [x] 落空、闪避或销毁的弹丸随节点退出自动释放预留；手动瞄准、点名锁定与 Boss 集火优先级保持不变。
- [x] 多重射击保持固定等夹角刚性扇面，只旋转整组弹道，在合法朝向中优先覆盖最多不同存活目标。
- [x] 确定性用例验证两发预留后自动切换，并验证三弹道覆盖左 / 中 / 右三个目标且夹角相等；M1 smoke 与全套非视觉数据门禁通过。

## 阶段 218 · 自爆僵尸基地伤害与反馈修复（2026-09-02）

- [x] 查明基地写实受击音资源和 `enemy_breach` 路由均未回退；异常听感来自远处死亡爆炸误入物理防线接触分支。
- [x] 移除死亡爆炸固定 `y > 1080` 的隐形远程判定，改由 `mechanic_params.radius` 与运行时防线的真实相交关系裁决；自爆 160px、毒云 190px。
- [x] 真正触及防线时补充爆炸圈到基地的传导轨迹、基地核心闪光、冲击环、震屏和“爆炸波及 / 毒云波及”文本；物理贴身撞击继续独占写实防线音。
- [x] 定点边界验证与 Godot 载入、M1 smoke、音频独占/写实度检查通过；未改伤害系数、敌人数值或音频资产。

## 阶段 219 · 角色详情底部说明与关闭键收口（2026-09-02）

- [x] 专属技能“每级”成长说明增加真实行高与 44px 滚动底部净空，不再被区块边框或固定操作栏裁掉。
- [x] 详情内容中的面板、标签和升级按钮统一允许触摸拖拽继续传递给 `DetailScroll`，滚动死区由 30px 收至 12px；静止点击升级按钮仍保持原行为。
- [x] 右上角关闭键不再把横向胶囊皮肤压成方形，改用单一生产级正方图标框；可视框增至 104px，叉号由 56px 放大至 68px。
- [x] 角色详情三高度 × 中英 × 四角色共 24 张运行时截图全绿，并人工复核 iPhone 高屏钢铁先锋详情；Godot 载入与 M1 smoke 通过。

## 阶段 220 · 三选一技能卡完整文案与等高布局（2026-09-02）

- [x] 三张技能卡先分别测量数值块、自动换行正文和标签行，再以本组三张中的最大自然高度统一卡高；短文案不塌卡，长文案不裁切。
- [x] 正文关闭文字裁切并保留不限行智能换行；标签统一贴齐等高卡片的下基线，三张卡保持清楚的横向比较节奏。
- [x] 推荐标签加宽 12px、英文标题栏补足 6px，消除同一弹窗内原有的边缘裁字。
- [x] iPhone 高屏中英文常规组合与含“蓄能重击”的定点组合共 4 张截图通过运行时 UI 审计，人工确认完整句子均可见。

## 阶段 221 · 全装备战术说明数据化（2026-09-02）

- [x] 武器战术说明改为读取真实元素、弹道、基础射速与成长段；免费武器和四套终焉武器均明确连锁、燃爆、碎冰、裁决、散弹、穿透、爆炸或毒云等实际机制。
- [x] 护甲说明覆盖生命倍率、定向抗性、屏障层数、受击反击、范围、修复与减速；芯片说明覆盖主属性、附加机制、等级成长与适配构筑。
- [x] 免费 / 付费宠物统一说明协战属性、基础攻击频率、主动机制、成长和阵容用途；维修与经济宠物明确标注为非攻击型支援。
- [x] 战术说明正文禁用裁切、增加行距并接入详情页触摸滚动与底部净空；iPhone 16 Pro Max 比例 10 张代表性详情截图通过运行时 UI 审计，M1 smoke 通过。

## 阶段 222 · 散弹炮永久等级弹丸成长（2026-09-02）

- [x] 修复散弹炮把满级 5 枚弹丸从 Lv1 起固定生效的问题；改为 Lv1–24 三枚、Lv25–49 四枚、Lv50 五枚。
- [x] 散弹弹丸与局内多重射击、先锋弹幕齐射保持两个独立维度：散弹数按每条外层弹道展开，任一来源不覆盖另一来源。
- [x] 收藏标签、详情属性、升级临界提示和战术说明读取同一等级阶梯，明确显示当前弹丸、满级上限与叠加关系。
- [x] M1 smoke 钉住 3/4/5 阶梯以及 3、10、15 枚实际组合数量；数据、本地化、发布字符串与 Godot 载入通过。

## 阶段 223 · 散弹与多重弹道等角合成（2026-09-02）

- [x] 修复散弹内层扇面逐条套入多重射击外层扇面后产生重合 / 近重合角度的问题；最终总弹数改为在完整覆盖角内一次性等角排布。
- [x] 保留散弹等级、多重射击与先锋齐射的原总发数及最外侧覆盖范围，不改伤害、射速、目标分配或全局五弹道上限。
- [x] 最终扇面重新锚定到已校正的目标弹道，自动瞄准与手动锁定继续保证至少一发穿过指定目标。
- [x] M1 smoke 新增 7 发、9 发及散弹 × 多重组合的逐相邻角度一致性断言，并钉住奇数扇面中央弹道。

## 阶段 251 · TestFlight Build 72(2026-09-18)

- [x] 合入 `codex/visual-fixes` bc91ec58(8 笔):战区首领徽章+长按说明、战区环境大图与紧凑关卡行、固定返回、四角色射击间隙/受击保持持枪、武器跨主题保持原貌(`resolve_weapon_asset` 恒返回原素材,主题商品卡改读主题描述)、主题中英标题留白。Fable 独立核实:m1 零错误、7 项静态门禁、主线聚合门禁 OK、四段免费指纹不变、`core/commerce`/`src/ios`/`core/save` 零改动;合并保留构建号与内购能力/StoreKit 插件/加密声明。
- [x] `1.0.0 (72)` 上传成功,Delivery UUID `67a6c34b-23d2-4393-9a85-9d93a5032f28`,IPA 938,393,999 bytes, SHA-256 `c309ec3ccb66a52a5be23beed9bd10974d78d54c88d4309ca760581b759307f2`。
- [x] 主题不再含武器涂装:2026-09-20 已在 ASC 改掉霓虹/炼狱两个主题内购的中英描述(极光/鎏金原文未承诺武器换色,保留),霓虹主题审核备注由 3 份重复旧文改为单份新文,接口回读确认;`design/32` 同步。`data/store_products.json` 主题行旧副标题已不再显示,可择机清理。
- [ ] 已知非阻断:商店主题详情仅在左右各 44px 安全区压力配置下右溢(竖屏真机左右 inset 为 0)。
- [x] 2026-09-20 ASC 版本页构建已从 70 换成 72 并保存(接口回读 1.0.0 → build 72 VALID)。
- [x] 2026-09-20 提审:首次提交(14:10)只含 App 版本,12 个内购未入草稿(每个内购须先在各自页面点“添加以供审核”进入草稿提交,不会自动随版本走)→ 撤回 → 12 内购 + 1.0.0 (72) 放入同一草稿 → Owner 14:26 重新提交,接口回读 13 项、版本与 12 内购均 `WAITING_FOR_REVIEW`。
- [ ] Owner 真机沙盒支付五步与 DSA 信息仍未做。

## 阶段 228 · TestFlight Build 71(2026-09-14)

- [x] 合入 `codex/visual-fixes` 688fc89d:UI 收尾批 28 项(UI-01~28)+ DEVICE-01(上线态透传 StoreKit 原生状态)+ UI-13 补修(选卡时整层隐藏世界名牌、顶界=波次条底+48px、高度吃紧启用 1020 紧凑宽)。Fable 逐项核实:battle.gd 只动布局,data 只改文案,指纹与主线一致,冻结路径零触碰;冒烟与七项静态门禁独立重跑全绿;主线聚合门禁 OK。
- [x] `1.0.0 (71)` 上传并经 Apple 验证 `VALID / APP_STORE_ELIGIBLE`,Delivery UUID `094f51f7-2f41-4d37-abae-3b4482b85192`,IPA 931,014,933 bytes, SHA-256 `cd64bf1e3887ccef45c33034aed3cb510322143338069feec252a555dc75e76e`。
- [ ] Owner 真机沙盒支付五步:购买 / 取消 / 待批准(Ask to Buy)/ 恢复购买 / 退款撤权后重启收回。这是提审前唯一未验项。
- [ ] Owner 填 ASC 数字服务法(DSA)贸易商信息;提审时 12 个内购与 Build 71 放进同一审核提交。

## 阶段 227 · TestFlight Build 70(2026-09-03)

- [x] 合入 `codex/visual-fixes` 5fdf0d22:F-X1 英文隐私说明按正文撑高、F-X2 武器名换行 + 等级独立徽章、F-X3 角色详情首屏信息块完整、F-X4 配装摘要按内容分列;购买完成弹窗堆叠主操作在上(648b040e)经 Codex 复核。
- [x] 核实:数据/字号/确定性/散弹路径零差异,四段指纹与主线一致,m1 新增 114 行布局合同;冒烟与静态门禁由 Fable 独立重跑通过;166 张证据(仓库 + 桌面 `12_视觉第三轮/`)。
- [x] Owner 接受 L038~L044 1★ 与 L088 边缘种子;548 个旧 corrupt 存档碎片已清。
- [x] `1.0.0 (70)` 上传并经 Apple 验证 `VALID` / eligible=True,Delivery UUID `416d6f50-12bd-402d-9f1d-f20dd7bdca2f`,SHA-256 `1f81d76a9149da7f846fe3c3c3b260232850250a153e1857e6ff4754937c6370`。
...
- [x] 合入 `codex/visual-fixes`(ff58bd8c:视觉大包 F-A~F-K 11 项);7 处冲突手工解,合并涉及界面 13 张截图复核(桌面 `11_合并验证_2026_09_02/`)。
- [x] 合并候选 99×10 相对当前主线 99×10:胜场下降 0 / 降星 0 / 基地跌>10pp 0;两份档案与对账见 `design/audits/b2c_*_2026_09_02.*`。**发现归档基线与夹具错配**,已批星表 L080 在当前夹具下为 2★,待 Owner 拍板。
- [x] 完整 RC、导出 PCK 启动/存档/M1 smoke/TestFlight 功能探针、Xcode 归档、IPA 审计通过;`1.0.0 (68)` 上传并经 Apple 验证 `VALID / VALID / APP_STORE_ELIGIBLE`,Delivery UUID `97a24187-bcbf-4919-a401-0c0a06fb05d7`,IPA 930,908,250 bytes,SHA-256 `5fe130b11accfe994047847286e95e3327b86ac3ae4f358ca5842a3c51da879a`。
- [ ] **Build 67 作废**:它由 9/1 会话未提交的工作树上传(`source_dirty=true`),含未批准的散弹枪弹丸阶梯与自动瞄准实验;TestFlight 测试请以 68 为准。

## 阶段 224 · TestFlight Build 67（2026-09-02）

- [x] 修复发布门禁发现的散弹 `pellets` 战斗数据消费合同，并以作者上限约束等级解析结果。
- [x] 将地图与商店中 7 处发行态平面 UI 基元替换为现有生产纹理 / 语义标签样式，运行时 UI 基元门禁恢复全绿。
...
## 阶段 227 · 散弹与多重弹道均匀收势（2026-09-02）

- [x] 修复多重射击外层 lane 各自嵌套完整散弹扇面后，中段弹丸成对拥挤、两侧间距突增的问题。
- [x] 最终弹丸按“外层跨度 + 18° 散弹跨度”的原包络一次性等角排布；总弹数、左右外缘、瞄准锚点、伤害、射速、追踪和逐 lane 衰减均保持不变。
- [x] M1 smoke 新增 2×5 与 3×5 组合的逐相邻夹角等值断言，并保持既有数量合同；完整冒烟通过。

## 阶段 228 · 单局属性弹首次选择锁定（2026-09-02）

- [x] 所有武器在本关首次选择属性弹前保留原有可选范围；一旦选择，本关后续属性弹候选只允许同一张卡继续升级，不再出现其他属性替换卡。
- [x] 修复可换属性的终焉武器绕过已选互斥组检查、导致火 / 冰 / 雷 / 毒可以在同一局反复换装的问题；免费物理武器与原生元素武器既有初始兼容范围不变。
- [x] M1 smoke 覆盖终焉雷霆首次可选多种属性、选择冰霜弹后只保留冰霜升级且屏蔽火 / 雷 / 毒。

## 阶段 229 · 技能选择卡正文与标签安全间距（2026-09-02）

- [x] 正文与标签使用独立 16px 安全间距，中文两行说明的末行描边不再与语义标签相碰。
- [x] 从卡片底部纯留白回收 8px，保留标签边缘净空，同时不增加三张卡的最坏总高度。
- [x] M1 smoke 遍历全部技能等级与中英文，锁定正文—标签间距及战场走廊上限；追踪弹 Lv2 / 冰霜弹 Lv2 / 伤害穿透定点实拍 UI 审计为零问题。

## 阶段 230 · 角色主动技能加速档演出可读性（2026-09-02）

- [x] 角色主动技能专属 14 帧序列与开场闪光改用真实时间播放，1× / 1.5× / 5× 下保持相同肉眼可读时长。
- [x] 伤害结算、冷却、后续打击节奏和普通武器特效继续随战速推进，不改变任何战斗数值或确定性审计路径。
- [x] M1 smoke 覆盖四名角色首次与第二次释放，并钉住 5× 下专属序列不会被压缩为五分之一时长。

## 阶段 231 · 散弹炮弹丸成长恢复为 3→4→5（2026-09-02）

- [x] 按 Owner 最终修订，将三段弹丸数设为 Lv1–17 三枚、Lv18–34 四枚、Lv35–50 五枚；等级分段保持均匀，不重算战力尺子。
- [x] 保留 `special.pellets = 5` 硬上限与现有等角合成逻辑，未改伤害、射速、追踪或多重射击叠加规则。
- [x] M1 smoke 钉住六个等级边界，收藏 / 详情 / 战斗继续从同一 `pellet_growth` 数据读取。

## 阶段 232 · F-X1 设置隐私正文完整显示（2026-09-03）

- [x] 关闭信息正文裁切，让换行后的自然最小高度撑开面板；按实际溢出量回收行间留白，不缩字号、不改隐私原文。
- [x] 中英 × 1080×1920 / 1080×2340 / 1320×2868 六张复拍 issues=[]；新增 M1 正文 / 容器 / 视口包含与授权字号断言。

## 阶段 233 · F-X2 武器标题保留授权字号（2026-09-03）

- [x] 名称智能换行、等级拆为独立纹理徽章；按两种语言的全武器名称与当前等级测量共享卡高，保持原有 8px / 6px 元数据间距。
- [x] 四主题 × 中英 × 列表顶部 / 付费武器区 16 张实拍 issues=[]，标题不再调用缩字；M1 新增授权字号、完整换行、独立徽章断言。

## 阶段 234 · F-X3 角色详情首屏完整区块（2026-09-03）

- [x] 被动 / 专属技能先于升级预览呈现；首屏边界按完整技能 / 升级信息块留白，不露半行。
- [x] 滚动尾部留白由固定操作栏实高撑开（202px 操作栏 + 44px 余量）；四角色双语四档高度的首屏与滚动到底断言通过。

## 阶段 235 · F-X4 配装摘要列宽（2026-09-03）

- [x] 弱点列按内容取宽，关卡列分配剩余空间；普通 / 挑战中英文 099 关名与波数保持一行，字号和语义不变。
- [x] 六张复拍 issues=[]，M1 增加三尺寸中英普通 / 挑战 12 组宽度断言；英文普通值需 369px，可用 491px。

## 阶段 236 · 购买完成弹窗按钮顺序复核（2026-09-03）

- [x] 保留主线 648b040e 实现不改；中英 × 1080×1920 / 2340 四张复拍确认立即应用整套在上，文字完整，issues=[]。

## 阶段 237 · 视觉第三轮最终验收（2026-09-03）

- [x] 最终完整非窗口 release candidate 通过、M1 零 ERROR，运行时纹理样式 / 文案 / 本地化门禁全部通过。
- [x] 58 张改后及弹窗复核图 issues=[]；与 54 张旧图组成 112 张原生证据，未降字号或放宽既有断言。
- [x] 免费侧四段指纹与本地 main 一致，gameplay / core / data / ui 零差异；共享测试存档按要求清理，不 push。

## 阶段 238 · 全主题 UI 只读审阅（2026-09-08）

- [x] 固定 `12e6e354c35a`，2236张截图覆盖五主题、四人物/20造型、12武器/44视觉、四套天启装备、核心界面中英三尺寸及边界专项；不是所有轴的完整笛卡尔积。
- [x] 输出中文一页摘要、28项图证报告、102组五主题同屏对照和可检索原图索引：`design/audits/ui_full_review_2026_09_08/`。
- [x] 非窗口聚合发布检查通过，m1零ERROR；截图尺寸/HEAD/哈希核验通过。保留79张/226条布局提示，人工发现11项P1、17项P2/P3，另1项StoreKit真机风险；不宣称UI全绿。
- [ ] 本轮仅审阅，发现项未修复；真实支付反馈待iOS沙盒验证。产品代码/文案/战斗数据未改，未提交、未push；捕获与测试使用独立HOME，真实用户存档未触碰。

## 阶段 239 · 全主题 UI 审阅后的首轮修复（2026-09-08）

- [x] Owner 授权先修一版：完成 UI-01/02/03/04/05/16/17/19/23/24/25/27，共 12 项独立运行时提交，终点 `01a5ee25`；不新增功能、不改字号或战斗数值。
- [x] 最终非窗口聚合发布检查通过，含 m1 零 ERROR、纹理样式、本地化及发布文案检查；新增显示断言覆盖十战区标题、原生 286×80 入口、锁定角色、百分比与结算完整行数。
- [x] 同一组 520 场景复拍：运行时提示从 58 图/187 条降为 4 图/4 条，像素规则零提示，未新增提示。五主题、中英、三主尺寸及部分历史高屏样本；这是针对性回归，不是全部 2236 图重跑。
- [ ] 保留 4 张英文 Privacy/Support 人工安全区压力越界，以及其余 16 项审阅优化和 1 项真机支付风险；等待 Owner 目视复看，不称视觉全绿。
- [x] 图证与中文报告：`design/audits/ui_first_fix_2026_09_08/`；本地大体积截图不纳入 Git。Godot 串行、离屏静音、独立测试 HOME；真实存档未触碰，未 push/打包。

## 阶段 240 · 全主题 UI 剩余16项收尾（2026-09-13）

- [x] Owner 授权全部修完，并于9月13日确认隐私/技能历史叙述清理及黄金图标常规透明背景整理；完成其余16项、设置4条压力警告、DEVICE-01真实消息保真。19个分项代码/素材提交，终点 `722fedc6`。
- [x] 居中按可见内容处理：武器名与等级、图标加文字、章节按钮对称内边距；霓虹剔除极淡导出残像并只绘制单层纹理。保留原生286×80与FONT_SCALE=1.5，英文选卡由20px提高至26px。
- [x] 最终非窗口聚合发布门禁60项通过，含m1零ERROR与runtime UI primitives；本轮中英三尺寸回归零失败。44个确定性函数、数值数据、弹道/瞄准不变，四段免费指纹与本地main对象一致。
- [ ] `final02` 在固定提交上进行全量2234独立截图/2236逻辑用例复拍；完整图证、人工复核与桌面报告仍待整批完成，不提前称视觉全绿。
- [ ] DEVICE-01真实StoreKit购买/取消/待批准/错误/恢复仍待iOS沙盒。测试使用隔离存档，未接触真实用户存档，未push/合并主仓库/TestFlight。

## 阶段 241 · Owner要求武器保留原样与最终续验（2026-09-14）

- [x] 撤回本轮新增武器主题染色，只复用已有高清展示素材；不新画武器、不动已验收套装和局内皮肤。追加商店卡18px内边距及宠物冷却小数显示，23项提交终点`b32841b7`。
- [x] 纠正旧资源引用工具的主仓库硬编码路径：旧聚合中的此项实际查错目录，现按工具自身所在仓库重验454条引用；规则与阈值不变。非预期只读扫描已记录，无主仓库写入。
- [x] 最新完整非窗口聚合60项通过，m1零ERROR；中英三尺寸补充UI断言零失败，含所有武器复用原素材、全部套装装备卡留白、整数/小数冷却。
- [x] 基础批封存后保留793张未受影响合格图，最新版本1441张续拍完成，覆盖2234独立图/2236逻辑用例：运行时和像素规则均0提示，原79张/226条提示归零。旧批26张退出清理错误排除重拍，诊断留档，组合版本证明单列。
- [x] 完整原生图、28项前后对照、126张五主题同屏图及中文人工复核报告完成；桌面副本校验见交付目录receipt。运行时23项提交另加文档证据提交，既有未跟踪产物保留，未push或TestFlight。
- [ ] 真实StoreKit购买/取消/待批准/失败/恢复仍需iOS沙盒，不把桌面离屏验证当真机交易通过。

## 阶段242 · UI-13选卡名牌/SE留白补修（2026-09-14）

- [x] Owner人工发现旧自动检查漏掉选卡边框上的半截世界名牌；选卡/详情期间整层隐藏，关闭恢复且不覆盖LOD。
- [x] 波次条下方保留48画布像素；高度紧张时复用水平空间和卡片真实高度，所有字号、图标、说明及间隔不变，操作按钮留在面板内。
- [x] 三尺寸中英专项0失败、完整非窗口发布门禁及m1零ERROR；78项选卡/详情复拍通过，2张退出清理失败保留并同源重拍。5个UI函数调整，44个确定性函数与免费指纹未变。
- [x] 桌面17_SE选卡名牌补修_2026_09_14附前后图；原15/16大包作为历史审阅版保留。未push、未打包。

## 阶段243 · 战区首领徽章留白与长按说明（2026-09-17）

- [x] 主按钮右移但保持286×80；徽章112×80，原字号不变，图标/数字间距加大。收回缩略图未使用的右侧空位，三尺寸安全区下按钮距卡片右缘至少30画布像素。
- [x] 0.5秒长按显示首领类型与数据中的关卡编号；短按不误开、拖动正常滚动并取消提示、切页关闭，锁定战区同样可看。无战斗/数值改动。
- [x] 五主题×三尺寸×中英地图30张、长按说明12张，最终42张复拍均零审计问题；三尺寸中英手势与布局专项零失败，覆盖实际第99关与鼠标。
- [x] 完整非窗口发布门禁60项通过、m1零ERROR；桌面18目录交付前后图及中英文案清单。未push、未打包。

## 阶段244 · 战区环境图加高与重复说明移除（2026-09-18）

- [x] 按Owner要求去掉十战区卡片重复的“击破战区首领，推进防线”；独有摘要、关卡范围、状态和徽章保留。
- [x] 原环境图292×132→292×174，复用移除文案腾出的高度，保持比例裁切、不拉伸；主按钮286×80与字号不变。
- [x] 五主题三尺寸中英30张回归＋6张前段战役最终构图，36张零审计问题；完整非窗口门禁60项及m1通过。桌面19目录提供最终截图和中文复看说明，未push或打包。

## 阶段245 · 关卡标题、信息同行与固定返回（2026-09-18）

- [x] 关卡标题统一30基准字号（实际47），取消标题缩字；推荐战力与弱点改内容定宽同行。中文保留紧凑左右布局，长英文按内容增加卡高，不删文案、不缩模式按钮。
- [x] 章节页左上角增加96×96钢青返回入口，固定在滚动区域外；返回总览隐藏，原卡内返回保留。
- [x] 99关×三尺寸×中英594项布局组合零失败；五主题/未通关/末战区/总览共48张最终截图零问题，完整非窗口发布60项与m1通过。
- [x] 桌面20_关卡列表布局优化_2026_09_18交付最终截图及中文复看说明；数值、确定性路径及四段指纹未改，未push、未打包。

## 阶段246 · 闪电少女持枪待机（2026-09-18）

- [x] 普通武器待机/受击原图是空手放电，现复用各武器已有F1持枪准备图；保留呼吸/受击骨架运动，定位使用对应中心姿势，不生成或覆盖素材。
- [x] 八武器三方向各三次开火/停火/受击72轮零失败，枪口坐标不变；五外观×八武器×两状态加散弹枪三尺寸，共84张截图零审计问题。
- [x] 完整非窗口发布门禁60项及m1新增持枪待机断言通过；桌面21目录交付84张原生截图、持枪前后状态放大与中文修补说明。
- [x] 仅修改表现加载/定位两函数，44个确定性相关函数及免费指纹不变；没有改数值、弹道、瞄准、其它角色或付费握持母版。

## 阶段247 · 其余三角色统一持枪待机（2026-09-18）

- [x] 复核确认钢铁先锋、火焰少年、冰霜少女也有旧待机/受击双手离枪问题；将上一轮持枪准备帧规则扩展到所有普通角色/武器，付费握持母版不变。
- [x] 四角色×八普通武器×三方向×三次切换，共288轮开火/停火/受击零失败，枪口位置不变；m1扩展持枪待机断言到全部普通组合。
- [x] 最终86张截图与3张修前图，覆盖五外观、八普通武器及三尺寸代表组合，零审计问题；完整非窗口门禁60项及m1通过。桌面22目录提供三角色前后对照与全部原图。
- [x] 本轮仅改动画加载函数的角色适用范围；44个确定性函数、数据/素材和免费指纹未改。独立测试存档，不push、不打包。

## 阶段248 · 武器原貌不随主题染色（2026-09-18）

- [x] 按Owner明确指示停止全部主题武器贴图解析，所有武器使用自身原始资源；收藏、共享图标、配装及独立武器图层不再应用主题surface材质。
- [x] 商店预览原色，主题说明移除涂装承诺；冻结商品数据不改，主题商品说明改取主题目录。旧素材保留、专属武器身份保留。
- [x] 五主题十二武器四类路径240项一致性，m1商店原图/原色断言通过；完整非窗口门禁60项通过。桌面23交付66张截图和五主题对照，首把武器区域像素逐字一致。
- [ ] 本轮额外发现：商店详情在左右44px安全区压力配置下横向溢出，固定四列/底部操作应独立内容驱动修补；本次仅记录，不扩大原貌任务范围。

## 阶段249 · 三主题标题边框留白（2026-09-18）

- [x] 按Owner截图修补霓虹、极光、黄金中英六张烘焙标题；扩大牌面、使装饰退出文字区。使用内置图像编辑，旧六图完整归档，提示词和资产索引同步。
- [x] 黄金旧1040画布裁切元数据更新为1536×1024；默认/炼狱、菜单控件、全局字号、按钮与战斗文件未改。不是全局缩字，也非逐像素保留原有字形。
- [x] 五主题×中英×三尺寸修前30张、修后30张实拍，零审计问题、零ERROR；首轮黄金留白不足的30张候选截图另行归档，不混充最终证据。
- [x] 完整非窗口聚合门禁60项及m1通过；桌面24交付中英三主题速览、五主题三尺寸前后60图与中文说明。四段免费指纹与本地main一致，不push、不打包。

## 阶段250 · 第一战区最新排布复查（2026-09-18）

- [x] 区分Owner看到的9月14日旧包和当前代码，重新拍摄当前基线48张；确认此前标题/信息行/固定返回已修，但介绍卡留白及英文按钮下沉仍需收紧。
- [x] 战区介绍/目标整宽内容驱动，去掉重复标题，进度与返回同排；第一战区400→310。英文关卡名与操作并排、长元信息走底行，原字号、触控尺寸、286×80返回及既有间隔阈值不变。
- [x] 五主题×中英×三尺寸×99关2970项专项零失败；修后48张截图零审计问题与ERROR，数据/素材/战斗文件和免费指纹未改。
- [x] 完整非窗口聚合门禁60项及m1通过；桌面25交付96张原生前后图、黄金中英三尺寸对照、五主题入口和中文复看说明。不push、不打包。
