...
- [ ] Phase 1B-2：补齐菜单 / 地图 / 收藏 / 配装 / 设置 / 战斗 / 结算的面板、标题、HUD 和基地覆盖层，接入免费武器霓虹涂装及模块化角色挂载；仍仅使用 Debug / TestFlight 预览权限，不接真实购买。
  - [x] Phase 1B-2a：四角色独立霓虹肖像、44 张透明模块化 idle / attack / hurt 帧、独立武器层、枪口与弹体霓虹材质、4 帧渲染式开火电弧翼、四主动技能霓虹叠层和设置页默认 / 霓虹可逆切换均已接入。
  - [x] 删除被 Owner 拒绝的程序网状开火线；新电弧翼位于人物后方、中心留空、随射击方向旋转，并支持“减弱特效”降级。
  - [x] 霓虹专项矩阵固定覆盖 43 路：菜单 / 设置 / 角色列表、四角色配装、完整 `4 × 8` 免费武器开枪矩阵和四角色主动技能。
  - [ ] Phase 1B-2b：剩余结构性面板、基地和 HUD 主题化；完整 `4 × 8` 运行时开枪矩阵已作为本阶段永久回归。
...
- [x] 四角色不再只靠默认模型叠 shader：接入 4 张霓虹肖像和 44 张独立透明战斗帧，每人包含 4 idle、4 attack / recoil、3 hurt。
- [x] 霓虹主题强制走模块化角色帧 + 独立武器层，避免复制 992 张合成帧；四个代表武器实机保持枪械身份与枪口方向。
