# 09 · 数值平衡

> 这里给出**具体公式与示例数值**，作为首版配表的基准。所有数字都是"可调初值"，上线前用真机刷关回归微调。
> 全部数字最终落进 `economy.json` 与各内容表，程序读表、设计师调表。

## 1. 设计目标（量化）

- **建议等级达标 + 核心装备按档** → 标准关 2~3 星、Boss 关 1~3 星。
- **欠 5 级或装备欠 1 档** → 标准关勉强 1~2 星、Boss 关需极限操作。
- **重刷 2~4 遍**某关 → 攒够金币把核心装备升 1 档。
- **天赋点总产出** ≈ 升满"任意 2 个完整角色全技能"所需 + 富余（鼓励专精，不强制全练）。
- 普通战役星总产出为 **297**；全部付费收藏为 **318**，再取得 **21** 颗挑战星即可收齐。
- **非推荐元素** → 普通关可首通但较难三星；Boss 硬免疫需要明确准备路径。
- **前 5 关** → 明确给甜头，允许玩家在低强化下快速通关、拿到正反馈。
- **第 6~49 关** → 难度按推荐等级和装备成长平滑抬升，避免突然墙点。
- **第 50~89 关** → 第 3 波以后用生命、尸潮数量和 Boss 生存窗口曲线持续起飞，开始检查完整配装而不是只看主武器等级。
- **第 90~99 关** → 进入终局对抗区，Boss 必须有时间跨阶段并释放多轮机制；不靠额外攻击倍率制造暴毙。

## 2. 角色等级与属性

```
level_xp(L)      = round( 50 * L^1.5 )          // 升到 L 级所需总经验近似阈值/级
base_atk(L)      = char.base_atk * (1 + 0.08*(L-1))    // 每级约 +8% 攻
base_hp(L)       = char.base_hp * (1 + 0.06*(L-1))     // 每级约 +6% 基地血
talent_per_level = (L <= 40) ? 1 : 2                    // 前40级1点/级，之后2点/级
```

到 Lv80（终局建议）累计天赋点 ≈ `40*1 + 40*2 = 120` 点/角色。
全技能升满成本（见 §5）约 100~110 点/角色 → **够升满本角色主力技能并有取舍空间**。

## 3. 怪物数值缩放（按关卡）

```
recommend_level(n) = round(1 + (n-1) * 49 / 98)       // L1 -> L50 线性
base_hp_ref(n)     = round(120 + (n-1) * 700 / 98)    // 120 -> 820 线性

target_pressure(n):
  1..5   = [0.28, 0.32, 0.37, 0.43, 0.52]             // 甜头段
  6..89  = 0.62 -> 1.55 线性
  90..99 = 1.65 -> 1.95 线性                          // 终局压制段

difficulty_coef = target_player_dps(recommend_level)
                * total_spawn_seconds
                * target_pressure
                / (base_hp_ref * enemy_hp_weight)

late_game_t      = clamp((n - 50) / (98 - 50), 0, 1)
wave3plus_hp     *= lerp(1.0, 2.05, late_game_t)
wave3plus_damage *= 1.0

late_count_t     = clamp((n - 55) / (90 - 55), 0, 1)
wave3plus_count  *= lerp(1.0, 1.25, late_count_t)

boss_survival_t  = clamp((n - 50) / (98 - 50), 0, 1)^1.15
boss_hp          *= lerp(1.0, 56.0, boss_survival_t)
```

- `GLOBAL_HP_BASE / GLOBAL_DMG_BASE` 是全局旋钮；99 关主线实际血量压力由 `tools/rebalance_difficulty.py` 反推并写入 `data/levels.json`。
- 第 1/2 波不吃 50+ 追加曲线，保留局内卡牌成型窗口；第 3 波后提高 HP 和数量，但压线 / 技能 / Boss 攻城伤害倍率固定为 `1.0x`。
- Boss：按 `phases` 分段，并额外吃生存窗口倍率；第 98 关达到 `56.0x`、第 99 关为 `60.48x`，只提高 HP。倍率由真实满配弹道审计反推，覆盖散弹、多重、穿透、分裂、弹射、暴击和主动技。
- 与"建议等级玩家的 DPS 曲线"对照（§6）确保前 5 关放松、中段线性、终局需要接近满强化。

## 4. 伤害与派生（接 `02` 公式，给系数）

```
最终单发 = base_atk(L) * weapon.base_atk_coef * Π(skill_mods)
         * PLAYER_SHOT_DAMAGE_MULT
         * (1 + Σ elem_dmg) * weakness_mult * (crit ? crit_dmg : 1)

crit_dmg 基础 = 1.5（可被 critical/chip 提升到 2.0+）
weakness_mult ∈ {1.5 弱点, 1.0, 0.5 抗性, 0 免疫}
DoT 单跳 = 快照最终伤害 * dot_coef（火≈0.25/跳、毒≈按层 0.05*最大生命 等，见各技能表）
```

## 5. 技能成长成本（天赋点）

每个技能 base_level Lv1→Lv5 的天赋点成本（统一基准，可个别微调）：

| 升到 | 单步成本 | 累计 |
|---|---|---|
| Lv1（学会） | 2 | 2 |
| Lv2 | 3 | 5 |
| Lv3（解锁分支） | 5 | 10 |
| Lv4 | 6 | 16 |
| Lv5（解锁终极） | 8 | 24 |

- 单技能满级 24 点。一套主力 = 2 专属 + 约 3 常用通用 ≈ 5 个满级 ≈ 120 点 → 与 Lv80 产出基本持平，留少量富余但需取舍（专精乐趣）。
- 角色独立计点（见 `03`）。练第 2 个角色相当于重新规划一套（提供新鲜感而非纯重复，因为元素流派不同）。

## 6. 玩家 DPS vs 关卡血量（可过性验证模型）

用一个简化模型在配表阶段自检每关"是否可过、几星可达"：

```
player_dps(L, gear, build) ≈ base_atk(L)*weapon_coef*fire_rate
                              *PLAYER_FIRE_RATE_MULT*PLAYER_SHOT_DAMAGE_MULT
                              *(1+crit_rate*(crit_dmg-1))
                              *avg_skill_multiplier(build)
                              *avg_weakness_mult(本关)

wave_total_hp(n)  = Σ enemy_hp(n, type)*count
required_clear_time = wave_total_hp / player_dps
leak = f(漏怪率, breach_damage)   // 漏怪导致基地掉血

预测星级 = 100% - 预测基地掉血%  → 映射 ★/★★/★★★
```

战力显示与难度比较使用同一把尺。Owner 于 2026-08-12 将玩家侧概念收敛为一个：

```
战力 = 核心配装战力
     * sqrt(以当前永久技能等级为种子、本关 target_card_picks 预算下的兼容技能倍率)
     * 本关元素克制倍率

推荐战力 = K * (clear_requirement.min_output 对应输出^0.82 * 固定参考生存^0.28)
           * sqrt(本关 target_card_picks 的通用选卡吞吐)
```

- 技能效果倍率直接读取 `skills.json` 每级 effect，覆盖伤害、射速、暴击、多重射击及其弹道衰减、穿透、分裂、连锁、追踪、元素持续伤害、破甲、减速和防线生命；纯经济/重抽技能不虚增战力。
- 配装页、入口警告与结算页只显示“战力 / 推荐”，不再并列“基准、预计成型、终局战力”。战斗中的真实选卡结果仍可作为战斗统计，但不命名成第二种战力。
- “推荐战力”是关卡固定门槛，只读取关卡数据与固定参考构筑；玩家升级技能或切换装备只改变“战力”，绝不能同步抬高推荐值。
- `target_card_picks > 4` 的关卡从第 3 波起追加静态 `run_skill_pressure`；当前十卡预算约为 `1.54x HP / 1.12x 移速`，挑战模式再独立叠加既定 `1.5x HP`。压力只由关卡表决定，不根据实时表现动态加难。

- 配表脚本（见 `13` 工具）对 99 关跑推荐等级 DPS 与关卡总血量模拟，输出 clear time / leak / star 预估。
- 验收线：**前 5 关应明显低压；6~49 关平滑抬升；50~98 关后半波压力近似线性增长；第 99 关允许单独终局跃升，但毕业 build 不应预测不可过**。第 98 关晚波 HP 为 `2.05x`，第 99 关为 `2.296x`；尸潮数量分别为 `1.25x / 1.35x`；额外伤害始终为 `1.0x`。
- 终局验收额外跑 `tools/check_endgame_balance.py`：三套满级、属性克制的物理主武器配装均须在 260 秒内通关，至少一套在 180 秒内通关；90 关 Apex 对标准三弹道满配克制输出至少存活 45 秒 / 5 次技能窗口，99 关至少 80 秒 / 10 次窗口且不超过 125 秒。不要求所有配装都能毕业，明显欠战力且主武器元素错误的方案不得被模型判为舒适通关。
- 额外跑两组 build：`recommended_build` 与 `off_recommend_build`。普通关中，非推荐 build 应预测 ≥1 星；Boss 关允许非推荐 build 失败，但必须在前置关卡和选关 UI 中提示。

## 7. 卡牌导演平衡

三选一不是纯随机，也不是完全点菜。基础权重模型：

```
card_weight = base
  * role_affinity(skill, character)
  * build_synergy(skill, owned_run_skills)
  * level_need(skill, level_threat_tags, primary_weakness)
  * emergency_mod(skill, leak_risk, elite_alive, boss_phase)
  * pity_mod(skill, recent_misses)
```

约束：
- 每次三选一至少 1 张 `build_synergy` 高的牌。
- 漏怪风险高时，`control/defense` 提权，`economy` 降权。
- 前 5 关人为提高 `split_shot/pierce/multishot/incendiary` 等爽感牌权重。
- 连续 2 次没有出现玩家核心 tag，第 3 次触发保底。
- 每局基础 1 次 reroll；精英击杀/`skill_recycle` 可额外给，避免烂牌挫败。

## 8. 金币经济曲线

```
gold_drop(type, n) = type.gold_coef * (10 + 2*n)        // 随关卡线性增长
first_clear_gold(n) = 100 + 20*n
强化成本(equip, lvl) = equip.cost_base * 1.15^lvl       // 指数，每10级一个台阶感
```

- 自检：在第 n 关重刷一遍的净金币产出，对照"把核心装备从档位 k 升到 k+1 的成本"，使**重刷 2~4 遍可升 1 档**。
- 经济技能/芯片（gold_rush / chip_greed）可把"刷"的效率提升约 30~60%，缩短肝度但不进战力平衡。

## 9. 星经济曲线（平滑消费制，总 318）

| 品类 | 付费价格带 | 小计 |
|---|---:|---:|
| 角色（先锋免费） | 10 / 12 / 16 | 38 |
| 武器（自动机枪免费） | 8–16 | 75 |
| 护甲 | 8–14 | 60 |
| 芯片 | 8–14 | 80 |
| 宠物 | 8–14 | 65 |
| **合计** | **单件最高 16** | **318** |

- 同一品类最高价不超过最低付费价 2 倍；强度更好的内容可以贵，但不再出现 60–230 星的陡峭跳价。
- 普通战役 297 星足以购买几乎全部收藏；21 颗挑战星完成全收集。挑战仍有价值，但不会成为尝试角色或装备的长期门槛。

## 10. 挑战与长期经济

无氪版本的长期内容不能给过强战力，否则会变成新的压力源：

| 内容 | 主奖励 | 平衡规则 |
|---|---|---|
| 变异尸潮 | 金币、皮肤碎片、图鉴标记 | 不掉唯一装备 |
| 无限尸潮 | 记录、排行榜、称号 | 不影响主线数值 |
| 章节挑战 | 章节徽章、外观 | 可给少量金币，不给必须战力 |
| 全三星 | 星盈余、皮肤 | 不要求全三星才能通主线 |

## 11. 平衡维护流程

1. 改表 → 跑 §6 自检脚本 → 看预测星级矩阵。
2. 跑 §7 卡牌导演模拟，检查核心 build 成型率、烂牌连续次数、经济牌危机误出率。
3. 真机/导出包刷代表关（前 5 关、每章首关、爽感关、狂潮关、各 Boss）。
4. 记录实际星级 vs 预测，偏差 >1 星的关卡回调。
5. 全局松紧只动 `GLOBAL_HP_BASE/GLOBAL_DMG_BASE`，局部动单关 `difficulty_coef`、波次或卡牌权重。
6. 所有改动进 git，配表版本与游戏版本对齐。
