...
- **前 5 关** → 明确给甜头，允许玩家在低强化下快速通关、拿到正反馈。
- **第 6~49 关** → 难度按推荐等级和装备成长平滑抬升，避免突然墙点。
- **第 50~89 关** → 第 3 波以后用线性生命/压线伤害曲线持续起飞，开始检查完整配装而不是只看主武器等级。
- **第 90~99 关** → 进入终局属性压制区，要求核心装备接近满强化、元素/技能选择正确。
...
```
recommend_level(n) = round(1 + (n-1) * 49 / 98)       // L1 -> L50 线性
base_hp_ref(n)     = round(120 + (n-1) * 700 / 98)    // 120 -> 820 线性

target_pressure(n):
  1..5   = [0.28, 0.32, 0.37, 0.43, 0.52]             // 甜头段
  6..89  = 0.62 -> 1.55 线性
  90..99 = 1.65 -> 1.95 线性                          // 终局压制段

difficulty_coef = target_player_dps(recommend_level)
                * total_spawn_seconds
                * target_pressure
                / (base_hp_ref * enemy_hp_weight)

late_game_t      = clamp((n - 50) / (98 - 50), 0, 1)
wave3plus_hp     *= lerp(1.0, 1.8, late_game_t)
wave3plus_damage *= lerp(1.0, 2.0, late_game_t)
```

- `GLOBAL_HP_BASE / GLOBAL_DMG_BASE` 是全局旋钮；99 关主线实际血量压力由 `tools/rebalance_difficulty.py` 反推并写入 `data/levels.json`。
- 第 1/2 波不吃 50+ 追加曲线，保留局内卡牌成型窗口；第 3 波后同时抬 HP 和压线伤害，避免欠战力配装靠基地血量无限磨怪。
- Boss：`hp_coef` 取 25~60（巨血），并按 `phases` 分段；Boss 关目标压力额外 +0.08。
- 与"建议等级玩家的 DPS 曲线"对照（§6）确保前 5 关放松、中段线性、终局需要接近满强化。

## 4. 伤害与派生（接 `02` 公式，给系数）

```
最终单发 = base_atk(L) * weapon.base_atk_coef * Π(skill_mods)
         * PLAYER_SHOT_DAMAGE_MULT
...
player_dps(L, gear, build) ≈ base_atk(L)*weapon_coef*fire_rate
                              *PLAYER_FIRE_RATE_MULT*PLAYER_SHOT_DAMAGE_MULT
...
战力显示与难度比较使用同一套技能量纲，禁止再只按装备等级画一条推荐曲线：

```
战前战力 = 核心配装战力 * sqrt(永久技能在 4 次基准选卡中的效果倍率)
预计成型 = 核心配装战力 * sqrt(当前关卡 target_card_picks 的最优兼容技能倍率)
终局战力 = 核心配装战力 * sqrt(本局真实已获得技能等级的效果倍率)

推荐战力 = (recommend_level * 6.25 + Boss/晚波加权)
           * card_budget_power_factor(target_card_picks)
```

- 技能效果倍率直接读取 `skills.json` 每级 effect，覆盖伤害、射速、暴击、多重射击及其弹道衰减、穿透、分裂、连锁、追踪、元素持续伤害、破甲、减速和防线生命；纯经济/重抽技能不虚增战力。
- 配装页同时显示“战前 / 成型 / 推荐”，战斗结算显示“战前 → 终局 / 关卡压力”，避免玩家看到 250 对 534 却实际靠局内十次选卡轻松碾压而无法理解原因。
- `target_card_picks > 4` 的关卡从第 3 波起追加静态 `run_skill_pressure`；当前十卡预算约为 `1.54x HP / 1.12x 移速`，挑战模式再独立叠加既定 `1.5x HP`。压力只由关卡表决定，不根据实时表现动态加难。

- 配表脚本（见 `13` 工具）对 99 关跑推荐等级 DPS 与关卡总血量模拟，输出 clear time / leak / star 预估。
- 验收线：**前 5 关应明显低压；6~49 关平滑抬升；50~98 关后半波压力近似线性增长；第 99 关允许单独终局跃升，但毕业 build 不应预测不可过**。当前第 97/98 关的晚波 HP 倍率已达到 `1.783x/1.80x`，压线伤害达到 `1.979x/2.0x`，不得出现倒数关卡空档。
- 终局验收额外跑 `tools/check_endgame_balance.py`：三套满级、属性克制的物理主武器配装均须在 260 秒内通关，至少一套在 180 秒内通关；不要求所有配装都能毕业，明显欠战力且主武器元素错误的方案不得被模型判为舒适通关。
