"""Quiet/read-only Volt idle and firing regression across all weapon skins."""
...
for theme in visual.THEME_IDS:
    for weapon,row in weapons.items():
        ...
        for pose in ('idle','fire'):
            p=dict(level_id='level_001',language='zh',viewport_size=[1080,1920],save_override=copy.deepcopy(visual.PREMIUM_CROSS_SAVE_OVERRIDES[theme]),equipment={'selected_character':'volt','selected_weapon':weapon,'volt':18,weapon:18},_visual_safe_insets=visual.DEBUG_SAFE_INSETS)
            ...
            label=f'{theme}_{weapon}_{pose}_1080x1920'
            cases.append(dict(group='final',route='battle',payload=p,label=label))
            if theme=='default' and weapon=='weapon_scattergun':
                for size in ([1320,2868],[750,1334]):
                    q=copy.deepcopy(p);q['viewport_size']=size
                    cases.append(dict(group='featured',route='battle',payload=q,label=f'{theme}_{weapon}_{pose}_{size[0]}x{size[1]}'))
cases.sort(key=lambda c: ('weapon_scattergun' not in c['label'],c['group']!='featured',c['label']))
...
capture.AUDIT=AUDIT;sys.argv=[__file__,'--case-file',str(path)]+sys.argv[1:]
code=capture.main()
