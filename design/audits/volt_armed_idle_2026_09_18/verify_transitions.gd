...
func _initialize() -> void:
	...
	for weapon in data.get_table("weapons"):
		...
		save.save_data["unlocks"]["characters"].append("volt")
		save.save_data["unlocks"]["weapons"].append(weapon)
		save.save_data["equipment"]["selected_character"] = "volt"
		save.save_data["equipment"]["selected_weapon"] = weapon
		save.save_data["equipment"][weapon] = 1
		# The focused fix covers the eight free weapons; premium masters already
		# keep an armed rest pose and are covered by the unchanged m1 matrix.
		...
		for direction in [Vector2.UP,Vector2(-0.75,-0.66),Vector2(0.75,-0.66)]:
			...
			check(origin.is_equal_approx(battle._weapon_fire_origin()),"armed rest must not alter the muzzle: "+weapon)
			...
			for cycle in range(3):
				...
				check(battle.character_sprite.texture == battle._character_combo_attack_frames()[1],"ignition remains F2: "+weapon)
				battle._process_character_animation(battle.character_attack_duration+0.01)
				battle._process_character_animation(0.01)
				check(battle.character_sprite.texture == battle.character_attack_frames[0],"post-shot rest holds gun: "+weapon)
				battle._play_character_hurt()
				battle._process_character_animation(0.01)
				check(battle.character_sprite.texture == battle.character_attack_frames[0],"hurt does not return to empty hands: "+weapon)
				battle.character_hurt_time = 0.0
			print("ARMED_REST ",weapon," aim=",direction," three fire/rest/hurt cycles passed")
	main.queue_free()
	for i in range(3):await process_frame
	UiKit.release_cached_resources_for_tests()
	print("Volt armed-rest transition regression: %d failures" % failures.size())
