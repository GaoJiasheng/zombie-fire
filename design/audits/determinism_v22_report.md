...
两个已实锤复现用例（level_086/seed 2207、level_090/seed 1103）串行各跑两次，结果层全字段逐位一致，`damage_total` 差值为 0（严于 ≤1e-9 门槛）；额外抽测 6 关 × 3 种子（51/59/66/80/90/95 × 1103/2207/3301）共 18 组 A/B 全部逐位一致；全 99 关 × 3 种子的确定性修复前后对比中，**victory 与档位（grade）零翻转**。战术语义未改，未触碰任何数值/关卡/经济数据/门禁阈值。
