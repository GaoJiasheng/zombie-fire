...
该快照用于解释立项时的改造规模；`ThemeManager` 现已建立，尚未完成语义路由的结构性触点仍按本清单继续收口。
...
- [x] 手机尺度角色和枪械仍可辨认。
- [x] 所有入选图无文字、数字或乱码。
- [x] Phase 1A-2 文件本身仍保留为合成证明；其运行时对应物已在 Phase 1B-2 接入四角色透明帧、独立武器层、确定性攻击帧和枪口 / 周身效果，并由 32 路真实射击截图覆盖完整 `4 × 8` 免费武器组合。
...
**Phase 1B-2b · 霓虹主题结构性场景与完整武器组合收口**

Phase 1B-2a 已经完成四角色肖像、44 张模块化角色帧、独立武器挂载、开火电弧翼、主动技能叠层和可逆预览切换。下一包继续补齐结构性主题，但仍不接真实购买：

1. 把菜单、地图、收藏、配装、设置、战斗和结算的面板、标题框、资源条、选中态和 HUD 框统一成霓虹雷暴语言。
2. 制作独立基地覆盖层并在熔岩、冰川、雷暴和终局四类环境验证，不复制 14 张全尺寸背景。
3. 在已通过完整 `4 × 8` 运行时加载 / 开枪矩阵的基础上，仅对人工复核发现的个别枪口 socket、手掌接触或前臂遮挡问题做定点修正，不回退到大规模合成帧。
4. 保持所有短 / 长控件使用对应原生模型；禁止复用纹理后非等比拉伸。
5. 保持默认主题完全不变；霓虹仍通过开发 fixture 开启。
6. 复跑标准屏 / 长屏、完整视觉矩阵、包体和性能门禁。
